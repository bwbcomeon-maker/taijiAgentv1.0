...
BUILDER_INPUT_HELPER_SHA256="451b0a0ff6fbfd7ad4238cb02eb247d6a50f3e93de181320d5480959ebf2f585"
...
write_source_archive() {
  ...
  /usr/bin/python3 -I -B "$FROZEN_SOURCE_INTEGRITY_HELPER" create \
...
write_builder_input_package() {
  ...
  /usr/bin/python3 -I -B "$FROZEN_BUILDER_INPUT_HELPER" create \
    ...
    || fail "制包机输入包固定清单生成失败"
  /usr/bin/python3 -I -B "$FROZEN_BUILDER_INPUT_HELPER" verify \
    --archive "$staged_output" \
    --manifest "$staged_manifest" \
    --checksum "$staged_checksum" \
    || fail "制包机输入包暂存后三件套回读验证失败"
  verify_frozen_source_identity || fail "发布前冻结 source commit 复核失败"
  /usr/bin/python3 -I -B "$FROZEN_BUILDER_INPUT_HELPER" publish \
    --archive "$staged_output" \
    --manifest "$staged_manifest" \
    --checksum "$staged_checksum" \
    --output "$output" \
