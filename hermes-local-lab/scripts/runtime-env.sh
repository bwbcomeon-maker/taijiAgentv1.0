...
if [ "$TAIJI_LAUNCH_PROFILE" = "installed-production" ]; then
  ...
  case "${TAIJI_SECURITY_PROFILE:-}" in
    strict)
  ...
  if [ "$TAIJI_SECURITY_PROFILE" = "strict" ]; then
    ...
    case "${TAIJI_ALLOW_DELEGATE_TASK:-}" in
      1|true|TRUE|yes|YES|on|ON|y|Y) TAIJI_ALLOW_DELEGATE_TASK="1" ;;
      *) TAIJI_ALLOW_DELEGATE_TASK="0" ;;
    esac
    case "${TAIJI_ALLOW_UNAPPROVED_SKILL_SCRIPTS:-}" in
      1|true|TRUE|yes|YES|on|ON|y|Y) TAIJI_ALLOW_UNAPPROVED_SKILL_SCRIPTS="1" ;;
      *) TAIJI_ALLOW_UNAPPROVED_SKILL_SCRIPTS="0" ;;
    esac
  fi
  export \
    TAIJI_ALLOW_TERMINAL TAIJI_ALLOW_EXECUTE_CODE \
    TAIJI_ALLOW_DELEGATE_TASK TAIJI_ALLOW_UNAPPROVED_SKILL_SCRIPTS
  export \
    TAIJI_AGENT_ROOT TAIJI_AGENT_AGENT_DIR TAIJI_AGENT_WEBUI_DIR \
    TAIJI_AGENT_PYTHON TAIJI_WEBUI_PYTHON TAIJI_WEBUI_AGENT_DIR \
