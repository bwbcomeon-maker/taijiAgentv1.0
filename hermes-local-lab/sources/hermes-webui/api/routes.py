...
_WRITEFLOW_ARTIFACTS = {
    "01_theme": "01_theme.md",
    "02_cases": "02_cases.md",
    "03_outline": "03_outline.md",
    "04_empathy_map": "04_empathy_map.md",
    "05_concrete_library": "05_concrete_library.md",
    "draft_v1": "draft_v1.md",
    "draft_v2": "draft_v2.md",
    "review_report": "review_report.md",
    "pre_publish_report": "pre_publish_report.md",
    "reader_test": "reader_test.md",
    "draft_final": "draft_final.md",
    "export": "export.md",
}
_WRITEFLOW_ACTIONS = {"start", "status", "next", "redo", "skip", "export", "style", "extract"}
_WRITEFLOW_ACTION_ALIASES = {
    "开始": "start",
    "启动": "start",
    "新建": "start",
    "状态": "status",
...
_WRITEFLOW_ARTIFACT_LABELS = {
    ...
    "style_model": "风格模型",
    "style_report": "风格分析报告",
    "extracted_article": "提取正文",
    "source_article": "提取正文",
    "material_notes": "素材整理稿",
}
_WRITEFLOW_TEAMS = {
    ...
    "style-modeler": {
        "title": "风格建模专家",
        "category": "风格模型",
        "status_label": "风格沉淀到工作区",
        "description": "从样稿中提炼语气、句式、节奏和表达偏好，沉淀为可复用写作风格。",
        "default_mode": "A",
        "default_action": "style",
        "tags": ["风格提取", "语气复用", "样稿分析"],
        "image": "static/assets/writeflow/team-style-modeler.png",
        "examples": [
            {
                "id": "style-from-samples",
                "label": "样稿建模",
                "prompt": "请基于我放在 workspace 里的样稿，提炼我的写作风格，包括开头方式、句式节奏、观点表达、案例使用和禁用表达，并保存为可复用风格模型。",
            },
            {
                "id": "style-apply",
                "label": "风格套用",
                "prompt": "请把当前文章改成我常用的专业实战风格：少空话、多判断、有具体场景和行动建议。先说明会调整哪些风格维度。",
            },
        ],
        "members": [
            {"id": "style-modeler", "name": "风格建模师", "role": "风格提取", "skill": "style-modeler", "responsibility": "读取样稿、建立风格模型并保存到风格库。"},
            {"id": "workflow-producer", "name": "写作总导演", "role": "应用风格", "skill": "workflow-producer", "responsibility": "把风格约束带入后续写作流程。"},
        ],
    },
    "web-article-extractor": {
        ...
        "tags": ["网页提取", "Markdown", "素材整理"],
        "image": "static/assets/writeflow/team-extractor.png",
        "examples": [
            {"id": "extract-url", "label": "提取链接", "prompt": "请提取这个网页里的文章正文、标题、作者、发布时间和关键小标题，并保存为 Markdown。链接："},
...
_WRITEFLOW_RUN_STATUSES = {"running", "waiting_user", "done", "error", "blocked"}
_WRITEFLOW_RUN_TERMINAL_STATUSES = {"done", "error", "blocked"}
_WRITEFLOW_TASK_STATUSES = {"pending", "running", "waiting_user", "done", "blocked", "error"}


_WRITEFLOW_COMMON_EVENT_RULES = {
    "direction_done": {
        "markers": [
            "确定方向」已完成",
            "确定方向阶段完成",
...
_WRITEFLOW_STATE_MACHINES = {
    "content-creator-team": {
        ...
        "event_rules": {
            ...
            "image_done": {"markers": ["封面图已完成", "配图已完成", "图片已生成"]},
        },
        "tasks": [
            {
                "id": "direction",
                ...
                "description": "确认主题、读者、目标和素材边界。",
                "phase": "确定方向",
                "done_events": ["direction_done"],
                "artifact_ids": ["01_theme"],
                "done_label": "方向已完成",
                "pending_label": "待确定",
                "auto_start": True,
            },
            {
                ...
                "done_events": ["draft_done"],
                "artifact_ids": ["draft_v1", "draft_v2", "draft_final"],
                "done_artifact_ids": ["draft_v1", "draft_v2", "draft_final"],
                "requires_artifact": True,
                "done_label": "初稿已完成",
                "pending_label": "待写作",
                "auto_start_after_previous": True,
            },
            {
                "id": "illustrations",
                "title": "生成版式和配图建议",
                ...
                "artifact_ids": ["illustration_prompts", "image_cover", "cover"],
                "artifact_path_contains": ["/imgs/prompts/"],
                "done_artifact_kinds": ["image"],
                "waiting_artifact_ids": ["illustration_prompts"],
                "waiting_artifact_kinds": ["prompt"],
                "requires_artifact": True,
                "done_label": "配图已完成",
                "waiting_label": "提示词已生成，待出图",
                "pending_label": "待执行",
            },
            {
                "id": "review",
                ...
                "artifact_ids": ["review_report", "pre_publish_report", "export", "draft_final"],
                "done_artifact_ids": ["review_report", "pre_publish_report", "export", "draft_final"],
                "requires_artifact": True,
                "done_label": "审稿已完成",
                "pending_label": "待审稿",
            },
    ...
    "deep-research-team": {
        "next_actions": {
            "running": ["继续定方向", "补充资料", "调整要求"],
            "waiting_user": ["开始写初稿", "调整大纲", "补充案例", "确认标题"],
            "done": ["审稿", "配图", "直接看稿", "调整方向"],
        },
        "event_rules": {
            "research_started": {
                "markers": ["启动调研", "开始调研", "正在调研", "资料研究员会", "搜索和整理真实案例", "Google被拦截", "换个方式搜索"],
            },
        ...
        "tasks": [
            {
                ...
                "title": "确定研究方向",
                "worker_id": "workflow-producer",
                "description": "确认主题、读者、目标和资料边界。",
                "phase": "确定方向",
                "done_events": ["direction_done", "research_started"],
                "artifact_ids": ["01_theme"],
                "done_label": "方向已完成",
                "pending_label": "待确定",
                "auto_start": True,
            },
            {
                ...
                "description": "收集案例、事实、论据和素材线索。",
                "phase": "确定方向",
                "start_events": ["research_started"],
                "done_events": ["research_done"],
                "artifact_ids": ["02_cases"],
                "done_artifact_ids": ["02_cases"],
                "requires_artifact": True,
                "done_requires_event": True,
                "done_label": "素材已完成",
                "pending_label": "待调研",
            },
            {
                ...
                "phase": "确定方向",
                "phase_on_done": "生成初稿",
                "start_events": ["outline_started", "research_done"],
                "done_events": ["outline_done"],
                "artifact_ids": ["03_outline"],
                "done_artifact_ids": ["03_outline"],
                "requires_artifact": True,
                "done_requires_event": True,
                "done_label": "大纲已完成",
                "pending_label": "待规划",
    ...
    "style-modeler": {
        "next_actions": {
            "running": ["继续建模", "补充样稿", "调整风格目标"],
            "waiting_user": ["套用风格", "继续优化", "下载风格模型"],
            "done": ["套用风格", "新建写作任务", "下载风格模型"],
        },
        "event_rules": {
            "style_input_done": {"markers": ["读取样稿", "样稿分析", "会调整哪些风格维度", "风格维度"]},
            "style_model_done": {"markers": ["风格模型已完成", "风格模型", "沉淀为可复用"]},
            "style_apply_started": {"markers": ["套用风格", "应用风格", "改成我常用"]},
        },
        "tasks": [
            {
                "id": "style_input",
                "title": "读取样稿和风格目标",
                "worker_id": "style-modeler",
                "description": "确认样稿来源、风格目标和禁用表达。",
                "phase": "确定方向",
                "done_events": ["style_input_done"],
                "done_label": "样稿已读取",
                "pending_label": "待读取",
                "auto_start": True,
            },
            {
                ...
                "title": "提炼风格模型",
                "worker_id": "style-modeler",
                "description": "提炼语气、句式、节奏、案例使用和禁用表达。",
                "phase": "生成初稿",
                "phase_on_done": "打磨发布",
                "start_events": ["style_input_done", "style_model_done"],
                "done_events": ["style_model_done"],
                "artifact_ids": ["style_model", "style_report"],
                "artifact_path_contains": ["articles/_styles/"],
                "done_artifact_ids": ["style_model", "style_report"],
                "done_artifact_path_contains": ["articles/_styles/"],
                "requires_artifact": True,
                "done_label": "风格模型已完成",
                "pending_label": "待建模",
                "auto_start_after_previous": True,
            },
            {
                ...
                "title": "应用风格到写作",
                "worker_id": "workflow-producer",
                "description": "把风格约束带入后续文章或改写任务。",
                "phase": "打磨发布",
                "start_events": ["style_apply_started"],
                "artifact_ids": ["draft_v1", "draft_v2", "draft_final"],
                "done_artifact_ids": ["draft_v1", "draft_v2", "draft_final"],
                "requires_artifact": True,
                "done_label": "风格已应用",
    ...
    "web-article-extractor": {
        "next_actions": {
            "running": ["继续提取", "更换链接", "整理素材"],
            "waiting_user": ["整理成素材", "用于写作", "下载正文"],
            "done": ["整理成素材", "新建写作任务", "下载正文"],
        },
        "event_rules": {
            "url_parsed": {"markers": ["链接解析", "开始提取", "网页提取专家"]},
            "article_extracted": {"markers": ["正文已提取", "提取正文", "保存为Markdown", "保存为Markdown"]},
            "material_done": {"markers": ["素材整理完成", "整理成素材库", "可用于写作的观点"]},
        },
        "tasks": [
            {
                "id": "parse",
                "title": "解析网页链接",
                "worker_id": "web-article-extractor",
                "description": "确认链接可访问并识别标题、来源和结构。",
                "phase": "确定方向",
                "done_events": ["url_parsed", "article_extracted"],
                "done_label": "链接已解析",
                "pending_label": "待解析",
                "auto_start": True,
            },
            {
                ...
                "title": "提取网页正文",
                "worker_id": "web-article-extractor",
                "description": "提取正文、标题、作者、发布时间和关键小标题。",
                "phase": "生成初稿",
                "phase_on_done": "打磨发布",
                "start_events": ["url_parsed", "article_extracted"],
                "done_events": ["article_extracted"],
                "artifact_ids": ["extracted_article", "source_article"],
                "done_artifact_ids": ["extracted_article", "source_article"],
                "requires_artifact": True,
                "done_label": "正文已提取",
                "pending_label": "待提取",
                "auto_start_after_previous": True,
            },
            {
                ...
                "title": "整理成写作素材",
                "worker_id": "research-expert",
                "description": "把提取内容归类成后续写作可用的观点、案例和金句。",
                "phase": "打磨发布",
                "start_events": ["material_done"],
                "done_events": ["material_done"],
                "artifact_ids": ["material_notes", "02_cases"],
                "done_artifact_ids": ["material_notes", "02_cases"],
                "requires_artifact": True,
                "done_label": "素材已整理",
                "pending_label": "待整理",
            },
...
def _writeflow_now() -> str:
    return datetime.now().astimezone().isoformat(timespec="seconds")


def _writeflow_team_template(team_id: str) -> dict:
    team = _WRITEFLOW_TEAMS.get(team_id) or _WRITEFLOW_TEAMS["content-creator-team"]
    return {
        "id": team_id if team_id in _WRITEFLOW_TEAMS else "content-creator-team",
        "title": team["title"],
        "category": team["category"],
        "status_label": team["status_label"],
        "description": team["description"],
        "default_mode": team.get("default_mode", "A"),
        "default_action": team.get("default_action", "start"),
        "tags": list(team.get("tags") or []),
        "image": team.get("image") or "",
        "members": [dict(member) for member in team.get("members") or []],
        "examples": [dict(example) for example in team.get("examples") or []],
    }


def _writeflow_team_templates() -> list[dict]:
    return [_writeflow_team_template(team_id) for team_id in _WRITEFLOW_TEAMS]


def _writeflow_public_team_lines(team_id: str) -> list[str]:
    ...
    if team_id == "style-modeler":
        return [
            "- 风格建模师先读样稿，提炼常用语气、句式和节奏。",
            "- 写作负责人把这些风格要求变成后续文章可复用的规则。",
        ]
    if team_id == "web-article-extractor":
        return [
            "- 网页提取专家先把链接里的正文、标题和关键信息摘出来。",
            "- 素材整理员再把内容归类成后续写作可以直接使用的素材。",
        ]
    return [
...
def _writeflow_public_member_lines(team_id: str, team: dict) -> list[str]:
    if team_id == "deep-research-team":
        return [
    ...
    if team_id == "style-modeler":
        return [
            "- 风格建模师（风格提取）：从样稿里总结语气、句式、节奏和禁用表达。",
            "- 写作总导演（风格应用）：把风格要求带入后续文章，让输出更一致。",
        ]
    if team_id == "web-article-extractor":
        return [
            "- 网页提取专家（文章提取）：从链接中整理标题、正文和关键小标题。",
            "- 素材整理员（素材整理）：把提取内容归类成后续写作可用的素材。",
        ]
...
def _writeflow_public_artifact_requirement_lines(team_id: str) -> list[str]:
    ...
    if team_id == "web-article-extractor":
        return [
            "- 主要通过聊天窗口和用户确认，不要让用户理解内部 10 阶段细节。",
            "- 提取阶段要形成：网页标题、来源信息、正文 Markdown。",
            "- 素材整理阶段要形成：观点、案例、金句和可用于写作的素材清单。",
            "- 链接不可访问或正文未实际提取时，不要写成已完成。",
            "- 需要分工时，按成员职责推进，不要向用户暴露内部调度方式。",
            "- 每轮结束必须用中文返回：本轮结论、已产出的成果物类型、当前三步位置、下一步可选操作。",
        ]
    return [
...
def _writeflow_normalize_action(value: str) -> str:
    raw = str(value or "status").strip()
    action = _WRITEFLOW_ACTION_ALIASES.get(raw, raw.lower())
    if action not in _WRITEFLOW_ACTIONS:
        return "status"
    return action


def _writeflow_display_artifacts(artifacts: dict) -> dict:
    display = {}
    for key, rel in artifacts.items():
        label = _WRITEFLOW_ARTIFACT_LABELS.get(str(key), str(key))
        display[label] = rel
    return display


def _writeflow_normalize_team_id(value: str | None, action: str = "start") -> str:
    raw = str(value or "").strip()
    if raw in _WRITEFLOW_TEAMS:
        return raw
    if action == "style":
        return "style-modeler"
    if action == "extract":
        return "web-article-extractor"
    return "content-creator-team"


def _writeflow_team_summary(meta: dict, team_id: str) -> dict:
    existing = meta.get("display_team") if isinstance(meta.get("display_team"), dict) else None
    if existing:
        return {
            "id": str(existing.get("id") or team_id),
            "title": str(existing.get("title") or existing.get("name") or team_id),
            "category": str(existing.get("category") or ""),
            "status_label": str(existing.get("status_label") or existing.get("statusLabel") or ""),
        }
    team = _WRITEFLOW_TEAMS.get(team_id) or _WRITEFLOW_TEAMS["content-creator-team"]
    return {
        "id": team_id,
...
def _writeflow_member_status(meta: dict, member_index: int) -> str:
    ...
    if status in {"error", "failed", "blocked"}:
        return "需要处理"
    if status in {"waiting_user", "waiting", "wait", "needs_confirmation"}:
        return "等待确认" if member_index == 0 else ("已完成" if artifacts else "待命")
    if status in {"running", "in_progress", "working", "drafting"}:
        return "处理中" if member_index <= 1 else "待命"
    return "待命"


def _writeflow_member_summaries(meta: dict, team_id: str) -> list[dict]:
    existing = meta.get("display_members")
    if isinstance(existing, list):
        ...
        for item in existing:
            ...
            rows.append(
                {
                    "id": str(item.get("id") or item.get("skill") or item.get("name") or ""),
                    "name": str(item.get("name") or item.get("id") or ""),
                    "role": str(item.get("role") or ""),
                    "skill": str(item.get("skill") or ""),
                    "status": str(item.get("status") or "待命"),
                }
            )
        if rows:
            return rows
    ...
    return [
        {
            ...
            "role": member["role"],
            "skill": member["skill"],
            "status": _writeflow_member_status(meta, idx),
        }
        for idx, member in enumerate(team["members"])
    ]


def _writeflow_display_phase(meta: dict) -> str:
    artifacts = meta.get("artifacts") if isinstance(meta.get("artifacts"), dict) else {}
    keys = set(str(k) for k in artifacts.keys())
    stage_raw = str(meta.get("stage") or "").strip().lower()
    try:
        stage_num = int(stage_raw)
    except (TypeError, ValueError):
        stage_num = None
    if keys & {"review_report", "pre_publish_report", "reader_test", "draft_final"}:
        return "打磨发布"
    if stage_num is not None and stage_num >= 7:
        return "打磨发布"
    if "draft_v1" in keys:
        return "生成初稿"
    if stage_num is not None and stage_num >= 4:
        return "生成初稿"
    return "确定方向"


def _writeflow_display_status(value: str) -> str:
...
def _writeflow_empty_state() -> dict:
    return {"version": 1, "active_project": None, "projects": {}}


def _writeflow_slug(value: str) -> str:
    raw = str(value or "").strip()
    if not raw:
        return "untitled-writeflow"
    slug = re.sub(r"[^0-9A-Za-z\u4e00-\u9fff._-]+", "-", raw).strip("-._")
    return slug[:80] or "untitled-writeflow"


def _writeflow_workspace(session_id: str | None = None) -> Path:
    if session_id:
...
def _writeflow_runs_dir(workspace: Path) -> Path:
    return workspace / "articles" / ".writeflow" / "runs"


def _writeflow_safe_run_id(value: str) -> str:
    run_id = str(value or "").strip()
    if not run_id or not re.fullmatch(r"[A-Za-z0-9_.-]{1,96}", run_id):
        raise ValueError("Invalid writeflow run_id")
    return run_id


def _writeflow_run_path(workspace: Path, run_id: str) -> Path:
    return _writeflow_runs_dir(workspace) / f"{_writeflow_safe_run_id(run_id)}.json"


def _writeflow_write_json(path: Path, payload: dict) -> None:
...
def _writeflow_persistable_run(run: dict) -> dict:
    row = dict(run or {})
    row.pop("run_path", None)
    return row


def _writeflow_write_if_changed(path: Path, current: dict, refreshed: dict) -> None:
    next_payload = _writeflow_persistable_run(refreshed)
    if _writeflow_persistable_run(current) != next_payload:
        _writeflow_write_json(path, next_payload)


def _writeflow_read_run(workspace: Path, run_id: str) -> tuple[dict | None, str | None]:
    path = _writeflow_run_path(workspace, run_id)
    if not path.exists():
        return None, "team_run not found"
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
    except Exception as exc:
        return None, f"team_run JSON 无效：{exc}"
    if not isinstance(data, dict):
        return None, "team_run JSON 无效：根节点必须是对象"
...
def _writeflow_read_state(workspace: Path) -> tuple[dict, bool, str | None]:
    path = _writeflow_state_path(workspace)
    if not path.exists():
        return _writeflow_empty_state(), False, None
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
    except Exception as exc:
        return _writeflow_empty_state(), True, f"写作状态文件 JSON 无效：{exc}"
    if not isinstance(data, dict):
        return _writeflow_empty_state(), True, "写作状态文件 JSON 无效：根节点必须是对象"
...
def _writeflow_image_generation_ready() -> bool:
    ...
    if not isinstance(image_cfg, dict):
        return bool(os.getenv("FAL_KEY") or os.getenv("FAL_API_KEY") or os.getenv("OPENAI_API_KEY") or os.getenv("XAI_API_KEY"))
    ...
    if any(str(image_cfg.get(field) or "").strip() for field in key_fields):
        return True
    env_name = str(image_cfg.get("key_env") or image_cfg.get("api_key_env") or "").strip()
    if env_name and os.getenv(env_name):
        return True
    return bool(os.getenv("FAL_KEY") or os.getenv("FAL_API_KEY") or os.getenv("OPENAI_API_KEY") or os.getenv("XAI_API_KEY"))


def _writeflow_status_label(value: str) -> str:
    status = str(value or "").strip().lower()
    if status in {"running", "in_progress", "working", "drafting"}:
        return "执行中"
    if status in {"waiting_user", "waiting", "wait", "needs_confirmation"}:
    ...
    if status == "pending":
        return "待执行"
    return str(value or "未开始")


def _writeflow_artifact_kind(rel_path: str, key: str = "") -> str:
    suffix = Path(str(rel_path or "")).suffix.lower()
    rel_lower = str(rel_path or "").lower().replace("\\", "/")
    key_lower = str(key or "").lower()
    if suffix in {".png", ".jpg", ".jpeg", ".gif", ".webp", ".avif", ".svg"}:
        return "image"
    if suffix in {".md", ".markdown", ".txt"}:
        if (
            "prompt" in key_lower
            or "prompt" in rel_lower
            or "/imgs/prompts/" in rel_lower
            or rel_lower.endswith("/illustration_prompts.md")
        ):
            return "prompt"
        return "markdown"
    return "file"
...
_WRITEFLOW_ARTIFACT_PATH_RE = re.compile(
    r"articles/[^\s`\"'<>]+?\.(?:md|markdown|txt|png|jpg|jpeg|webp|gif|svg|pdf|docx)",
    re.IGNORECASE,
)
_WRITEFLOW_ARTICLE_DIR_RE = re.compile(r"articles/[^\s`\"'<>]+?/", re.IGNORECASE)
_WRITEFLOW_BARE_ARTIFACT_RE = re.compile(
    r"(?<![/\\])([A-Za-z0-9][A-Za-z0-9_.-]*\.(?:md|markdown|txt|png|jpg|jpeg|webp|gif|svg|pdf|docx))",
    re.IGNORECASE,
)
_WRITEFLOW_TOP_LEVEL_ARTIFACT_NAMES = {
    Path(name).name.lower()
    for name in (
        *(_WRITEFLOW_ARTIFACTS.values()),
...
def _writeflow_clean_artifact_path(value: str) -> str:
    ...
    if path.is_absolute() or ".." in path.parts:
        return ""
    if rel.startswith("articles/.writeflow/"):
        return ""
    return rel


def _writeflow_artifact_paths_from_text(text: str, project_slug: str = "") -> list[str]:
    if not text:
        return []
    rows = []
    seen = set()
    ...
    def append(rel_path: str) -> None:
        rel = _writeflow_clean_artifact_path(rel_path)
        if rel and rel not in seen:
            seen.add(rel)
            rows.append(rel)

    for match in _WRITEFLOW_ARTIFACT_PATH_RE.finditer(text_s):
        append(match.group(0))

    article_dirs: list[str] = []
    for rel in rows:
        parts = rel.split("/")
        if len(parts) >= 3:
            article_dirs.append("/".join(parts[:2]) + "/")
    for match in _WRITEFLOW_ARTICLE_DIR_RE.finditer(text_s):
        article_dir = _writeflow_clean_artifact_path(f"{match.group(0).rstrip('/')}/draft_v1.md")
        if article_dir:
            article_dirs.append("/".join(article_dir.split("/")[:2]) + "/")
    if project_slug:
        article_dirs.append(f"articles/{project_slug}/")

    unique_article_dirs = []
    seen_dirs = set()
    for article_dir in article_dirs:
        if article_dir not in seen_dirs:
            seen_dirs.add(article_dir)
            unique_article_dirs.append(article_dir)

    if unique_article_dirs:
        for match in _WRITEFLOW_BARE_ARTIFACT_RE.finditer(text_s):
            filename = match.group(1).strip()
            if filename.lower() not in _WRITEFLOW_TOP_LEVEL_ARTIFACT_NAMES:
                continue
            for article_dir in unique_article_dirs:
                append(f"{article_dir}{filename}")
    return rows


def _writeflow_external_article_roots(workspace: Path) -> list[Path]:
    roots: list[Path] = []
    for raw in str(os.getenv("HERMES_WRITEFLOW_ARTICLE_ROOTS") or "").split(os.pathsep):
        if raw.strip():
            roots.append(Path(raw).expanduser())
    try:
        sources_root = Path(__file__).resolve().parents[2]
        roots.append(sources_root / "hermes-agent" / "articles")
    except Exception:
        pass
    try:
        roots.append(Path(workspace).resolve().parent / "sources" / "hermes-agent" / "articles")
    except Exception:
        pass
    workspace_articles = (Path(workspace) / "articles").resolve()
    unique = []
    seen = set()
    for root in roots:
        try:
            ...
            continue
        if resolved == workspace_articles or not resolved.exists() or not resolved.is_dir():
            continue
        key = str(resolved)
        if key in seen:
            continue
...
def _writeflow_materialize_article_artifact_info(workspace: Path, rel_path: str) -> tuple[bool, str]:
    rel = _writeflow_clean_artifact_path(rel_path)
    if not rel:
        return False, ""
    try:
        target = safe_resolve(Path(workspace), rel)
    except Exception:
        return False, ""
    if target.is_file():
        return True, ""
    try:
        article_rel = Path(rel).relative_to("articles")
    except Exception:
        return False, ""
    for root in _writeflow_external_article_roots(workspace):
        try:
            resolved_root = root.resolve()
            source = (root / article_rel).resolve()
        ...
        try:
            source_label = source.relative_to(resolved_root).as_posix()
        except Exception:
            source_label = article_rel.as_posix()
        return True, source_label
    return False, ""


def _writeflow_materialize_article_artifact(workspace: Path, rel_path: str) -> bool:
    exists, _source = _writeflow_materialize_article_artifact_info(workspace, rel_path)
    return exists


def _writeflow_artifact_label_for_path(rel_path: str, key: str = "", fallback: str = "") -> str:
    key_s = str(key or "").strip()
    if key_s in _WRITEFLOW_ARTIFACT_LABELS:
        return _WRITEFLOW_ARTIFACT_LABELS[key_s]
    stem = Path(str(rel_path or "")).stem
    if stem in _WRITEFLOW_ARTIFACT_LABELS:
        return _WRITEFLOW_ARTIFACT_LABELS[stem]
    fallback_s = str(fallback or "").strip()
    if fallback_s and not re.fullmatch(r"[A-Za-z0-9_.-]+", fallback_s):
        return fallback_s
    kind = _writeflow_artifact_kind(rel_path, key_s)
    if "articles/_styles/" in str(rel_path or "").lower().replace("\\", "/"):
        return "风格模型"
    if kind == "image":
        return "文章配图"
    if kind == "prompt":
...
def _writeflow_artifact_download_name(label: str, rel_path: str) -> str:
    suffix = Path(str(rel_path or "")).suffix or ".md"
    safe_label = re.sub(r'[\\/:*?"<>|]+', "", str(label or "写作产物")).strip() or "写作产物"
    return f"{safe_label}{suffix}"


def _writeflow_artifact_record(
    item,
    *,
    project_slug: str = "",
    ...
    raw_path = raw.get("path") or raw.get("file") or raw.get("rel_path") or raw.get("name") or ""
    rel = _writeflow_clean_artifact_path(str(raw_path))
    if not rel:
        name = str(raw.get("name") or "").strip()
        if project_slug and name:
            rel = _writeflow_clean_artifact_path(f"articles/{project_slug}/{name}")
    if not rel:
        return None
    ...
    if workspace is not None:
        exists, materialized_from = _writeflow_materialize_article_artifact_info(workspace, rel)
        if require_existing and not exists and not placeholder:
            return None
    key = str(raw.get("id") or raw.get("key") or Path(rel).stem or "").strip()
    kind = str(raw.get("kind") or "").strip() or _writeflow_artifact_kind(rel, key)
    ...
    if not task_id:
        ...
        if stem == "01_theme":
            ...
            task_id = "research"
        elif stem == "03_outline":
            task_id = "outline"
        elif stem in {"review_report", "pre_publish_report", "reader_test", "export"}:
            task_id = "review"
        elif stem in {"style_model", "style_report"} or "articles/_styles/" in rel_lower:
            task_id = "style_model"
        elif stem in {"extracted_article", "source_article"}:
            task_id = "extract"
        elif stem == "material_notes":
            task_id = "organize"
        elif kind in {"image", "prompt"} or "image" in key:
            task_id = "illustrations"
        else:
            task_id = "draft"
    label = str(raw.get("label") or "").strip() or _writeflow_artifact_label_for_path(
        rel,
        key,
        str(raw.get("type") or raw.get("description") or ""),
    )
    status = str(raw.get("status") or ("planned" if placeholder else "ready")).strip()
    origin = str(raw.get("origin") or raw.get("source") or ("placeholder" if placeholder else "")).strip()
    current_run_owned_raw = raw.get("current_run_owned")
    current_run_owned = current_run_owned_raw if isinstance(current_run_owned_raw, bool) else None
    return {
        "id": key or Path(rel).stem,
        "label": label,
        "kind": kind,
        "path": rel,
        "task_id": task_id,
        ...
        "exists": bool(exists or (workspace is None and not require_existing)),
        "download_name": _writeflow_artifact_download_name(label, rel),
        "note": str(raw.get("note") or raw.get("description") or ""),
        "origin": origin,
        "source_run_id": str(raw.get("source_run_id") or raw.get("run_id") or ""),
        "source_session_id": str(raw.get("source_session_id") or raw.get("session_id") or ""),
        "source_path": str(raw.get("source_path") or materialized_from or ""),
        "change_type": str(raw.get("change_type") or ""),
        "visible_scope": str(raw.get("visible_scope") or raw.get("scope") or ""),
        "stale_reason": str(raw.get("stale_reason") or ""),
        **({"current_run_owned": current_run_owned} if current_run_owned is not None else {}),
    }


def _writeflow_merge_artifact(rows: list[dict], item: dict | None) -> None:
    if not item:
        return
    ...
    for idx, existing in enumerate(rows):
        existing_key = existing.get("path") or existing.get("id")
        if existing_key == key:
            merged = {**existing, **{k: v for k, v in item.items() if v not in (None, "")}}
            if existing.get("origin") in {"session_text", "historical_match", "external_recovery"} and item.get("origin") == "project_scan":
                merged["origin"] = existing.get("origin")
            if existing.get("source_path") and not item.get("source_path"):
                merged["source_path"] = existing.get("source_path")
            rows[idx] = merged
            return
    rows.append(item)


def _writeflow_parse_time(value: str) -> float | None:
    raw = str(value or "").strip()
    if not raw:
        return None
    try:
...
def _writeflow_artifact_root_for_run(run: dict) -> str:
    root = _writeflow_clean_artifact_path(str(run.get("artifact_root") or ""))
    if root:
        return root.rstrip("/")
    project_slug = _writeflow_slug(run.get("project_slug") or run.get("title") or "")
    return _writeflow_clean_artifact_path(f"articles/{project_slug}").rstrip("/")


def _writeflow_path_is_under(rel_path: str, root: str) -> bool:
    rel = _writeflow_clean_artifact_path(rel_path).rstrip("/")
    root_s = _writeflow_clean_artifact_path(root).rstrip("/")
    return bool(rel and root_s and (rel == root_s or rel.startswith(f"{root_s}/")))


def _writeflow_artifact_mtime(workspace: Path, item: dict) -> float | None:
    rel = _writeflow_clean_artifact_path(str((item or {}).get("path") or ""))
    if not rel:
        return None
    try:
        path = safe_resolve(Path(workspace), rel)
    except Exception:
        return None
    try:
...
def _writeflow_artifact_current_for_run(workspace: Path, run: dict, item: dict) -> bool:
    if not item:
        return False
    ...
    if item.get("exists") is False:
        return False
    explicit_owned = item.get("current_run_owned")
    if isinstance(explicit_owned, bool):
        return explicit_owned
    origin = str(item.get("origin") or "").strip()
    if origin == "current_run":
        return True
    if str(item.get("source_run_id") or "") and str(item.get("source_run_id") or "") == str(run.get("run_id") or ""):
        return True
    rel = str(item.get("path") or "")
    under_run_root = _writeflow_path_is_under(rel, _writeflow_artifact_root_for_run(run))
    style_team_artifact = str(run.get("team_id") or "") == "style-modeler" and _writeflow_path_is_under(rel, "articles/_styles")
    if not under_run_root and not style_team_artifact:
        return False
    run_ts = _writeflow_parse_time(str(run.get("created_at") or ""))
    mtime = _writeflow_artifact_mtime(workspace, item)
    if run_ts is None or mtime is None:
        return False
    return mtime + 1 >= run_ts


def _writeflow_artifact_reference_reason(run: dict, item: dict) -> str:
    if item.get("stale_reason"):
        return str(item.get("stale_reason") or "")
    origin = str(item.get("origin") or "").strip()
    if origin == "historical_match":
        return "来自同主题历史目录"
    if origin == "session_text":
        return "来自聊天文本提及，未确认为本轮产物"
    if origin == "external_recovery":
        return "来自外部目录回收，未确认为本轮产物"
    if not _writeflow_path_is_under(str(item.get("path") or ""), _writeflow_artifact_root_for_run(run)):
        return "不在本轮产物目录"
    return "早于本轮运行或未登记为本轮产物"


def _writeflow_split_current_and_reference_artifacts(
    workspace: Path,
    run: dict,
    artifacts: list[dict],
    ...
    for item in artifacts:
        if not item:
            continue
        if _writeflow_artifact_current_for_run(workspace, run, item):
            scoped = {**item, "visible_scope": item.get("visible_scope") or "current", "current_run_owned": True}
            _writeflow_merge_artifact(current_rows, scoped)
            continue
        if item.get("placeholder") or item.get("exists") is False:
            continue
        scoped = {
            **item,
            "visible_scope": item.get("visible_scope") or "reference",
            "current_run_owned": False,
            "stale_reason": _writeflow_artifact_reference_reason(run, item),
        }
        _writeflow_merge_artifact(reference_rows, scoped)
    current_paths = {str(item.get("path") or "") for item in current_rows if item.get("path")}
    reference_rows = [item for item in reference_rows if str(item.get("path") or "") not in current_paths]
    return current_rows, reference_rows


def _writeflow_project_slug_from_artifact_path(rel_path: str) -> str:
    rel = _writeflow_clean_artifact_path(rel_path)
    parts = rel.split("/")
...
def _writeflow_session_artifact_paths(run: dict, project_slug: str = "") -> list[str]:
    sid = str(run.get("session_id") or "").strip()
    if not sid or sid.startswith("wr-"):
        return []
    try:
        session = get_session(sid)
    except Exception:
    ...
    def add_from_text(text: str) -> None:
        for rel in _writeflow_artifact_paths_from_text(text, project_slug):
            if rel not in seen:
                seen.add(rel)
                rows.append(rel)
    ...
    for msg in getattr(session, "messages", []) or []:
        if isinstance(msg, dict):
            add_from_text(str(msg.get("content") or ""))
    for tc in getattr(session, "tool_calls", []) or []:
        if not isinstance(tc, dict):
            continue
        for field in ("preview", "snippet", "result", "output"):
            add_from_text(str(tc.get(field) or ""))
        for field in ("args", "arguments", "input"):
            value = tc.get(field)
            if isinstance(value, str):
                add_from_text(value)
            elif isinstance(value, dict):
                add_from_text(json.dumps(value, ensure_ascii=False))
    return rows


def _writeflow_session_messages(session_id: str) -> list[dict]:
    ...
    try:
        ...
        return []
    return [msg for msg in (getattr(session, "messages", []) or []) if isinstance(msg, dict)]


def _writeflow_session_topic_for_run(run: dict) -> str:
    texts = [_writeflow_sidebar_message_text(msg) for msg in _writeflow_session_messages(str(run.get("session_id") or ""))]
    ...
    for pattern in (
        ...
        candidate = re.sub(r"^(帮我|请|围绕)", "", candidate).strip()
        if candidate:
            return candidate[:80]
    return str(run.get("title") or run.get("project_slug") or "").strip()


def _writeflow_article_roots_for_discovery(workspace: Path) -> list[Path]:
    roots: list[Path] = []
    local_root = Path(workspace) / "articles"
...
def _writeflow_topic_match_score(topic: str, slug: str, sample: str) -> int:
    topic_s = str(topic or "").strip()
    haystack = f"{slug}\n{sample}".lower()
    compact_haystack = re.sub(r"\s+", "", haystack)
    compact_topic = re.sub(r"\s+", "", topic_s.lower())
    score = 0
    if compact_topic and compact_topic in compact_haystack:
        score += 8
    for word in set(re.findall(r"[a-z0-9]+", topic_s.lower())):
        if len(word) >= 2 and word in haystack:
            score += 1
    for marker in ("企业", "本地", "工作台", "数据", "成本", "安全", "合规", "案例", "技术", "科普", "产品", "发布"):
        if marker in topic_s and marker in sample:
            score += 2
    return score


def _writeflow_matching_article_slug(workspace: Path, run: dict) -> str:
    topic = _writeflow_session_topic_for_run(run)
    if not topic:
        return ""
    best: tuple[int, str] = (0, "")
    for root in _writeflow_article_roots_for_discovery(workspace):
        try:
            children = sorted(root.iterdir(), key=lambda item: item.name)
        except Exception:
            continue
        for child in children:
            if not child.is_dir() or child.name.startswith(".") or child.name == "_styles":
                continue
            samples: list[str] = []
            for filename in ("01_theme.md", "02_cases.md", "03_outline.md", "draft_v1.md"):
                path = child / filename
                if not path.is_file():
                    continue
                try:
                    samples.append(path.read_text(encoding="utf-8", errors="ignore")[:6000])
                except Exception:
                    continue
            if not samples:
                continue
            score = _writeflow_topic_match_score(topic, child.name, "\n".join(samples))
            if score > best[0]:
                best = (score, child.name)
    return best[1] if best[0] >= 6 else ""


def _writeflow_research_artifact_records(workspace: Path, run: dict, flags: dict) -> list[dict]:
    slug = _writeflow_matching_article_slug(workspace, run)
    if not slug:
        return []
    rows: list[dict] = []
    candidates = [
        ("01_theme", "01_theme.md", "direction", flags.get("direction_done")),
        ("02_cases", "02_cases.md", "research", flags.get("research_done")),
        ("03_outline", "03_outline.md", "outline", flags.get("outline_done")),
    ]
    if flags.get("draft_done"):
        candidates.extend(
            [
                ("draft_v1", "draft_v1.md", "draft", True),
                ("draft_v2", "draft_v2.md", "draft", True),
                ("review_report", "review_report.md", "review", True),
                ("export", "export.md", "publish", True),
            ]
        )
    for key, filename, task_id, enabled in candidates:
        if not enabled:
            continue
        artifact = _writeflow_artifact_record(
            {"id": key, "path": f"articles/{slug}/{filename}", "task_id": task_id, "origin": "historical_match"},
            workspace=workspace,
            require_existing=True,
        )
        if artifact:
            _writeflow_merge_artifact(rows, artifact)
    return rows


def _writeflow_project_dir_artifact_paths(workspace: Path, project_slug: str) -> list[str]:
    if not project_slug:
        return []
    candidate_dirs: list[tuple[Path, Path]] = []
    try:
        project_dir = safe_resolve(Path(workspace), f"articles/{project_slug}")
        candidate_dirs.append((project_dir, project_dir))
    except Exception:
        pass
    for root in _writeflow_external_article_roots(workspace):
        try:
            resolved_root = root.resolve()
            project_dir = (resolved_root / project_slug).resolve()
            if project_dir.is_relative_to(resolved_root):
                candidate_dirs.append((project_dir, project_dir))
        except Exception:
            continue
    rows = []
    seen = set()
    for project_dir, base_dir in candidate_dirs:
        if not project_dir.is_dir():
            continue
        for child in sorted(project_dir.rglob("*"), key=lambda p: str(p.relative_to(base_dir))):
            if not child.is_file():
                continue
            try:
                child_rel = child.relative_to(base_dir).as_posix()
            except Exception:
                continue
            rel = _writeflow_clean_artifact_path(f"articles/{project_slug}/{child_rel}")
            if not rel or rel in seen:
                continue
            if _writeflow_artifact_kind(rel, child.stem) == "file":
                continue
            seen.add(rel)
            rows.append(rel)
    return rows
...
def _writeflow_style_artifact_paths(workspace: Path) -> list[str]:
    try:
        style_dir = safe_resolve(Path(workspace), "articles/_styles")
    except Exception:
        return []
    if not style_dir.is_dir():
        return []
    rows = []
    for child in sorted(style_dir.rglob("*"), key=lambda p: str(p.relative_to(style_dir))):
        if not child.is_file():
            continue
        try:
            rel = _writeflow_clean_artifact_path(f"articles/_styles/{child.relative_to(style_dir).as_posix()}")
        except Exception:
            continue
        if rel and _writeflow_artifact_kind(rel, child.stem) != "file":
            rows.append(rel)
    return rows


def _writeflow_root_artifact_candidate(filename: str) -> tuple[str, str, str] | None:
    ...
    if stem_lower in {"03_outline", "outline"}:
        return "03_outline", "文章提纲", "outline"
    if stem_lower in {"01_theme", "theme", "direction"}:
        return "01_theme", "选题方向", "direction"
    if stem_lower in {"style_model", "style_report"}:
        return stem_lower, "风格模型" if stem_lower == "style_model" else "风格报告", "style_model"
    if stem_lower in {"extracted_article", "source_article"}:
    ...
    if "大纲" in stem or "提纲" in stem:
        return "03_outline", "文章提纲", "outline"
    if "框架" in stem or "方向" in stem:
        return "01_theme", "选题方向", "direction"
    return None


def _writeflow_recover_workspace_root_artifacts(workspace: Path, run: dict) -> list[dict]:
    run_ts = _writeflow_parse_time(str(run.get("created_at") or "")) or 0
    artifact_root = _writeflow_artifact_root_for_run(run)
    if not artifact_root:
        return []
    try:
        root = Path(workspace)
        target_root = safe_resolve(root, artifact_root)
    except Exception:
        return []
    rows: list[dict] = []
    seen_targets: set[str] = set()
    try:
        children = sorted(root.iterdir(), key=lambda item: item.stat().st_mtime if item.is_file() else 0, reverse=True)
    except Exception:
        return rows
    for child in children:
        if not child.is_file():
        ...
        try:
            ...
            continue
        if run_ts and mtime + 1 < run_ts:
            continue
        candidate = _writeflow_root_artifact_candidate(child.name)
        if not candidate:
            continue
        artifact_id, label, task_id = candidate
        suffix = child.suffix or ".md"
        target_name = f"{artifact_id}{suffix.lower()}"
        try:
            ...
            continue
        rel = _writeflow_clean_artifact_path(f"{artifact_root}/{target_name}")
        if not rel or rel in seen_targets:
            continue
        target.parent.mkdir(parents=True, exist_ok=True)
        if not target.is_file() or child.stat().st_mtime > target.stat().st_mtime + 0.5:
        ...
        artifact = _writeflow_artifact_record(
            {
                ...
                "path": rel,
                "task_id": task_id,
                "origin": "external_recovery",
                "current_run_owned": True,
                "source_path": child.name,
                "note": f"从工作区根目录回收：{child.name}",
            },
...
def _writeflow_project_artifacts(project_slug: str, meta: dict, workspace: Path | None = None) -> list[dict]:
    artifacts = meta.get("artifacts") if isinstance(meta.get("artifacts"), dict) else {}
    rows: list[dict] = []
    for key, rel in artifacts.items():
        if not rel:
            continue
        _writeflow_merge_artifact(
            rows,
            _writeflow_artifact_record(
                {"id": str(key), "path": str(rel), "origin": "current_run", "current_run_owned": True},
                project_slug=project_slug,
                workspace=workspace,
            ),
        )
    if not any(item.get("task_id") == "illustrations" for item in rows) and not _writeflow_image_generation_ready():
        rows.append(
            {
                "id": "illustration_prompts",
                "label": "配图提示词",
                "kind": "prompt",
                "path": f"articles/{project_slug}/illustration_prompts.md",
                "task_id": "illustrations",
                "status": "planned",
                "placeholder": True,
                "exists": False,
                "origin": "placeholder",
                "current_run_owned": True,
                "download_name": "配图提示词.md",
                "note": "图片生成后端未配置或未验证时，先输出可复用配图 prompt，不标记图片完成。",
            }
        )
    return rows


def _writeflow_project_file_changes(project_slug: str, artifacts: list[dict]) -> list[dict]:
    rows = []
    for item in artifacts:
        path = str(item.get("path") or "")
        if not path or item.get("placeholder"):
            continue
        origin = str(item.get("origin") or "")
        change_type = str(item.get("change_type") or "").strip()
        if not change_type:
            if origin in {"historical_match", "session_text", "project_scan"} and not item.get("current_run_owned"):
                change_type = "referenced"
            elif origin == "external_recovery":
...
def _writeflow_default_tasks(project_slug: str, meta: dict, workspace: Path | None = None) -> list[dict]:
    artifacts = meta.get("artifacts") if isinstance(meta.get("artifacts"), dict) else {}
    draft_done = bool(artifacts.get("draft_v1") or artifacts.get("draft_final"))
    image_done = any(
        _writeflow_artifact_kind(str(rel), str(key)) == "image" or str(key).startswith("image")
        for key, rel in artifacts.items()
    )
    artifact_rows = _writeflow_project_artifacts(project_slug, meta, workspace)
    run_status = str(meta.get("status") or "").strip().lower()
    draft_status = "done" if draft_done else ("running" if run_status in {"running", "in_progress", "working", "drafting"} else "pending")
    image_status = "done" if image_done else "pending"
    return [
        {
            "id": "draft",
            "title": "起草办公材料初稿",
            ...
            "worker_name": "文案创作专家",
            "status": draft_status,
            "status_label": _writeflow_status_label(draft_status),
            "description": "标题方案、正文、配图建议和发布建议。",
            "artifacts": [item["path"] for item in artifact_rows if item.get("task_id") == "draft" and not item.get("placeholder")],
        },
        {
            "id": "illustrations",
            "title": "生成封面和文中配图",
            ...
            "worker_name": "配图专家",
            "status": image_status,
            "status_label": _writeflow_status_label(image_status),
            "description": "封面图、文中配图；图片后端不可用时产出可复用配图 prompt。",
            "artifacts": [item["path"] for item in artifact_rows if item.get("task_id") == "illustrations" and not item.get("placeholder")],
        },
    ]


def _writeflow_members_for_run(team_id: str, meta: dict, workspace: Path | None = None) -> list[dict]:
    team = _WRITEFLOW_TEAMS.get(team_id) or _WRITEFLOW_TEAMS["content-creator-team"]
    artifacts = meta.get("artifacts") if isinstance(meta.get("artifacts"), dict) else {}
    draft_done = bool(artifacts.get("draft_v1") or artifacts.get("draft_final"))
    image_done = any(_writeflow_artifact_kind(str(rel), str(key)) == "image" for key, rel in artifacts.items())
    prompt_done = any(_writeflow_artifact_kind(str(rel), str(key)) == "prompt" for key, rel in artifacts.items())
    rows = []
    for member in team.get("members") or []:
        status = "待命"
        if member["id"] == "workflow-producer":
            status = _writeflow_status_label(meta.get("status") or "running")
        elif member["id"] == "writing-executor":
            status = "已完成" if draft_done else "待命"
        elif member["id"] == "article-illustrator":
            status = "已完成" if image_done else ("待出图" if prompt_done else "待命")
        rows.append({**member, "status": status})
    return rows


def _writeflow_normalize_run(run: dict) -> dict:
    now = _writeflow_now()
    run_id = _writeflow_safe_run_id(run.get("run_id") or f"wr-{uuid.uuid4().hex[:10]}")
    team_id = _writeflow_normalize_team_id(run.get("team_id") or run.get("team"), "start")
    project_slug = _writeflow_slug(run.get("project_slug") or run.get("project") or run.get("title") or "")
    tasks = run.get("tasks") if isinstance(run.get("tasks"), list) else []
    artifacts = []
    for item in (run.get("artifacts") if isinstance(run.get("artifacts"), list) else []):
        _writeflow_merge_artifact(artifacts, _writeflow_artifact_record(item))
    reference_artifacts = []
    for item in (run.get("reference_artifacts") if isinstance(run.get("reference_artifacts"), list) else []):
        _writeflow_merge_artifact(reference_artifacts, _writeflow_artifact_record(item))
    file_changes = []
    for item in (run.get("file_changes") if isinstance(run.get("file_changes"), list) else []):
        if isinstance(item, dict):
            rel = _writeflow_clean_artifact_path(str(item.get("path") or item.get("file") or ""))
            if rel:
                file_changes.append({**item, "path": rel})
    events = run.get("events") if isinstance(run.get("events"), list) else []
    status = str(run.get("status") or "running").strip().lower()
    if status not in _WRITEFLOW_RUN_STATUSES:
        status = "running"
    done_count = sum(1 for task in tasks if str(task.get("status") or "").lower() in {"done", "complete", "completed"})
    ...
    run.update(
        {
            ...
            "session_id": str(run.get("session_id") or ""),
            "project_slug": project_slug,
            "artifact_root": _writeflow_clean_artifact_path(str(run.get("artifact_root") or f"articles/{project_slug}")),
            "title": str(run.get("title") or run.get("project_slug") or "写作项目"),
            "status": status,
            "status_label": _writeflow_status_label(status),
            "phase": str(run.get("phase") or "确定方向"),
            "tasks": tasks,
            "members": run.get("members") if isinstance(run.get("members"), list) else [],
            "artifacts": artifacts,
            "reference_artifacts": reference_artifacts,
            "file_changes": file_changes,
            "events": events,
            "created_at": str(run.get("created_at") or now),
            "updated_at": str(run.get("updated_at") or now),
            "progress": {"done": done_count, "total": total_count},
            "next_actions": run.get("next_actions") if isinstance(run.get("next_actions"), list) else ["继续", "调整要求", "导出"],
...
def _writeflow_run_from_project(
    workspace: Path,
    project_slug: str,
    meta: dict,
    *,
    ...
    now = _writeflow_now()
    team_id = _writeflow_normalize_team_id(team_id or meta.get("team_id") or meta.get("team"), "start")
    title = str(meta.get("name") or project_slug)
    artifacts = _writeflow_project_artifacts(project_slug, meta, workspace)
    tasks = _writeflow_default_tasks(project_slug, meta, workspace)
    status = str(meta.get("status") or "running").strip().lower()
    if status == "waiting":
        status = "waiting_user"
    if status not in _WRITEFLOW_RUN_STATUSES:
        status = "running"
    machine = _writeflow_state_machine_for_team(team_id)
    task_titles = "、".join(str(task.get("title") or task.get("id") or "") for task in machine.get("tasks") or [] if task.get("title"))
    run = {
        "run_id": run_id or f"wr-{int(time.time())}-{uuid.uuid4().hex[:8]}",
        "team_id": team_id,
        "session_id": session_id or "",
        "project_slug": project_slug,
        "artifact_root": f"articles/{project_slug}",
        "title": title,
        "status": status,
        "phase": _writeflow_display_phase(meta),
        "tasks": tasks,
        "members": _writeflow_members_for_run(team_id, meta, workspace),
        "artifacts": artifacts,
        "file_changes": _writeflow_project_file_changes(project_slug, artifacts),
        "events": [
            {"type": "team_created", "label": f"创建团队 {_WRITEFLOW_TEAMS[team_id]['title']}", "created_at": now},
            {"type": "tasks_created", "label": f"创建任务：{task_titles}", "created_at": now},
        ],
        "created_at": now,
        "updated_at": now,
    ...
    if not _writeflow_image_generation_ready():
        run["events"].append(
            {
                "type": "image_provider_missing",
                "label": "图片生成后端未配置或未验证；配图任务将先产出 prompt 计划。",
                "created_at": now,
            }
        )
    return _writeflow_apply_state_machine(workspace, run, artifacts)


def _writeflow_update_run_from_project(run: dict, workspace: Path, meta: dict) -> dict:
    project_slug = _writeflow_slug(run.get("project_slug") or meta.get("name") or "")
    refreshed = _writeflow_run_from_project(
        workspace,
        project_slug,
        meta,
        session_id=run.get("session_id") or None,
        team_id=run.get("team_id") or None,
        run_id=run.get("run_id") or None,
    )
    preserved_events = run.get("events") if isinstance(run.get("events"), list) else []
    refreshed["created_at"] = run.get("created_at") or refreshed["created_at"]
    refreshed["events"] = preserved_events or refreshed["events"]
    return _writeflow_normalize_run(refreshed)


def _writeflow_artifact_stem(item: dict) -> str:
    raw_id = str((item or {}).get("id") or "").strip()
    if raw_id:
        return raw_id
    return Path(str((item or {}).get("path") or "")).stem


def _writeflow_state_machine_for_team(team_id: str) -> dict:
    normalized = _writeflow_normalize_team_id(team_id, "start")
    return _WRITEFLOW_STATE_MACHINES.get(normalized) or _WRITEFLOW_STATE_MACHINES["content-creator-team"]


def _writeflow_state_event_text(session_messages: list[dict]) -> tuple[str, str]:
    texts = [
        _writeflow_sidebar_message_text(msg)
...
def _writeflow_event_rule_matches(rule: dict, text: str, compact: str) -> bool:
    for marker in rule.get("markers") or []:
        marker_s = re.sub(r"\s+", "", str(marker or ""))
        if marker_s and marker_s in compact:
            return True
    for pattern in rule.get("regex") or []:
        try:
            if re.search(str(pattern), text, flags=re.S):
                return True
        except re.error:
            continue
    return False
...
def _writeflow_extract_state_events(run: dict, session_messages: list[dict], artifacts: list[dict]) -> dict:
    machine = _writeflow_state_machine_for_team(str(run.get("team_id") or ""))
    text, compact = _writeflow_state_event_text(session_messages)
    event_ids: set[str] = set()
    if str(run.get("status") or "").lower() not in _WRITEFLOW_RUN_TERMINAL_STATUSES:
        event_ids.add("run_started")
    for event in run.get("events") or []:
        ...
        if event_type:
            ...
            event_ids.add(f"event:{event_type}")
        label = str(event.get("label") or "")
        if label and event_type not in {"team_created", "tasks_created", "image_provider_missing"}:
            text = f"{text}\n{label}"
            compact_label = re.sub(r"\s+", "", label)
            compact = f"{compact}{compact_label}"
    rules = dict(_WRITEFLOW_COMMON_EVENT_RULES)
    rules.update(machine.get("event_rules") or {})
    for event_id, rule in rules.items():
        if isinstance(rule, dict) and _writeflow_event_rule_matches(rule, text, compact):
            event_ids.add(str(event_id))
    ready_artifacts = [item for item in artifacts if not item.get("placeholder") and item.get("exists", True)]
    for item in ready_artifacts:
        stem = _writeflow_artifact_stem(item)
        kind = str(item.get("kind") or "")
        task_id = str(item.get("task_id") or "")
        if stem:
            event_ids.add(f"artifact:{stem}")
        if kind:
            event_ids.add(f"artifact_kind:{kind}")
        if task_id:
            event_ids.add(f"artifact_task:{task_id}")
    return {"ids": event_ids, "text": text, "compact": compact, "ready_artifacts": ready_artifacts}


def _writeflow_artifact_matches_task(item: dict, task_def: dict, *, spec_prefix: str = "", include_default: bool = True) -> bool:
    stem = _writeflow_artifact_stem(item)
    ...
    def values(name: str) -> list[str]:
        raw = task_def.get(f"{spec_prefix}_{name}") if spec_prefix else task_def.get(name)
        if raw is None and include_default and spec_prefix:
            raw = task_def.get(name)
        if isinstance(raw, (list, tuple, set)):
            return [str(item) for item in raw if str(item)]
        return [str(raw)] if raw else []

    if stem and stem in set(values("artifact_ids")):
        return True
    artifact_task_ids = values("artifact_task_ids")
    if not artifact_task_ids and not spec_prefix:
        artifact_task_ids = [str(task_def.get("id") or "")]
    if task_id and task_id in set(artifact_task_ids):
        return True
    if kind and kind in {value.lower() for value in values("artifact_kinds")}:
        return True
    for fragment in values("artifact_path_contains"):
        if str(fragment).lower() in path_lower:
            return True
    return False


def _writeflow_task_artifacts(
    artifacts: list[dict],
    task_def: dict,
    *,
    spec_prefix: str = "",
    ...
    rows: list[dict] = []
    for item in artifacts:
        if item.get("placeholder") and not placeholders:
            continue
        if _writeflow_artifact_matches_task(item, task_def, spec_prefix=spec_prefix, include_default=include_default):
            row = {**item, "task_id": str(task_def.get("id") or item.get("task_id") or "")}
            _writeflow_merge_artifact(rows, row)
    return rows


def _writeflow_task_status(
    run: dict,
    task_def: dict,
    events: dict,
    artifacts: list[dict],
    *,
    ...
    event_ids = events.get("ids") if isinstance(events.get("ids"), set) else set()
    done_events = {str(item) for item in task_def.get("done_events") or []}
    start_events = {str(item) for item in task_def.get("start_events") or []}
    done_by_event = bool(done_events & event_ids)
    started_by_event = bool(start_events & event_ids)
    done_artifacts = _writeflow_task_artifacts(
        artifacts,
        task_def,
        spec_prefix="done",
        placeholders=False,
    ...
    if requires_artifact and done_requires_event:
        ...
        done = done_by_artifact
    else:
        done = bool(done_by_event or done_by_artifact)
    waiting_for_artifact = bool(not done and requires_artifact and done_by_event and not done_by_artifact)
    waiting = bool(not done and (waiting_artifacts or waiting_for_artifact))
    run_active = str(run.get("status") or "").lower() not in _WRITEFLOW_RUN_TERMINAL_STATUSES
    auto_start = bool(task_def.get("auto_start") and index == 0 and run_active)
    auto_after_previous = bool(task_def.get("auto_start_after_previous") and previous_done and run_active)
    running = bool(not done and not waiting and (started_by_event or auto_start or auto_after_previous))
    if done:
        status = "done"
        status_label = str(task_def.get("done_label") or _writeflow_status_label(status))
    elif waiting:
        status = "waiting_user"
        status_label = str(task_def.get("waiting_label") or ("等待本轮产物" if waiting_for_artifact else "等待确认"))
    elif running:
        status = "running"
        status_label = _writeflow_status_label(status)
    else:
        status = "pending"
        status_label = str(task_def.get("pending_label") or _writeflow_status_label(status))
    task_artifacts = []
    ...
    team = _WRITEFLOW_TEAMS.get(str(run.get("team_id") or "")) or {}
    worker = next((member for member in team.get("members") or [] if member.get("id") == worker_id), {})
    task = {
        "id": str(task_def.get("id") or ""),
        "title": str(task_def.get("title") or task_def.get("id") or "任务"),
        "worker_id": worker_id,
        "worker_name": str(worker.get("name") or task_def.get("worker_name") or worker_id),
        "status": status,
        "status_label": status_label,
        "description": str(task_def.get("description") or ""),
        "artifacts": task_artifacts,
    }
    phase = str(task_def.get("phase_on_done") if done and task_def.get("phase_on_done") else task_def.get("phase") or "确定方向")
    return task, done, phase


def _writeflow_member_rows_for_tasks(team_id: str, tasks: list[dict]) -> list[dict]:
    team = _WRITEFLOW_TEAMS.get(team_id) or _WRITEFLOW_TEAMS["content-creator-team"]
    active_indices = [
        idx
        for idx, task in enumerate(tasks)
        if str(task.get("status") or "") in {"running", "waiting_user", "blocked", "error"}
    ]
    rows = []
    for member in team.get("members") or []:
        member_id = str(member.get("id") or "")
        member_task_indices = [idx for idx, task in enumerate(tasks) if str(task.get("worker_id") or "") == member_id]
        member_statuses = [str(tasks[idx].get("status") or "") for idx in member_task_indices]
        waiting_indices = [idx for idx in member_task_indices if str(tasks[idx].get("status") or "") == "waiting_user"]
        ready_pending_idx = next(
            (
                idx
                for idx in member_task_indices
                if str(tasks[idx].get("status") or "") == "pending"
                and idx > 0
                and all(str(prev.get("status") or "") == "done" for prev in tasks[:idx])
                and not any(active_idx < idx for active_idx in active_indices)
            ),
            -1,
        )
        if any(status == "running" for status in member_statuses):
            status = "执行中"
        elif any(status == "blocked" for status in member_statuses):
            status = "受阻"
        elif any(status == "error" for status in member_statuses):
            status = "需要处理"
        elif any(status == "waiting_user" for status in member_statuses):
            waiting_for_artifacts = all("等待本轮产物" in str(tasks[idx].get("status_label") or "") for idx in waiting_indices)
            if waiting_for_artifacts:
                first_waiting = min(waiting_indices) if waiting_indices else -1
                later_active_elsewhere = any(
                    active_idx > first_waiting and str(tasks[active_idx].get("worker_id") or "") != member_id
                    for active_idx in active_indices
                )
                status = "已交接" if later_active_elsewhere else "待落盘"
            else:
                status = "等待确认"
        elif any(status == "done" for status in member_statuses):
            last_done = max((idx for idx in member_task_indices if str(tasks[idx].get("status") or "") == "done"), default=-1)
            next_waiting_idx = next(
                (
                    idx
                    for idx in member_task_indices
                    if idx > last_done
                    and str(tasks[idx].get("status") or "") == "pending"
                    and all(str(prev.get("status") or "") == "done" for prev in tasks[:idx])
                ),
                -1,
            )
            later_active_elsewhere = any(
                active_idx > last_done and str(tasks[active_idx].get("worker_id") or "") != member_id
                for active_idx in active_indices
            )
            if next_waiting_idx >= 0 and not later_active_elsewhere:
                status = "等待确认"
            else:
                status = "已交接" if later_active_elsewhere else "已完成"
        elif ready_pending_idx >= 0:
            status = "等待确认"
        else:
            status = "待命"
        rows.append({**member, "status": status})
...
def _writeflow_apply_state_machine(workspace: Path, run: dict, artifacts: list[dict] | None = None) -> dict:
    ...
    team_id = str(run.get("team_id") or "content-creator-team")
    machine = _writeflow_state_machine_for_team(team_id)
    all_artifacts = list(artifacts if artifacts is not None else run.get("artifacts") or [])
    current_artifacts, reference_artifacts = _writeflow_split_current_and_reference_artifacts(workspace, run, all_artifacts)
    session_messages = _writeflow_session_messages(str(run.get("session_id") or ""))
    events = _writeflow_extract_state_events(run, session_messages, current_artifacts)
    tasks: list[dict] = []
    matched_artifacts: list[dict] = []
    previous_done = True
    task_defs = list(machine.get("tasks") or [])
    for index, task_def in enumerate(task_defs):
        task, done, task_phase = _writeflow_task_status(
            run,
            task_def,
            events,
            current_artifacts,
        ...
        for item in _writeflow_task_artifacts(current_artifacts, task_def, placeholders=True):
            if item.get("placeholder"):
                continue
            if task_status == "pending":
                continue
            _writeflow_merge_artifact(matched_artifacts, {**item, "task_id": task["id"], "visible_scope": "current"})
    last_done_index = max((idx for idx, task in enumerate(tasks) if str(task.get("status") or "") == "done"), default=-1)
    for idx in range(last_done_index):
        if str(tasks[idx].get("status") or "") == "done":
            continue
        task_def = task_defs[idx]
        if task_def.get("requires_artifact"):
            continue
        tasks[idx]["status"] = "done"
        tasks[idx]["status_label"] = str(task_def.get("done_label") or "已完成")
        tasks[idx]["artifacts"] = []
    phase = "确定方向"
    last_done_phase = ""
    running_phase = ""
    waiting_user_phase = ""
    waiting_artifact_phase = ""
    waiting_artifact_index = -1
    for idx, task in enumerate(tasks):
        task_def = task_defs[idx] if idx < len(task_defs) else {}
        task_phase = str(
            task_def.get("phase_on_done")
            if str(task.get("status") or "") == "done" and task_def.get("phase_on_done")
            else task_def.get("phase") or "确定方向"
        )
        task_status = str(task.get("status") or "")
        if task_status in {"running", "blocked", "error"} and not running_phase:
            running_phase = task_phase
        if task_status == "waiting_user":
            if "等待本轮产物" in str(task.get("status_label") or ""):
                waiting_artifact_phase = task_phase
                waiting_artifact_index = idx
            elif not waiting_user_phase:
                waiting_user_phase = task_phase
        if str(task.get("status") or "") == "done":
            last_done_phase = task_phase
    if running_phase:
        phase = running_phase
    elif waiting_user_phase:
        phase = waiting_user_phase
    elif last_done_index > waiting_artifact_index:
        phase = last_done_phase
    elif waiting_artifact_phase:
        phase = waiting_artifact_phase
    elif last_done_phase:
        phase = last_done_phase
    statuses = {str(task.get("status") or "") for task in tasks}
    current_status = str(run.get("status") or "running").lower()
    if current_status in _WRITEFLOW_RUN_TERMINAL_STATUSES:
        ...
        run_status = "running"
    elif "waiting_user" in statuses or "done" in statuses:
        run_status = "waiting_user"
    else:
        run_status = "running"
    next_actions_map = machine.get("next_actions") if isinstance(machine.get("next_actions"), dict) else {}
    next_key = "done" if all(str(task.get("status") or "") == "done" for task in tasks) and tasks else run_status
    run["phase"] = phase
    run["status"] = run_status
    run["tasks"] = tasks
    run["members"] = _writeflow_member_rows_for_tasks(team_id, tasks)
    run["artifacts"] = matched_artifacts
...
def _writeflow_enrich_run_from_workspace(workspace: Path, run: dict) -> dict:
    ...
    project_slug = _writeflow_slug(run.get("project_slug") or run.get("title") or "")
    artifacts: list[dict] = []
    scan_project_slugs = {project_slug} if project_slug else set()
    scanned_project_slugs: set[str] = set()

    def scan_project_dir_artifacts() -> None:
        for scan_slug in sorted(scan_project_slugs - scanned_project_slugs):
            scanned_project_slugs.add(scan_slug)
            for rel in _writeflow_project_dir_artifact_paths(workspace, scan_slug):
                _writeflow_merge_artifact(
                    artifacts,
                    _writeflow_artifact_record(
                        {"path": rel, "origin": "project_scan"},
                        project_slug=project_slug,
                        workspace=workspace,
                        require_existing=True,
                    ),
    ...
    for item in run.get("artifacts") or []:
        ...
        if artifact:
            slug_from_path = _writeflow_project_slug_from_artifact_path(str(artifact.get("path") or ""))
            if slug_from_path:
                scan_project_slugs.add(slug_from_path)
        _writeflow_merge_artifact(artifacts, artifact)
    scan_project_dir_artifacts()
    for rel in _writeflow_session_artifact_paths(run, project_slug):
        slug_from_path = _writeflow_project_slug_from_artifact_path(rel)
        if slug_from_path:
            scan_project_slugs.add(slug_from_path)
        _writeflow_merge_artifact(
            artifacts,
            _writeflow_artifact_record(
                {"path": rel, "origin": "session_text"},
                project_slug=project_slug,
                workspace=workspace,
                require_existing=True,
            ),
    ...
    scan_project_dir_artifacts()
    for artifact in _writeflow_recover_workspace_root_artifacts(workspace, run):
        _writeflow_merge_artifact(artifacts, artifact)
    if run.get("team_id") == "style-modeler":
        for rel in _writeflow_style_artifact_paths(workspace):
            _writeflow_merge_artifact(
                artifacts,
                _writeflow_artifact_record(
                    {"path": rel, "origin": "project_scan"},
    ...
    if run.get("team_id") == "deep-research-team":
        state_events = _writeflow_extract_state_events(
            run,
            _writeflow_session_messages(str(run.get("session_id") or "")),
            artifacts,
        )
        event_ids = state_events.get("ids") if isinstance(state_events.get("ids"), set) else set()
        research_flags = {
            "direction_done": "direction_done" in event_ids,
            "research_done": "research_done" in event_ids,
            "outline_done": "outline_done" in event_ids,
            "draft_done": "draft_done" in event_ids,
        }
        for artifact in _writeflow_research_artifact_records(workspace, run, research_flags):
            _writeflow_merge_artifact(artifacts, artifact)
    if run.get("team_id") == "content-creator-team" and not any(item.get("task_id") == "illustrations" for item in artifacts) and not _writeflow_image_generation_ready():
        _writeflow_merge_artifact(
            artifacts,
            _writeflow_artifact_record(
                {
...
def _writeflow_list_runs(workspace: Path, project: str | None = None) -> list[dict]:
    runs_dir = _writeflow_runs_dir(workspace)
    project_slug = _writeflow_slug(project) if project else None
    rows = []
    if runs_dir.exists():
...
def _writeflow_sidebar_title_needs_override(title: str) -> bool:
    normalized = re.sub(r"\s+", " ", str(title or "")).strip()
    if not normalized or normalized in {"Untitled", "New Chat"}:
        return True
    if normalized.startswith("请【") and "接手这个写作任务" in normalized:
        return True
    if normalized.startswith("请使用 Hermes skill `workflow-producer`"):
        return True
    if normalized.startswith("Use the Hermes skill `workflow-producer`"):
        return True
    return False


def _writeflow_sidebar_topic(title: str, project_slug: str = "") -> str:
    text = re.sub(r"\s+", " ", str(title or project_slug or "写作项目")).strip()
    text = text.strip(" \t\r\n。.!！?？、，,：:；;「」『』“”\"'")
    replacements = (
        r"^帮我写一篇",
        r"^请帮我写一篇",
    ...
    text = re.sub(r"\bagent\b", "Agent", text, flags=re.IGNORECASE)
    return text[:36] or "写作项目"


def _writeflow_sidebar_display_title(run: dict) -> str:
    team_id = str(run.get("team_id") or "")
    team = _WRITEFLOW_TEAMS.get(team_id) or _WRITEFLOW_TEAMS["content-creator-team"]
    team_label = str(team.get("category") or team.get("title") or "写作团队").strip() or "写作团队"
    topic = _writeflow_sidebar_topic(str(run.get("title") or ""), str(run.get("project_slug") or ""))
    return f"{team_label}｜{topic}"


def _writeflow_sidebar_team_id_from_text(text: str) -> str:
    raw = str(text or "")
    match = re.search(r"team_id:\s*`?([A-Za-z0-9_.-]+)`?", raw)
    if match and match.group(1) in _WRITEFLOW_TEAMS:
        return match.group(1)
    for team_id, team in _WRITEFLOW_TEAMS.items():
        if str(team.get("title") or "") and str(team.get("title")) in raw:
            return team_id
    return "content-creator-team"


def _writeflow_sidebar_topic_from_text(text: str) -> str:
    raw = str(text or "")
    for pattern in (
        r"本次需求：\s*(.+?)(?:\n\n对用户呈现|\n\n执行团队备忘|\n\n产物要求|\Z)",
        r"稿件名称：\s*(.+?)(?:\n|$)",
        ...
        if not match:
            continue
        candidate = re.sub(r"\s+", " ", match.group(1)).strip()
        quoted = re.search(r"主题是[「\"](.+?)[」\"]", candidate)
        if quoted:
            candidate = quoted.group(1)
        return _writeflow_sidebar_topic(candidate)
    return "写作项目"


def _writeflow_sidebar_message_text(message: dict) -> str:
...
def _writeflow_sidebar_display_title_from_session(row: dict) -> str:
    ...
    for message in getattr(session, "messages", []) or []:
        text = _writeflow_sidebar_message_text(message)
        if "接手这个写作任务" not in text and "workflow-producer" not in text:
            continue
        team_id = _writeflow_sidebar_team_id_from_text(text)
        team = _WRITEFLOW_TEAMS.get(team_id) or _WRITEFLOW_TEAMS["content-creator-team"]
        team_label = str(team.get("category") or team.get("title") or "写作团队").strip() or "写作团队"
        return f"{team_label}｜{_writeflow_sidebar_topic_from_text(text)}"
    return ""


def _writeflow_enrich_sidebar_sessions(sessions: list[dict]) -> list[dict]:
    ...
    for workspace, _rows in by_workspace.items():
        try:
            resolved = resolve_trusted_workspace(workspace)
        except Exception:
            continue
        for run in _writeflow_list_runs(resolved):
            sid = str(run.get("session_id") or "").strip()
            if sid and sid not in runs_by_session:
                runs_by_session[sid] = run

    for row in sessions or []:
        if not isinstance(row, dict):
            continue
        run = runs_by_session.get(str(row.get("session_id") or ""))
        if not run:
            continue
        if not _writeflow_sidebar_title_needs_override(str(row.get("title") or "")):
            continue
        row["display_title"] = _writeflow_sidebar_display_title(run)
        row["writeflow_team_id"] = run.get("team_id") or ""
        row["writeflow_title"] = run.get("title") or ""
    fallback_count = 0
    for row in sessions or []:
        if not isinstance(row, dict) or row.get("display_title"):
            continue
        if not _writeflow_sidebar_title_needs_override(str(row.get("title") or "")):
            continue
        if fallback_count >= 50:
            break
        fallback_count += 1
        display_title = _writeflow_sidebar_display_title_from_session(row)
        if display_title:
            row["display_title"] = display_title
    return sessions


def _writeflow_find_run(
    workspace: Path,
    *,
    run_id: str | None = None,
    ...
    if run_id:
        ...
        if error:
            return None
        return run
    runs = _writeflow_list_runs(workspace, project_slug)
    if team_id:
        normalized_team_id = _writeflow_normalize_team_id(team_id, "start")
        runs = [run for run in runs if str(run.get("team_id") or "") == normalized_team_id]
...
def _writeflow_active_run_context(workspace: Path, session_id: str | None) -> dict | None:
    ...
    runs = [
        run
        for run in _writeflow_list_runs(workspace)
        if str(run.get("session_id") or "") == sid
        and str(run.get("status") or "").lower() not in _WRITEFLOW_RUN_TERMINAL_STATUSES
    ]
    if not runs:
        return None
    run = runs[0]
...
def _writeflow_artifact_target_for_file_write(workspace: Path, session_id: str | None, requested_path: str) -> dict | None:
    context = _writeflow_active_run_context(workspace, session_id)
    if not context:
        return None
    raw_rel = str(requested_path or "").strip().replace("\\", "/").lstrip("/")
    if not raw_rel:
        return None
    path = Path(raw_rel)
    if path.is_absolute() or ".." in path.parts:
        return None
    artifact_root = str(context.get("artifact_root") or "").rstrip("/")
    project_slug = str(context.get("project_slug") or "")
    explicit_rel = _writeflow_clean_artifact_path(raw_rel)
    candidate = _writeflow_root_artifact_candidate(path.name)

    if explicit_rel:
        if not _writeflow_path_is_under(explicit_rel, artifact_root):
            return None
        artifact_id, label, task_id = candidate or (Path(explicit_rel).stem, "", "")
        return {
            **context,
            "path": explicit_rel,
            "artifact": {
    ...
    if len(path.parts) != 1 or not candidate:
        return None
    artifact_id, label, task_id = candidate
    ...
    return {
        **context,
        ...
        "redirected_from": raw_rel,
        "project_slug": project_slug,
    }


def _writeflow_register_artifact(
    workspace: Path,
    run: dict,
    artifact: dict,
    *,
    ...
    raw = {
        **dict(artifact or {}),
        "origin": str((artifact or {}).get("origin") or "current_run"),
        "current_run_owned": True,
        "source_run_id": run_id,
        "change_type": change_type,
    }
    record = _writeflow_artifact_record(
        raw,
        project_slug=str((run or {}).get("project_slug") or ""),
        workspace=workspace,
        require_existing=True,
    )
    if not record:
    ...
    path = _writeflow_run_path(workspace, run_id)
    try:
        persisted = json.loads(path.read_text(encoding="utf-8")) if path.exists() else dict(run or {})
    except Exception:
        persisted = dict(run or {})
    if not isinstance(persisted, dict):
        persisted = dict(run or {})
    persisted.setdefault("run_id", run_id)
    persisted.setdefault("team_id", str((run or {}).get("team_id") or ""))
    persisted.setdefault("project_slug", str((run or {}).get("project_slug") or ""))
    persisted.setdefault("session_id", str((run or {}).get("session_id") or ""))
    persisted.setdefault("artifact_root", _writeflow_artifact_root_for_run(run))
    persisted.setdefault("title", str((run or {}).get("title") or persisted.get("project_slug") or "写作项目"))
    rows: list[dict] = []
    for item in (persisted.get("artifacts") if isinstance(persisted.get("artifacts"), list) else []):
        _writeflow_merge_artifact(rows, _writeflow_artifact_record(item, project_slug=str(persisted.get("project_slug") or "")))
    _writeflow_merge_artifact(rows, record)
    persisted["artifacts"] = rows
    persisted["updated_at"] = _writeflow_now()
    refreshed = _writeflow_apply_state_machine(workspace, persisted, rows)
    _writeflow_write_json(path, refreshed)
    return record


def _writeflow_tool_env_for_session(workspace: Path, session_id: str | None) -> dict[str, str]:
    env = {"HERMES_WORKSPACE": str(Path(workspace).resolve())}
    context = _writeflow_active_run_context(workspace, session_id)
    if not context:
        return env
    try:
        artifact_root_abs = safe_resolve(Path(workspace), str(context.get("artifact_root") or ""))
    except Exception:
        artifact_root_abs = Path(workspace)
    env.update(
        {
            "HERMES_WRITEFLOW_RUN_ID": str(context.get("run_id") or ""),
            "HERMES_WRITEFLOW_TEAM_ID": str(context.get("team_id") or ""),
            "HERMES_WRITEFLOW_PROJECT_SLUG": str(context.get("project_slug") or ""),
            "HERMES_WRITEFLOW_ARTIFACT_ROOT": str(artifact_root_abs),
        }
    )
    return env


def _writeflow_run_meta_from_session(session_id: str) -> dict | None:
    sid = str(session_id or "").strip()
    if not sid:
        return None
    try:
    ...
    for message in getattr(session, "messages", []) or []:
        text = _writeflow_sidebar_message_text(message)
        if "接手这个写作任务" not in text:
            continue
        team_id = _writeflow_sidebar_team_id_from_text(text)
        title_match = re.search(r"稿件名称：\s*(.+?)(?:\n|$)", text)
        title = re.sub(r"\s+", " ", title_match.group(1)).strip() if title_match else _writeflow_sidebar_topic_from_text(text)
        if not title:
            title = "写作项目"
        return {
            "name": title,
...
def _writeflow_ensure_session_run(workspace: Path, session_id: str, project: str | None = None) -> dict | None:
    sid = str(session_id or "").strip()
    if not sid:
        return None
    ...
    if not meta:
        return None
    project_slug = _writeflow_slug(project or meta.get("name") or "写作项目")
    run = _writeflow_run_from_project(
        workspace,
        project_slug,
        meta,
        session_id=sid,
    ...
    _writeflow_write_json(_writeflow_run_path(workspace, run["run_id"]), run)
    return run


def _writeflow_ensure_run(workspace: Path, body: dict, meta: dict | None = None) -> dict:
    session_id = str(body.get("session_id") or "").strip()
    raw_project = str(body.get("project") or body.get("project_name") or "").strip()
    project_slug = _writeflow_slug(raw_project)
    meta = dict(meta or {})
    action = _writeflow_normalize_action(body.get("action") or meta.get("action") or "start")
    team_id = _writeflow_normalize_team_id(
        body.get("team_id") or body.get("teamId") or body.get("team") or meta.get("team_id") or meta.get("team"),
        action,
    )
    requested_run_id = body.get("run_id") or body.get("runId")
    existing = _writeflow_find_run(workspace, run_id=requested_run_id, project_slug=project_slug, team_id=team_id)
    ...
    if body.get("mode") and not meta.get("mode"):
        meta["mode"] = str(body.get("mode")).strip().upper()[:1]
    if not meta.get("status"):
        meta["status"] = "running"
    meta["team_id"] = team_id
    run = _writeflow_run_from_project(workspace, project_slug, meta, session_id=session_id, team_id=team_id)
    _writeflow_write_json(_writeflow_run_path(workspace, run["run_id"]), run)
    return run


def _writeflow_artifacts_payload(workspace: Path, run_id: str) -> dict:
    run, error = _writeflow_read_run(workspace, run_id)
    if error:
        return {"ok": False, "error": error, "run_id": run_id, "artifacts": [], "file_changes": []}
    return {
        "ok": True,
        "workspace": str(workspace),
        "run_id": run["run_id"],
...
def _writeflow_public_status(workspace: Path, project: str | None = None) -> dict:
    state, exists, error = _writeflow_read_state(workspace)
    project_slug = _writeflow_slug(project) if project else None
    ...
    for slug, meta in sorted(projects.items()):
        ...
        if not isinstance(meta, dict):
            meta = {}
        artifacts = meta.get("artifacts") if isinstance(meta.get("artifacts"), dict) else {}
        team_id = _writeflow_normalize_team_id(meta.get("team_id") or meta.get("team"), "start")
        project_rows.append(
            {
                "slug": slug,
                ...
                "artifacts": artifacts,
                "display_phase": _writeflow_display_phase(meta),
                "display_status": _writeflow_display_status(meta.get("status") or ""),
                "display_artifacts": _writeflow_display_artifacts(artifacts),
                "display_team": _writeflow_team_summary(meta, team_id),
                "display_members": _writeflow_member_summaries(meta, team_id),
    ...
    return {
        "ok": error is None,
        "workspace": str(workspace),
        "state_path": str(_writeflow_state_path(workspace)),
        "state_exists": exists,
        "state_error": error,
        ...
        "artifact_contract": dict(_WRITEFLOW_ARTIFACTS),
    }


def _writeflow_compose_message(body: dict) -> dict:
    action = _writeflow_normalize_action(body.get("action") or "status")
    session_id = str(body.get("session_id") or "").strip() or None
    workspace = _writeflow_workspace(session_id)
    raw_project = str(body.get("project") or body.get("project_name") or "").strip()
    project_slug = _writeflow_slug(raw_project)
    mode = str(body.get("mode") or "").strip().upper()[:1]
    if mode not in _WRITEFLOW_MODES:
        mode = ""
    stage = str(body.get("stage") or "").strip()
    prompt = str(body.get("prompt") or "").strip()
    team_id = _writeflow_normalize_team_id(
        body.get("team_id") or body.get("teamId") or body.get("team"),
        action,
    )
    team = _WRITEFLOW_TEAMS.get(team_id) or _WRITEFLOW_TEAMS["content-creator-team"]
    ...
    if action == "style":
        ...
        skill = "workflow-producer"

    status = _writeflow_public_status(workspace, project_slug)
    project_status = next(
        (row for row in status.get("projects", []) if row.get("slug") == project_slug),
        None,
    )
    project_meta = {
        "name": raw_project or project_slug,
        "mode": mode,
        "stage": stage,
        "status": "running" if action == "start" else "waiting_user",
        "team_id": team_id,
    }
    if project_status:
        project_meta.update(project_status)
    run = _writeflow_ensure_run(workspace, body, project_meta) if action in {"start", "style", "extract"} else _writeflow_find_run(
        workspace,
        run_id=body.get("run_id") or body.get("runId"),
        project_slug=project_slug,
    )
    if run and not raw_project:
        project_slug = str(run.get("project_slug") or project_slug)
        raw_project = str(run.get("title") or project_slug)
    if run and not project_status:
        display_phase = str(run.get("phase") or "确定方向")
        display_status = str(run.get("status_label") or _writeflow_status_label(run.get("status") or "running"))
    else:
        display_phase = (project_status or {}).get("display_phase") or "确定方向"
        display_status = (project_status or {}).get("display_status") or "未开始"
    run_id = run.get("run_id") if run else ""
    prompt_line = prompt if prompt else "无补充需求。"
    action_label = _WRITEFLOW_ACTION_LABELS[action]
    public_team_lines = _writeflow_public_team_lines(team_id)
    members_line = "\n".join(_writeflow_public_member_lines(team_id, team))
    progress_line = _writeflow_public_progress_line(team_id)
    artifact_requirement_lines = _writeflow_public_artifact_requirement_lines(team_id)

    message = "\n".join(
        [
            f"请【{team['title']}】接手这个写作任务。",
            "",
            "你可以把这次协作理解成把一个选题交给一间小型内容工作室：",
            *public_team_lines,
            "",
            f"本轮要做：{action_label}",
            f"稿件名称：{raw_project or project_slug}",
            f"当前进度：{display_phase}",
            ...
            "",
            "对话要求：",
            "- 像真实编辑团队一样沟通，不要把用户带进内部工具、文件路径和技术术语里。",
            progress_line,
            "- 如果信息不足，先问最少数量的澄清问题，不要直接编造素材。",
            "",
            "系统处理原则（用户无需操作）：",
            "- 文件保存位置、进度同步、成果物登记和下载入口由当前系统根据会话自动处理。",
    ...
    return {
        ...
        "skill": skill,
        "team_id": team_id,
        "display_team": _writeflow_team_summary({"team_id": team_id}, team_id),
        "display_members": _writeflow_member_summaries({"status": "running"}, team_id),
        "template_id": template_id,
        "run_id": run_id,
        "run": run,
        "project": project_slug,
        "project_name": raw_project or project_slug,
        "mode": mode,
        "stage": stage,
        "message": message,
        "status": status,
...
def handle_get(handler, parsed) -> bool:
    ...
    if parsed.path == "/api/model-config":
        from api.model_config import get_model_config
    ...
    if parsed.path == "/api/sessions":
        ...
        try:
            ...
            diag.stage("writeflow_sidebar_titles")
            scoped = _writeflow_enrich_sidebar_sessions(scoped)
    ...
    if parsed.path == "/api/writeflow/status":
        qs = parse_qs(parsed.query)
    ...
    if parsed.path == "/api/writeflow/run":
        qs = parse_qs(parsed.query)
        session_id = qs.get("session_id", [""])[0].strip() or None
        run_id = qs.get("run_id", [""])[0].strip()
        if not run_id and not session_id:
    ...
    if parsed.path == "/api/writeflow/artifacts":
        ...
        try:
            workspace = _writeflow_workspace(session_id)
            payload = _writeflow_artifacts_payload(workspace, run_id)
        except Exception as exc:
...
def handle_post(handler, parsed) -> bool:
    ...
    if parsed.path == "/api/docx-engine-v2/drafts/package":
        return _handle_docx_engine_v2_package_draft(handler, body)
...
def _handle_session_export(handler, parsed):
    ...
    handler.send_header("Content-Type", "application/json; charset=utf-8")
    handler.send_header(
        "Content-Disposition", f'attachment; filename="taiji-session-{sid}.json"'
    )
...
def _handle_terminal_start(handler, body):
    try:
        ...
        term = start_terminal(
            ...
            extra_env=_writeflow_tool_env_for_session(workspace, sid),
        )
        return j(
            handler,
            {
...
def _handle_file_save(handler, body):
    ...
    try:
        ...
        routing = _writeflow_artifact_target_for_file_write(workspace, body["session_id"], body["path"])
        rel_path = str((routing or {}).get("path") or body["path"])
        target = safe_resolve(workspace, rel_path)
        existed_before = target.exists()
        if not existed_before and not routing:
            return bad(handler, "File not found", 404)
        if target.is_dir():
        ...
        registered = None
        if routing:
            registered = _writeflow_register_artifact(
                workspace,
                routing["run"],
                routing["artifact"],
                change_type="updated" if existed_before else "created",
            )
        payload = {"ok": True, "path": rel_path, "size": target.stat().st_size}
        if routing:
            payload["writeflow_artifact"] = {
                "registered": bool(registered),
                "run_id": str(routing.get("run_id") or ""),
                "path": rel_path,
                "label": str((registered or routing.get("artifact") or {}).get("label") or ""),
                "redirected_from": str(routing.get("redirected_from") or ""),
            }
        return j(
            handler, payload
        )
...
def _handle_file_create(handler, body):
    ...
    try:
        ...
        payload = {"ok": True, "path": str(target.relative_to(workspace))}
        if routing:
            payload["writeflow_artifact"] = {
                "registered": bool(registered),
                "run_id": str(routing.get("run_id") or ""),
