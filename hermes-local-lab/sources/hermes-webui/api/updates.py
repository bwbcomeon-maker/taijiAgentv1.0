...
def _dirty_worktree_stamp(path: Path, timeout=1) -> str:
    """Return a short stamp for tracked dirty files.

    The static shell is served with immutable `?v=WEBUI_VERSION` URLs. A plain
    `-dirty` suffix is not enough during local development because multiple
    restarts after different JS/CSS edits keep the same URL and browsers can
    legally reuse the old script. Use a cheap filename+stat digest so every
    ...
    for rel in sorted({line.strip() for line in out.splitlines() if line.strip()}):
        hasher.update(rel.encode('utf-8', 'surrogatepass'))
        try:
            p = (path / rel).resolve()
            p.relative_to(path.resolve())
            st = p.stat()
            hasher.update(f"{st.st_size}:{st.st_mtime_ns}".encode('ascii'))
...
def _dirty_suffix(path: Path, timeout=1) -> str:
    ...
    if not out or out.startswith('git exited with status '):
        stamp = _dirty_worktree_stamp(path, timeout=timeout)
        if stamp:
            return f"-dirty.{stamp}"
