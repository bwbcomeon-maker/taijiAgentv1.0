...
## Changing global model defaults

After first setup, use Settings -> Model Configuration to update the active
profile's global defaults. The panel writes the same active `HERMES_HOME`
`config.yaml` and `.env` files that Hermes CLI reads, so saved defaults apply to
new WebUI sessions and CLI runs started under the same `HERMES_HOME`.

The composer footer model dropdown remains a current-session selector. It does
not rewrite the global default.

The panel covers main chat models, auxiliary task models such as vision, and
image generation providers. API keys are accepted only for API-key providers and
are stored in `.env`; the read API returns configured/not-configured status, not
secret values. OAuth providers still use the CLI auth flow.
