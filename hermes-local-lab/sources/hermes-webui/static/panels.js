...
function _cronGatewayNoticeHtml(status) {
  ...
  const body = notConfigured
    ? '定时任务可以创建和手动运行，但自动定时执行需要本地任务服务可用。请重启太极智能体，或导出诊断信息交给管理员排查。'
    : (unknown
      ? '定时任务自动执行依赖本地任务服务。当前暂未确认服务状态，请刷新或重启太极智能体后再检查。'
      : '定时任务自动执行依赖本地任务服务。当前服务未运行，定时任务仍可管理和手动运行；请重启太极智能体，或导出诊断信息交给管理员排查。');
  return `
...
async function _populateCronDeliverOptions(selectedValue, isEdit) {
  ...
  try {
    ...
    sel.innerHTML = '<option value="local">本地（仅保存输出）</option>';
  }
  sel.disabled = false;
}
...
async function submitKanbanBoardModal(){
  ...
  if (mode === 'create') {
    ...
    if (!slug) { errEl.textContent = '缺少标识'; return; }
    if (submitBtn) submitBtn.disabled = true;
    try {
...
let _writeflowData = null;
let _writeflowSelectedTeamId = 'content-creator-team';
...
const WRITEFLOW_FALLBACK_TEAM = {
  ...
  category: '内容创作',
  statusLabel: '本地交付已就绪',
  description: '支持需求确认、分阶段协作与本地文档交付。',
  tags: [],
...
function _writeflowStatusUrl() {
  ...
  return '/api/writeflow/status' + (sid ? `?session_id=${encodeURIComponent(sid)}` : '');
}

function _writeflowNormalizeServerTeam(team) {
  const existing = WRITEFLOW_TEAMS.find(item => item.id === team.id) || (team.id === WRITEFLOW_FALLBACK_TEAM.id ? WRITEFLOW_FALLBACK_TEAM : {});
  return {
    ...existing,
    id: team.id || existing.id || 'content-creator-team',
    title: team.title || existing.title || '内容创作专家团',
    category: team.category || existing.category || '内容创作',
    statusLabel: team.status_label || team.statusLabel || existing.statusLabel || '',
    description: team.description || existing.description || '',
    tags: Array.isArray(team.tags) ? team.tags : (existing.tags || []),
    image: team.image || existing.image || '',
    defaultMode: team.default_mode || team.defaultMode || existing.defaultMode || 'A',
    defaultAction: team.default_action || team.defaultAction || existing.defaultAction || 'start',
    members: Array.isArray(team.members)
      ? team.members.map(member => ({ ...member, image: member.image || WRITEFLOW_MEMBER_IMAGES[member.id] || WRITEFLOW_MEMBER_IMAGES[member.skill] || '' }))
      : (existing.members || []),
    examples: Array.isArray(team.examples) ? team.examples : (existing.examples || []),
  };
}
...
function _writeflowAvatarHtml(src, label, cls) {
  ...
  if (!src) {
    return `<span class="${cls || 'writeflow-avatar'}"><span class="writeflow-avatar-fallback" style="display:inline-flex">${esc(fallback)}</span></span>`;
  }
  return `<span class="${cls || 'writeflow-avatar'}"><img src="${esc(src || '')}" alt="${esc(label || '')}" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='inline-flex'"><span class="writeflow-avatar-fallback" style="display:none">${esc(fallback)}</span></span>`;
}

function _writeflowProjectTitleFromPrompt(prompt, team) {
  const text = String(prompt || '').replace(/\s+/g, ' ').trim();
  if (!text) return team ? team.title : '写作项目';
  return text.replace(/[「」《》"'“”]/g, '').slice(0, 26);
}

function renderWriteflowTeams() {
  const grid = $('writeflowTeamGrid');
  if (!grid) return;
  const statusEl = $('writeflowCatalogStatus');
  if (!WRITEFLOW_TEAMS.length) {
    grid.innerHTML = '<div class="writeflow-empty">正在读取专家团模板...</div>';
    if (statusEl) statusEl.textContent = '';
    return;
  }
  const query = (($('writeflowTeamSearch') || {}).value || '').trim().toLowerCase();
  const teams = query
    ? WRITEFLOW_TEAMS.filter(team => [
      team.title,
      team.category,
      team.description,
      ...(team.tags || []),
      ...(team.members || []).map(member => `${member.name || ''} ${member.role || ''}`),
    ].join(' ').toLowerCase().includes(query))
    : WRITEFLOW_TEAMS;
  if (statusEl) statusEl.textContent = query ? `找到 ${teams.length} 个专家团` : '';
  if (!teams.length) {
    grid.innerHTML = '<div class="writeflow-empty">没有匹配的写作专家团。</div>';
    return;
  }
  grid.innerHTML = teams.map(team => {
    const selected = team.id === _writeflowSelectedTeamId;
    const memberAvatars = team.members.slice(0, 4).map(member => _writeflowAvatarHtml(member.image, member.name, 'writeflow-mini-avatar')).join('');
    const tags = team.tags.map(tag => `<span>${esc(tag)}</span>`).join('');
    return `
      <button type="button" class="writeflow-team-card ${selected ? 'selected' : ''}" data-writeflow-team="${esc(team.id)}">
        <span class="writeflow-team-image">${_writeflowAvatarHtml(team.image, team.title, 'writeflow-team-avatar')}</span>
        <span class="writeflow-team-copy">
          <strong>${esc(team.title)}</strong>
          <em>${esc(team.category)} · ${esc(team.statusLabel)}</em>
          <span>${esc(team.description)}</span>
          <span class="writeflow-team-tags">${tags}</span>
          <span class="writeflow-team-members">${memberAvatars}</span>
        </span>
      </button>`;
  }).join('');
  grid.querySelectorAll('[data-writeflow-team]').forEach(btn => {
    btn.addEventListener('click', () => openWriteflowTeamModal(btn.getAttribute('data-writeflow-team')));
  });
}

async function loadWriteflow(force) {
  const grid = $('writeflowTeamGrid');
  const statusEl = $('writeflowCatalogStatus');
  if (!grid) return;
  renderWriteflowTeams();
  if (_writeflowData && !force) {
...
function _writeflowInputPayload(action) {
  const project = (($('writeflowProject') || {}).value || '').trim();
  const mode = (($('writeflowMode') || {}).value || '').trim();
  const stage = (($('writeflowStage') || {}).value || '').trim();
  const prompt = (($('writeflowPrompt') || {}).value || '').trim();
  const payload = { action, project, mode, stage, prompt, team_id: _writeflowSelectedTeamId };
  if (_writeflowSelectedRunId && action !== 'start') payload.run_id = _writeflowSelectedRunId;
  return payload;
}

function _writeflowProjectFromState() {
  if (!_writeflowData || !Array.isArray(_writeflowData.projects)) return '';
  if (_writeflowSelectedRunId && Array.isArray(_writeflowData.runs)) {
    const run = _writeflowData.runs.find(item => item.run_id === _writeflowSelectedRunId);
    if (run) return run.project_slug || run.title || '';
  }
  const active = _writeflowData.active_project;
  const match = _writeflowData.projects.find(p => p.slug === active) || _writeflowData.projects[0];
  return match ? (match.slug || match.name || '') : '';
}

function startWriteflow() {
  sendWriteflowPanelAction('start');
}

function sendWriteflowPanelAction(action) {
  const payload = _writeflowInputPayload(action);
  const team = _writeflowTeamById(payload.team_id);
  if (!payload.project) payload.project = _writeflowProjectFromState();
  if (!payload.project && action !== 'status') payload.project = _writeflowProjectTitleFromPrompt(payload.prompt, team);
  if (!payload.mode && team && team.defaultMode) payload.mode = team.defaultMode;
  if (!payload.project && action !== 'status') {
    showToast('请先填写项目名，或选择一个已有写作项目。');
    return;
  }
  if (typeof sendWriteflowAction !== 'function') {
    showToast('写作命令运行环境不可用。');
    return;
  }
  sendWriteflowAction(payload);
}
...
function openWriteflowTeamModal(teamId) {
  ...
  body.querySelectorAll('.writeflow-example').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      _writeflowSelectedTemplateId = btn.getAttribute('data-template-id') || '';
      const prompt = btn.getAttribute('data-example-prompt') || '';
      const modalPrompt = $('writeflowTeamPrompt');
      if (modalPrompt) modalPrompt.value = prompt;
      const panelPrompt = $('writeflowPrompt');
      if (panelPrompt) panelPrompt.value = prompt;
    });
  });
}
...
function _writeflowModeLabel(mode) {
  ...
  if (key === 'A') return '轻量写作';
  if (key === 'B') return '深度协作';
  if (key === 'C') return '从选题开始';
  return '未指定模式';
}

function _writeflowArtifactEntries(project) {
  const display = project.display_artifacts || {};
  const displayEntries = Object.entries(display).filter(([, rel]) => rel);
  if (displayEntries.length) return displayEntries;
  return Object.entries(project.artifacts || {}).map(([key, rel]) => [WRITEFLOW_ARTIFACT_LABELS[key] || key, rel]);
}

function _writeflowProgressHtml(phase) {
  const current = WRITEFLOW_PHASES.includes(phase) ? phase : WRITEFLOW_PHASES[0];
  const currentIdx = WRITEFLOW_PHASES.indexOf(current);
  return `<div class="writeflow-progress" aria-label="写作三步流程">${WRITEFLOW_PHASES.map((label, idx) => {
    const cls = idx < currentIdx ? 'done' : (idx === currentIdx ? 'active' : 'todo');
    return `<span class="writeflow-step ${cls}"><b>${idx + 1}</b>${esc(label)}</span>`;
  }).join('')}</div>`;
}

function _writeflowMemberStatusClass(status) {
  const text = String(status || '');
  if (text.includes('完成')) return 'done';
  if (text.includes('处理') || text.includes('写作')) return 'running';
  if (text.includes('确认')) return 'waiting';
  if (text.includes('需要')) return 'issue';
  return 'idle';
}

function _writeflowRunStatusClass(status) {
  const text = String(status || '');
  if (text.includes('完成') || text === 'done') return 'done';
  if (text.includes('确认') || text === 'waiting_user') return 'waiting';
  if (text.includes('需要') || text === 'error' || text === 'blocked') return 'issue';
  if (text.includes('执行') || text === 'running') return 'running';
  return 'idle';
}

function _writeflowTaskRows(tasks) {
  const rows = Array.isArray(tasks) ? tasks : [];
  if (!rows.length) return '<div class="writeflow-empty">还没有任务拆解。</div>';
  return rows.map(task => {
    const cls = _writeflowRunStatusClass(task.status_label || task.status);
    const artifacts = Array.isArray(task.artifacts) && task.artifacts.length ? ` · ${task.artifacts.length} 个产物` : '';
    return `<div class="writeflow-task ${cls}">
      <span class="writeflow-task-dot"></span>
      <span class="writeflow-task-copy">
        <strong>${esc(task.title || task.id || '')}</strong>
        <em>${esc(task.worker_name || task.worker_id || '')} · ${esc(task.status_label || task.status || '待执行')}${artifacts}</em>
      </span>
    </div>`;
  }).join('');
}

function _writeflowArtifactRows(items, emptyText) {
  const rows = Array.isArray(items) ? items : [];
  if (!rows.length) return `<div class="writeflow-empty">${esc(emptyText || '还没有记录产物。')}</div>`;
  return rows.map(item => {
    const path = item.path || '';
    const placeholder = item.placeholder ? ' placeholder' : '';
    const disabled = path ? '' : ' disabled';
    const kind = item.kind || item.change_type || 'file';
    return `<button class="writeflow-artifact${placeholder}" data-writeflow-path="${esc(path)}"${disabled}>
      <span>${esc(item.label || item.path || item.id || '产物')}</span>
      <small>${esc(kind)}${path ? ` · ${path}` : ''}${item.note ? ` · ${item.note}` : ''}</small>
    </button>`;
  }).join('');
}

function _writeflowEventRows(events) {
  const rows = Array.isArray(events) ? events.slice(-4) : [];
  if (!rows.length) return '<div class="writeflow-empty">还没有团队事件。</div>';
  return rows.map(event => `<div class="writeflow-event">
    <span>${esc(event.label || event.type || '事件')}</span>
    <em>${esc(event.created_at || '')}</em>
  </div>`).join('');
}

function _writeflowRunMembers(run) {
  const rows = Array.isArray(run.members) ? run.members : [];
  if (!rows.length) return '<span class="writeflow-empty">等待成员状态。</span>';
  return rows.map(member => {
    const cls = _writeflowMemberStatusClass(member.status);
    const image = member.image || WRITEFLOW_MEMBER_IMAGES[member.id] || WRITEFLOW_MEMBER_IMAGES[member.skill] || '';
    return `<span class="writeflow-runtime-member ${cls}">
      ${_writeflowAvatarHtml(image, member.name, 'writeflow-runtime-avatar')}
      <span><strong>${esc(member.name || member.id || '')}</strong><em>${esc(member.status || '待命')}</em></span>
    </span>`;
  }).join('');
}

function _writeflowRunCards(data) {
  const runs = Array.isArray(data && data.runs) ? data.runs : [];
  if (!runs.length) return '';
  if (!_writeflowSelectedRunId || !runs.some(run => run.run_id === _writeflowSelectedRunId)) {
    _writeflowSelectedRunId = runs[0].run_id || '';
  }
  return runs.map(run => {
    const progress = run.progress || {};
    const done = Number(progress.done || 0);
    const total = Number(progress.total || (Array.isArray(run.tasks) ? run.tasks.length : 0));
    const selected = run.run_id === _writeflowSelectedRunId;
    const team = _writeflowTeamById(run.team_id) || {};
    return `<section class="writeflow-run ${selected ? 'active' : ''}" data-writeflow-run="${esc(run.run_id || '')}">
      <div class="writeflow-run-head">
        <div>
          <strong>${esc(run.title || run.project_slug || '写作任务')}</strong>
          <span>${esc(team.title || run.team_id || '')} · ${esc(run.phase || '确定方向')}</span>
        </div>
        <em class="${esc(_writeflowRunStatusClass(run.status_label || run.status))}">${esc(run.status_label || run.status || '执行中')}</em>
      </div>
      <div class="writeflow-run-progress">
        <span>任务列表 · ${done}/${total || 2} 已完成</span>
        <small>${esc(run.updated_at || '')}</small>
      </div>
      <div class="writeflow-runtime-members">${_writeflowRunMembers(run)}</div>
      <div class="writeflow-task-list">${_writeflowTaskRows(run.tasks)}</div>
      <div class="writeflow-run-subtitle">任务产物</div>
      <div class="writeflow-artifacts">${_writeflowArtifactRows(run.artifacts, '还没有任务产物。')}</div>
      <div class="writeflow-run-subtitle">文件变更</div>
      <div class="writeflow-artifacts">${_writeflowArtifactRows(run.file_changes, '还没有文件变更。')}</div>
      <div class="writeflow-run-subtitle">过程状态</div>
      <div class="writeflow-events">${_writeflowEventRows(run.events)}</div>
    </section>`;
  }).join('');
}

function renderWriteflow(data) {
  const grid = $('writeflowTeamGrid');
  const statusEl = $('writeflowCatalogStatus');
  if (!grid) return;
  _writeflowApplyServerTeams(data && data.teams);
  renderWriteflowTeams();
  if (data && data.state_error) {
    if (statusEl) statusEl.textContent = '状态文件无法读取：' + data.state_error;
    return;
  }
  if (statusEl && !($('writeflowTeamSearch') || {}).value) statusEl.textContent = '';

  const projectsEl = $('writeflowProjects');
  const runsEl = $('writeflowRuns');
  if (!projectsEl || !runsEl) return;
  const projects = Array.isArray(data && data.projects) ? data.projects : [];
  runsEl.innerHTML = _writeflowRunCards(data);
  projectsEl.innerHTML = projects.map(project => {
    const isActive = project.slug === data.active_project;
    const phase = project.display_phase || '确定方向';
    const displayStatus = project.display_status || project.status || '未开始';
    const team = project.display_team || {};
    const members = Array.isArray(project.display_members) ? project.display_members : [];
    const memberRows = members.map(member => {
      const localTeam = WRITEFLOW_TEAMS.flatMap(item => item.members).find(item => item.id === member.id || item.id === member.skill);
      const image = (localTeam && localTeam.image) || '';
      const cls = _writeflowMemberStatusClass(member.status);
      return `<span class="writeflow-runtime-member ${cls}">${_writeflowAvatarHtml(image, member.name, 'writeflow-runtime-avatar')}<span><strong>${esc(member.name || member.id || '')}</strong><em>${esc(member.status || '待命')}</em></span></span>`;
    }).join('');
    const artifactRows = _writeflowArtifactEntries(project).map(([label, rel]) => {
      return `<button class="writeflow-artifact" data-writeflow-path="${esc(rel)}"><span>${esc(label)}</span><small>${esc(rel)}</small></button>`;
    }).join('') || '<div class="writeflow-empty">还没有记录产物。</div>';
    return `
      <section class="writeflow-project ${isActive ? 'active' : ''}">
        <div class="writeflow-project-head">
          <div>
            <strong>${esc(project.name || project.slug)}</strong>
            <span>${esc(project.slug || '')}</span>
          </div>
          <em>${esc(displayStatus)}</em>
        </div>
        <div class="writeflow-runtime">
          <div class="writeflow-runtime-title">
            <strong>${esc(team.title || '内容创作专家团')}</strong>
            <span>${esc(team.category || '写作')} · ${esc(team.status_label || team.statusLabel || '本地技能已接入')}</span>
          </div>
          <div class="writeflow-runtime-members">${memberRows || '<span class="writeflow-empty">等待专家团状态。</span>'}</div>
        </div>
        ${_writeflowProgressHtml(phase)}
        <div class="writeflow-meta">
          <span>${esc(_writeflowModeLabel(project.mode))}</span>
          <span>${esc(phase)}</span>
          <span>${esc(project.updated_at || '尚未更新')}</span>
        </div>
        <div class="writeflow-artifacts">${artifactRows}</div>
      </section>`;
  }).join('');
  const bindPathButtons = root => root.querySelectorAll('[data-writeflow-path]').forEach(btn => {
    btn.addEventListener('click', async () => {
      const rel = btn.getAttribute('data-writeflow-path');
      if (!rel) return;
      if (!S.session) {
        await newSession();
  ...
  bindPathButtons(runsEl);
  bindPathButtons(projectsEl);
  runsEl.querySelectorAll('[data-writeflow-run]').forEach(card => {
    card.addEventListener('click', event => {
      if (event.target.closest('[data-writeflow-path]')) return;
      _writeflowSelectedRunId = card.getAttribute('data-writeflow-run') || '';
      renderWriteflow(_writeflowData || data);
    });
  });
}
...
function _renderProfileDetail(p, activeName){
  ...
  if (p.total_skills && p.total_skills > 0) rows.push(`<div class="detail-row"><div class="detail-row-label">技能</div><div class="detail-row-value">${esc(t('profile_skill_count', p.total_skills).replace(String(p.total_skills), `${p.enabled_skills} / ${p.total_skills}`))}</div></div>`);
  else if (typeof p.skill_count === 'number') rows.push(`<div class="detail-row"><div class="detail-row-label">技能</div><div class="detail-row-value">${esc(t('profile_skill_count', p.skill_count))}</div></div>`);
  if (p.default_workspace) rows.push(`<div class="detail-row"><div class="detail-row-label">默认工作区</div><div class="detail-row-value"><code>${esc(p.default_workspace)}</code></div></div>`);
  body.innerHTML = `
    <div class="main-view-content">
      <div class="detail-card">
  ...
  try{
    ...
    window._populateTtsVoices=function(){
      ...
      if(engine==='edge'){
        ...
        ttsVoiceSel.innerHTML='<option value="">系统默认语音</option>';
        voices.forEach(v=>{
          const opt=document.createElement('option');
...
function openImageProvidersPanel(){
 switchSettingsSection('providers');
 setTimeout(()=>{
  const target=$('providerImageGenServices')||$('providerImageGenTemplates')||$('customImageProviderPanel');
  if(target&&target.scrollIntoView) target.scrollIntoView({block:'start',behavior:'smooth'});
 },0);
 return false;
}

function _openCustomImageProviderTemplate(templateId){
 openImageProvidersPanel();
 setTimeout(()=>openCustomImageProviderEditor('',templateId),0);
}
...
async function loadProvidersPanel(){
  ...
  try{
    ...
    list.innerHTML='<div style="color:var(--error);padding:12px;font-size:13px">加载提供商失败：'+esc(e.message||String(e))+'</div>';
    _renderProviderImageGenSettings(_modelConfigData);
  }
}
...
function _buildProviderCard(p){
  ...
  if(p.configurable){
    ...
    toggleBtn.onclick=()=>{
      ...
      toggleBtn.textContent=revealed?'显示':'隐藏';
    };
...
async function _removeProviderKey(providerId){
  ...
  try{
    ...
    if(e&&e.status===403){
      showToast(e.message||'会话已过期。请重新加载页面后重试。',6000,'error');
    }else{
      showToast('错误：'+e.message);
    }
    if(els.saveBtn){els.saveBtn.disabled=false;els.saveBtn.textContent=t('providers_save');}
...
async function _refreshProviderModels(providerId, btn){
  ...
  try{
    ...
    if(res.ok){
      showToast(t('providers_models_refreshed')||('已刷新 '+res.provider+' 的模型'));
    }else{
      showToast(res.error||'刷新模型失败');
    }
  }catch(e){
    showToast('错误：'+e.message);
  }finally{
...
async function loadPasskeys(){
  ...
  if(!window.PublicKeyCredential||!navigator.credentials){
    list.textContent='此浏览器或上下文不支持通行密钥。';
  ...
  try{
    ...
    if(!creds.length){list.textContent='尚未注册通行密钥。';return;}
    list.innerHTML=creds.map(c=>`<div style="display:flex;align-items:center;justify-content:space-between;gap:8px;border:1px solid var(--border);border-radius:8px;padding:8px;margin-top:6px"><span>${esc(c.label||'通行密钥')}</span><button class="btn-tiny" onclick="deletePasskey('${esc(c.id)}')">移除</button></div>`).join('');
  }catch(e){list.textContent='加载通行密钥失败：'+e.message;}
}

async function registerPasskey(){
  if(!window.PublicKeyCredential||!navigator.credentials){showToast('通行密钥需要受支持的浏览器和安全上下文。');return;}
  const label='此设备';
  try{
    ...
    if(!cred) throw new Error('通行密钥注册已取消');
  ...
  }catch(e){showToast('通行密钥注册失败：'+e.message);}
}

async function deletePasskey(id){
  const ok=await showConfirmDialog({title:'移除通行密钥？',message:'此浏览器/设备将无法再使用该通行密钥登录。',confirmLabel:'移除',danger:true,focusCancel:true});
  if(!ok) return;
  try{await api('/api/auth/passkey/delete',{method:'POST',body:JSON.stringify({id})});showToast('通行密钥已移除');loadPasskeys();try{_syncPasswordlessButton(await api('/api/auth/status'));}catch(_e){}}
  catch(e){showToast('移除通行密钥失败：'+e.message);}
}
...
function _modelConfigProviderRows(data){
 const rows=Array.isArray(data&&data.providers)?data.providers.slice():[];
 if(!rows.some(p=>p&&p.id==='custom')){
  rows.push({id:'custom',display_name:'Custom OpenAI-compatible',models:[],configurable:true,has_key:false});
 }
 const active=((data&&data.main&&data.main.provider)||'').trim();
 if(active&&!rows.some(p=>p&&p.id===active)){
  rows.push({id:active,display_name:active,models:[],configurable:false,has_key:false});
 }
 return rows;
}

function _modelConfigProviderById(providerId){
 const rows=_modelConfigProviderRows(_modelConfigData);
 const id=String(providerId||'').trim();
 return rows.find(p=>String(p.id||'')===id)||null;
}

function _setDatalistOptions(id,models){
 const dl=$(id);
 if(!dl) return;
 dl.innerHTML='';
 (models||[]).forEach(model=>{
  const value=typeof model==='string'?model:String(model&&model.id||'').trim();
  if(!value) return;
  const opt=document.createElement('option');
  opt.value=value;
  const label=typeof model==='string'?model:String(model&&model.label||model.id||'');
...
function _invalidateAlibabaQuickOperation(message){
 ...
 if(keyInput&&operation.secret&&String(keyInput.value||'').trim()===operation.secret) keyInput.value='';
...
function _mergeAlibabaQuickConfigResponse(result){
 ...
 if(Array.isArray(data.provider_credentials)) _modelConfigData.provider_credentials=data.provider_credentials;
...
async function saveAndVerifyAlibabaImageCapabilities(){
 ...
 try{
  ...
  const current=_alibabaQuickOperation===operation;
  const ownsInput=!!(keyInput&&operation.secret&&String(keyInput.value||'').trim()===operation.secret);
  if(ownsInput&&(current||!_alibabaQuickOperation)) keyInput.value='';
  if(current){
   _alibabaQuickOperation=null;
   _setAlibabaQuickBusy(false);
  }
...
function _captureImageCapabilityProviderDraft(capability,providerId){
 const id=String(providerId||'').trim();
 if(!id) return;
 const prefix=capability==='vision'?'visionConfig':'imageGenConfig';
 const draft={};
 ['Model','BaseUrl','ApiKey','Credential','EndpointMode','Region','WorkspaceId'].forEach(suffix=>{
 ...
 if(capability==='image'){
  ...
  document.querySelectorAll('[data-image-gen-credential]').forEach(input=>{
   const key=String(input.dataset.imageGenCredential||'').trim();
   if(key&&(input.dataset.secret==='1'||input.dataset.secretField==='true')) draft.CredentialSecretKeys.push(key);
  });
 }
 _imageCapabilityProviderDrafts[capability][id]=draft;
}

function _restoreImageCapabilityProviderDraft(capability,providerId){
 const id=String(providerId||'').trim();
 const draft=_imageCapabilityProviderDrafts[capability][id];
 ...
 if(capability==='image'&&draft.Credentials){
  document.querySelectorAll('[data-image-gen-credential]').forEach(input=>{
   const key=String(input.dataset.imageGenCredential||'').trim();
   if(key&&Object.prototype.hasOwnProperty.call(draft.Credentials,key)) input.value=draft.Credentials[key];
  });
 }
 return true;
}
...
function _providerSupportsNamedCredential(capability,providerId){
 const id=String(providerId||'').trim().toLowerCase();
 return capability==='vision'?id==='alibaba':id==='dashscope';
}

function _imageCapabilityCredentialRef(capability,providerId){
 if(!_providerSupportsNamedCredential(capability,providerId)) return '';
 const id=capability==='vision'?'visionConfigCredential':'imageGenConfigCredential';
 return String((($(id)||{}).value)||'').trim();
}

function _syncPlatformCredentialSurface(){
 const visionProvider=String((($('visionConfigProvider')||{}).value)||'').trim();
 const imageProvider=String((($('imageGenConfigProvider')||{}).value)||'').trim();
 const visionNamed=_providerSupportsNamedCredential('vision',visionProvider);
 const imageNamed=_providerSupportsNamedCredential('image',imageProvider);
 const visionRow=$('visionConfigCredentialRow');
 const imageRow=$('imageGenConfigCredentialRow');
 const add=$('btnAddPlatformCredential');
 if(visionRow) visionRow.hidden=!visionNamed;
 if(imageRow) imageRow.hidden=!imageNamed;
 if(add) add.hidden=!(visionNamed||imageNamed);
 if(!visionNamed){const select=$('visionConfigCredential');if(select) select.value='';}
 if(!imageNamed){const select=$('imageGenConfigCredential');if(select) select.value='';}
}
...
function _renderImageCapabilityEndpointPreview(capability){
 ...
 if(namedCustomVision){
  ...
  else if(!/^[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?$/.test(workspace)) problem='Workspace ID 必须以字母或数字开头和结尾，中间可使用连字符，最长 63 个字符。';
  ...
  Object.keys(values).forEach(id=>{
   ...
   if(el) el.value=values[id];
	  ...
	  const values={
	   ...
	   customImageProviderModels:template.models||'',
	   customImageProviderDefaultModel:template.defaultModel||'',
	  };
	  Object.keys(values).forEach(id=>{
	   const el=$(id);
	   if(el) el.value=values[id];
...
function _providerImageGenProtocol(row){
 const id=String(row&&row.id||'').toLowerCase();
 if(row&&row.oauth_managed) return '授权托管';
 if(row&&row.custom) return '外部模型';
 if(id.indexOf('dashscope')!==-1) return 'DashScope';
 if(id.indexOf('seedream')!==-1||id.indexOf('doubao')!==-1) return '火山方舟';
 if(id.indexOf('qianfan')!==-1||id.indexOf('baidu')!==-1) return '千帆大模型';
 if(id.indexOf('hunyuan')!==-1) return '腾讯云';
 return '内置适配';
}

function _providerImageGenStatus(row,activeProvider){
 const id=String(row&&row.id||'');
 if(row&&row.available===true){
  return {label:id&&id===activeProvider?'已生效':'可用',state:'ok'};
 }
 const key=row&&row.key_status?row.key_status:{};
 if(key.configured) return {label:'待验证',state:'warn'};
 if(row&&row.oauth_managed) return {label:'待授权',state:'warn'};
 return {label:'待配置',state:'warn'};
}

function _providerImageGenModelLabel(row,data){
 const active=(data&&data.image_gen)||{};
 const id=String(row&&row.id||'');
 if(id&&id===String(active.provider||'')&&active.model) return active.model;
 return String((row&&row.default_model)||((row&&Array.isArray(row.models)&&row.models[0])||'')||'未配置模型');
}

function _providerImageServiceCell(text,className){
...
function _renderProviderImageGenSettings(data){
 const host=$('providerImageGenServices');
 ...
 const rows=Array.isArray(cfg.image_gen_providers)?cfg.image_gen_providers:[];
 const customRows=_modelConfigCustomImageRows(cfg);
 const activeProvider=String((cfg.image_gen&&cfg.image_gen.provider)||'');
 const available=rows.filter(row=>row&&row.available===true).length;
 const waiting=rows.filter(row=>row&&row.available!==true).length;
 if(overview) overview.textContent=available+' 可用 / '+customRows.length+' 外部 / '+waiting+' 待处理';
 _renderCustomImageProviderList(cfg);
 if(!host) return;
 host.innerHTML='';
 if(!rows.length){
  const empty=document.createElement('div');
  empty.className='provider-image-service-empty';
  empty.textContent='尚未读取到图片生成提供商。';
  host.appendChild(empty);
  return;
 }
 const table=document.createElement('div');
 table.className='provider-image-service-grid';
 ['用途','厂商/服务','协议','状态','模型','操作'].forEach(label=>{
  const head=document.createElement('div');
  head.className='provider-image-service-head';
  head.textContent=label;
  table.appendChild(head);
 });
 rows.forEach(row=>{
  const id=String(row&&row.id||'');
  const name=String((row&&(row.name||row.display_name))||id||'图片生成服务');
  const status=_providerImageGenStatus(row,activeProvider);
  const model=_providerImageGenModelLabel(row,cfg);
  const purpose=id===activeProvider?'当前图片生成':'可选服务';
  table.appendChild(_providerImageServiceCell(purpose,'provider-image-service-purpose'));
  table.appendChild(_providerImageServiceCell(name+' · '+id,'provider-image-service-name'));
  table.appendChild(_providerImageServiceCell(_providerImageGenProtocol(row),'provider-image-service-protocol'));
  const statusCell=_providerImageServiceCell(status.label,'provider-image-service-status');
  statusCell.dataset.state=status.state;
  table.appendChild(statusCell);
  table.appendChild(_providerImageServiceCell(model,'provider-image-service-model'));
  const actions=document.createElement('div');
  actions.className='provider-image-service-actions';
  const select=document.createElement('button');
  select.className='btn-tiny';
  select.type='button';
  select.textContent=id===activeProvider?'修改':'选择';
  select.onclick=()=>_selectImageProviderFromProviders(id,model);
  ...
  if(row&&row.custom){
   ...
   edit.className='btn-tiny';
   edit.type='button';
   edit.textContent='更新密钥';
   edit.onclick=()=>openCustomImageProviderEditor(id);
   actions.appendChild(edit);
  }
...
async function _selectImageProviderFromProviders(providerId,modelId){
 ...
 if(modelInput&&modelId&&modelId!=='未配置模型') modelInput.value=modelId;
...
async function refreshProviderImageGenStatus(){
 try{
  ...
  if(typeof showToast==='function') showToast('图片生成提供商状态刷新失败：'+(e.message||e),5000,'error');
 }
}
...
function _renderModelConfigPanel(data){
 ...
 if(providerSel){
  ...
  _modelConfigProviderRows(data).forEach(p=>{
   if(!p||!p.id) return;
   const opt=document.createElement('option');
   opt.value=p.id;
   opt.textContent=(p.display_name||p.id)+(p.id?' · '+p.id:'');
 ...
 if(visionProviderSel){
  ...
  visionProviderSel.onchange=()=>{
   _invalidateVisionTest();
   const previous=String(visionProviderSel.dataset.lastProvider||'').trim();
   if(previous&&previous!==visionProviderSel.value) _captureImageCapabilityProviderDraft('vision',previous);
   const p=(data.vision_providers||[]).find(item=>String(item.id||'')===visionProviderSel.value)||null;
   const modelInput=$('visionConfigModel');
   if(modelInput&&!_imageCapabilityProviderDrafts.vision[visionProviderSel.value]){
    modelInput.value=(p&&p.default_model)||'';
   }
   _syncVisionConfigControls();
   _restoreImageCapabilityProviderDraft('vision',visionProviderSel.value);
   visionProviderSel.dataset.lastProvider=visionProviderSel.value;
   _syncImageCapabilityEndpointFields('vision');
 ...
 if(imageProviderSel){
  ...
  (data.image_gen_providers||[]).forEach(p=>{
   const opt=document.createElement('option');
   opt.value=p.id;
   opt.textContent=(p.name||p.id)+(p.id?' · '+p.id:'');
  ...
  imageProviderSel.onchange=()=>{
   _invalidateImageGenTest();
   const previous=String(imageProviderSel.dataset.lastProvider||'').trim();
   if(previous&&previous!==imageProviderSel.value) _captureImageCapabilityProviderDraft('image',previous);
   const p=(data.image_gen_providers||[]).find(item=>String(item.id||'')===imageProviderSel.value)||null;
   const modelInput=$('imageGenConfigModel');
   if(modelInput&&!_imageCapabilityProviderDrafts.image[imageProviderSel.value]){
    modelInput.value=(p&&p.default_model)||'';
   }
   _syncImageGenConfigControls();
   _restoreImageCapabilityProviderDraft('image',imageProviderSel.value);
   imageProviderSel.dataset.lastProvider=imageProviderSel.value;
   _syncImageCapabilityEndpointFields('image');
  };
  imageProviderSel.dataset.lastProvider=imageProviderSel.value;
 }
...
function _taijiLicenseSourceLabel(data){
 const mode=String(data&&data.activation_mode||'').toLowerCase();
 if(mode==='offline_machine_file') return '离线授权文件';
 if(mode==='online_code') return '联网激活';
 if(mode==='qr_proxy') return '扫码激活';
 return '—';
}
...
function _renderTaijiLicenseStatus(data){
 ...
 const sourceEl=$('taijiLicenseSource');
...
function _platformCredentialEditorHasUnsavedChanges(){
 const editor=$('platformCredentialEditor');
 if(!editor||editor.hidden) return false;
 const id=String((($('platformCredentialId')||{}).value)||'').trim();
 const row=Array.isArray(_modelConfigData&&_modelConfigData.provider_credentials)
...
function _modelConfigHasUnsavedChanges(){
 ...
 return _modelConfigMainHasUnsavedChanges()
  ...
  || Object.keys(_imageCapabilityProviderDrafts.vision).length>0
  || Object.keys(_imageCapabilityProviderDrafts.image).length>0
...
function _discardImageCapabilityProviderDrafts(){
 Object.keys(_imageCapabilityProviderDrafts.vision).forEach(key=>{delete _imageCapabilityProviderDrafts.vision[key];});
 Object.keys(_imageCapabilityProviderDrafts.image).forEach(key=>{delete _imageCapabilityProviderDrafts.image[key];});
}

function _setModelConfigDraftStatus(message){
...
async function loadModelConfigPanel(force,options){
 ...
 if(_modelConfigData&&!force){
  if(_modelConfigHasUnsavedChanges()){
 ...
 const generation=++_modelConfigLoadGeneration;
...
async function saveMainModelConfig(){
 const btn=$('btnSaveMainModel');
 const provider=($('modelConfigProvider')||{}).value||'';
...
async function saveVisionConfig(){
 ...
 const credentialRef=_imageCapabilityCredentialRef('vision',provider);
...
async function saveImageGenConfig(){
 ...
 const credentialRef=_imageCapabilityCredentialRef('image',provider);
...
function _modelConfigBuildAuxProviderOptions(sel,providers,currentProvider){
...
function _markModelConfigAuxDirty(){
 const applyBtn=$('btnApplyModelConfigAux');
 if(applyBtn) applyBtn.style.display='';
}

function _onModelConfigAuxProviderChange(taskKey,providers){
 const provSel=$('model-aux-prov-'+taskKey);
 const modelSel=$('model-aux-model-'+taskKey);
 if(!provSel||!modelSel) return;
 _modelConfigBuildAuxModelOptions(modelSel,provSel.value,providers,'');
 _markModelConfigAuxDirty();
}

async function _onModelConfigAuxModelChange(taskKey){
 const modelSel=$('model-aux-model-'+taskKey);
 if(!modelSel) return;
...
async function _loadModelConfigAuxiliaryModels(){
 const container=$('modelConfigAuxContainer');
 if(!container) return;
 container.innerHTML='<div class="model-config-loading">正在加载辅助模型...</div>';
 try{
  ...
  _modelConfigAuxProviders=groups.filter(g=>g.provider&&g.models&&g.models.length>0).map(g=>({
  ...
  for(const slot of _AUX_TASK_SLOTS){
   ...
   const row=document.createElement('div');
   row.className='model-config-aux-row';
   const label=document.createElement('div');
   label.className='model-config-aux-label';
   label.innerHTML=esc(slot.name)+'<span>'+esc(slot.desc)+'</span>';
   row.appendChild(label);
   const provSel=document.createElement('select');
   provSel.id='model-aux-prov-'+slot.key;
   _modelConfigBuildAuxProviderOptions(provSel,_modelConfigAuxProviders,cfg.provider);
   provSel.addEventListener('change',()=>_onModelConfigAuxProviderChange(slot.key,_modelConfigAuxProviders));
   row.appendChild(provSel);
   const modelSel=document.createElement('select');
   modelSel.id='model-aux-model-'+slot.key;
   _modelConfigBuildAuxModelOptions(modelSel,cfg.provider,_modelConfigAuxProviders,cfg.model);
   modelSel.addEventListener('change',()=>_onModelConfigAuxModelChange(slot.key));
  ...
  if(applyBtn){
   ...
   if(!applyBtn._modelConfigBound){
    applyBtn._modelConfigBound=true;
    applyBtn.addEventListener('click',_applyModelConfigAuxModels);
   }
  }
  const resetBtn=$('btnResetModelConfigAux');
  if(resetBtn&&!resetBtn._modelConfigBound){
   resetBtn._modelConfigBound=true;
   resetBtn.addEventListener('click',async()=>{
    if(!(await showConfirmDialog({title:'重置辅助模型？',message:'所有专用任务会回到 Auto，使用主模型。',confirmLabel:'重置',danger:true}))) return;
    try{
     await api('/api/model/set',{method:'POST',body:JSON.stringify({scope:'auxiliary',task:'__reset__',provider:'auto',model:''})});
     if(typeof showToast==='function') showToast('辅助模型已重置');
     _loadModelConfigAuxiliaryModels();
    }catch(e){
     if(typeof showToast==='function') showToast('重置辅助模型失败：'+(e.message||e),5000,'error');
    }
   });
  }
 }catch(e){
...
async function _applyModelConfigAuxModels(){
 let saved=0;
 for(const slot of _AUX_TASK_SLOTS){
  const provSel=$('model-aux-prov-'+slot.key);
  const modelSel=$('model-aux-model-'+slot.key);
  if(!provSel) continue;
  const provider=provSel.value;
  const model=(modelSel&&modelSel.value!=='__custom__')?(modelSel.value||''):'';
  const orig=_modelConfigAuxOriginalConfig?.[slot.key]||{provider:'auto',model:''};
...
async function _loadAuxiliaryModels(){
 ...
 if(!container) return;
 container.innerHTML='<div style="color:var(--muted);font-size:12px">'+(t('settings_aux_loading')||'加载中…')+'</div>';
 ...
 const tasks=(auxData&&auxData.tasks)||[];
  ...
  container.innerHTML='<div style="color:var(--muted);font-size:12px">'+(t('settings_aux_load_failed')||'无法加载辅助模型设置。请确认 Agent API 可用。')+'</div>';
 }
}
    ...
    try{
      ...
      if(modelChanged && model){
        try{
          ...
          if(typeof showToast==='function') showToast('更新默认模型失败 — 设置已保存');
        }
      }
...
async function goPasswordless(){
  const ok=await showConfirmDialog({title:'切换为无密码？',message:'这会移除密码，并保持通行密钥登录启用。请至少保留一个已注册通行密钥，否则可能无法访问。',confirmLabel:'切换为无密码',danger:false,focusCancel:true});
  if(!ok) return;
  try{
    const saved=await api('/api/settings',{method:'POST',body:JSON.stringify({_passwordless:true})});
    showToast('密码已移除。通行密钥登录仍保持启用。');
    _setSettingsAuthButtonsVisible(!!saved.auth_enabled);
  ...
  }catch(e){showToast('切换为无密码失败：'+e.message);}
}
