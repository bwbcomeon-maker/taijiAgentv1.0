async function api(path,opts={}){
  ...
  for(let attempt=0;attempt<3;attempt++){
    ...
    try{
      ...
      if(isTimeout){
        const err=(e&&e.name==='TimeoutError')?e:new Error('请求超时，请重试。');
        err.name='TimeoutError';
        err.timeout=true;
        if(timeoutToast&&typeof showToast==='function') showToast('请求超时，请重试。',5000,'error');
        throw err;
      }
...
const ARTIFACT_DIRECT_PATH_RE = /articles\/[^\s`"'<>]+?\.(?:md|markdown|txt|png|jpg|jpeg|webp|gif|svg|pdf|docx)/gi;
...
function _artifactLabelForPath(path){
  ...
  if(/\.(png|jpe?g|webp|gif|svg)$/i.test(file))return '文章配图';
  if(/\.(md|markdown|txt)$/i.test(file))return '文稿文件';
  return file||'项目产物';
}
...
function _artifactCandidatesFromToolCall(tc){
  ...
  if(ARTIFACT_MUTATION_TOOLS.has(name)){
    const directText=[tc.preview,tc.snippet,resultText].filter(Boolean).join('\n');
    let match;
    while((match=ARTIFACT_DIRECT_PATH_RE.exec(directText))){
      add(match[0]);
    }
  }
...
function renderSessionArtifacts(){
  ...
  if(!S.session){
    root.innerHTML = '<div class="workspace-artifact-empty">打开一个对话后，可查看此会话中变更的文件。</div>';
    return;
  }
  if(!items.length){
    root.innerHTML = '<div class="workspace-artifact-empty">尚未检测到产物。此会话中创建或编辑的文件会显示在这里。</div>';
    return;
  }
  ...
  root.innerHTML = items.map(item => `<button type="button" class="workspace-artifact-item" data-artifact-path="${esc(item.path)}" onclick="openArtifactPath(this.dataset.artifactPath)"><div class="workspace-artifact-title">${esc(_artifactLabelForPath(item.path))}</div><div class="workspace-artifact-path">${esc(displayPath(item.path))}</div><div class="workspace-artifact-meta">${esc(item.source || 'session')}</div></button>`).join('');
}
...
function updateEditBtn(){
  ...
  if(_previewDirty) btn.innerHTML = '&#128190; 保存*';
}
...
function downloadFile(path, filenameOverride=''){
  if(!S.session)return;
  ...
  const filename=filenameOverride||path.split('/').pop();
