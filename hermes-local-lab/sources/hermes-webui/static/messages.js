...
async function send(){
  ...
  try{
    ...
    if(typeof window!=='undefined'&&window._pendingWriteflowStatusCard){
      const pending=window._pendingWriteflowStatusCard;
    ...
    renderMessages();appendThinking('',{pending:true});setBusy(true);
    ...
    source.addEventListener('done',e=>{
      ...
      const _finishDone=()=>{
        ...
        if(isActiveSession){
          ...
          if(_markerOnlyAssistantError&&typeof showToast==='function') showToast('上下文压缩后未收到回复。请重试。',5000,'error');
        ...
        renderSessionList();
        if(isActiveSession&&typeof refreshWriteflowStatusDockForActiveSession==='function'){
          refreshWriteflowStatusDockForActiveSession();
        }
