...
const COMMANDS=[
  ...
  {name:'writeflow', desc:t('cmd_writeflow')||'运行写作工作流', fn:cmdWriteflow, arg:'开始|状态|继续|返工|跳过|导出|风格|提取', subArgs:['start','status','next','redo','skip','export','style','extract','开始','状态','继续','返工','跳过','导出','风格','提取'], noEcho:true},
...
async function _runManualCompression(focusTopic){
  ...
  try{
    ...
    try{
      ...
      if(typeof setComposerStatus==='function') setComposerStatus('');
      renderMessages();
      showToast('压缩失败：'+(preflightErr.message||'会话已不可用'));
      return;
    }
    if(typeof setBusy==='function') setBusy(true);
    ...
    if(started&&started.status==='error'){
      const err=new Error(started.error||'压缩失败');
...
async function cmdSkills(args){
  try{
    ...
    showToast('加载技能失败：'+e.message);
  }
}

function _parseWriteflowArgs(args){
  const raw=String(args||'').trim();
  const aliases={
    '开始':'start','启动':'start','新建':'start',
    '状态':'status','查看状态':'status',
    '继续':'next','下一步':'next',
    '调整':'redo','调整要求':'redo','返工':'redo','重做':'redo',
    '跳过':'skip','导出':'export','风格':'style','提取':'extract'
  };
  const known=new Set(['start','status','next','redo','skip','export','style','extract']);
  if(!raw)return{action:'status',prompt:''};
  const parts=raw.split(/\s+/);
  let action=aliases[parts[0]]||parts[0].toLowerCase();
  let rest=parts.slice(1);
  if(!known.has(action)){
    action='start';
    rest=parts;
  }
  const body={action,prompt:rest.join(' ').trim()};
  const modeIdx=rest.findIndex(p=>/^--?mode$/i.test(p));
  if(modeIdx>=0&&rest[modeIdx+1]){
    body.mode=String(rest[modeIdx+1]).trim().toUpperCase().slice(0,1);
    rest.splice(modeIdx,2);
  }
  const stageIdx=rest.findIndex(p=>/^--?stage$/i.test(p));
  if(stageIdx>=0&&rest[stageIdx+1]){
    body.stage=String(rest[stageIdx+1]).trim();
    rest.splice(stageIdx,2);
  }
  if(action==='redo'||action==='skip'){
    body.stage=body.stage||rest.shift()||'';
  }
  if(rest.length){
    body.project=rest.shift();
    body.prompt=rest.join(' ').trim();
  }
  return body;
}

async function sendWriteflowAction(payload){
  const body={...(payload||{})};
  const wantsNewSession=body.new_session===true||body.open_new_session===true;
  const wantsNewWindow=body.new_window===true||body.open_new_window===true;
  delete body.new_session;
  delete body.open_new_session;
  delete body.new_window;
  delete body.open_new_window;
  const previousSid=S.session&&S.session.session_id||'';
  const previousUrl=(typeof window!=='undefined'&&window.location)
    ? window.location.pathname+window.location.search+window.location.hash
    : '';
  let writeflowWindow=null;
  let writeflowWindowNavigated=false;
  if(wantsNewWindow&&typeof window!=='undefined'&&typeof window.open==='function'){
    try{writeflowWindow=window.open('about:blank','_blank');}catch(_){writeflowWindow=null;}
  }
  const closeWriteflowWindow=()=>{
    if(wantsNewWindow&&writeflowWindow&&!writeflowWindowNavigated&&!writeflowWindow.closed){
      try{writeflowWindow.close();}catch(_){}
    }
  };
  if(S.busy&&!wantsNewSession){
    closeWriteflowWindow();
    showToast(t('writeflow_busy')||'写作工作流正在执行。');
    return;
  }
  ...
  try{
    const data=await api('/api/writeflow/compose',{method:'POST',body:JSON.stringify(body)});
    if(!data||!data.message)throw new Error('生成写作指令失败');
    const ta=$('msg');
    ...
    if(body.summon_only){
      closeWriteflowWindow();
      showToast('写作任务已放入聊天输入框，请确认后发送。');
      return;
    }
    await send();
    if(wantsNewWindow&&S.session&&S.session.session_id){
      const newSid=S.session.session_id;
      const path=typeof _sessionUrlForSid==='function'
        ? _sessionUrlForSid(newSid)
        : `/session/${encodeURIComponent(newSid)}`;
      const href=new URL(path,window.location.origin).href;
      if(writeflowWindow&&!writeflowWindow.closed){
        writeflowWindow.location.href=href;
        try{writeflowWindow.focus();}catch(_){}
        writeflowWindowNavigated=true;
      }else if(typeof window!=='undefined'&&typeof window.open==='function'){
        const opened=window.open(href,'_blank');
        if(opened){try{opened.focus();}catch(_){};writeflowWindowNavigated=true;}
      }
      if(previousSid&&typeof loadSession==='function'){
        await loadSession(previousSid);
        await renderSessionList();
      }else if(previousUrl&&typeof window!=='undefined'&&window.history){
        window.history.pushState({},'',previousUrl);
      }
      if(typeof switchPanel==='function')await switchPanel('writing');
      if(!writeflowWindowNavigated&&typeof window!=='undefined'){
        window.location.href=href;
      }
    }
  }catch(e){
    closeWriteflowWindow();
    showToast((t('writeflow_compose_failed')||'写作工作流失败：')+(e&&e.message||e));
  }
}
if(typeof window!=='undefined')window.sendWriteflowAction=sendWriteflowAction;

function cmdWriteflow(args){
  sendWriteflowAction(_parseWriteflowArgs(args));
}

const WRITEFLOW_STATUS_MEMBER_IMAGES={
...
function _writeflowStatusCardFromCompose(data){
  ...
  const visualMembers=members.map(member=>({
    ...
    image:member.image||WRITEFLOW_STATUS_MEMBER_IMAGES[member.id]||WRITEFLOW_STATUS_MEMBER_IMAGES[member.skill]||'',
  }));
  ...
  const rows=[
    ...
    {label:'产物',value:artifacts.length?`${artifacts.length} 个记录产物`:'等待产物生成'},
  ];
	  return {
	    type:'writeflow',
	    title:'专家团运行',
...
function _queueWriteflowStatusCard(data){
  const card=_writeflowStatusCardFromCompose(data);
...
function _formatStatusTokens(s){
  ...
  return `输入 ${fmtNum(input)} / 输出 ${fmtNum(output)}${cost?`（约 $${cost.toFixed(4)}）`:''}`;
}
...
function cmdReasoning(args){
  ...
  if(!arg){
    ...
    api('/api/reasoning'+q).then(function(st){showToast(_fmtStatus(st));})
      .catch(function(){showToast(BRAIN+' /reasoning — 状态不可用');});
    return true;
  }
  if(arg==='show'||arg==='on'||arg==='hide'||arg==='off'){
    ...
    showToast(BRAIN+' 思考块：'+(on?'开启':'关闭')+'（已保存）');
    return true;
  }
  if(EFFORTS.includes(arg)){
    ...
    api('/api/reasoning',{method:'POST',body:JSON.stringify({effort:arg})})
      .then(function(st){
        ...
        showToast(BRAIN+' 推理强度：'+eff+'（已保存；从下一轮生效）');
        if(typeof _applyReasoningChip==='function') _applyReasoningChip(eff, st||{});
      })
      .catch(function(e){
        showToast(BRAIN+' 设置推理强度失败：'+(e && e.message ? e.message : arg));
      });
    return true;
  }
  showToast('未知参数：'+arg+' — 可用 show|hide|'+EFFORTS.join('|'));
  return true;
}
function cmdVoice(){
