...
const STATUS_CARD_WRITEFLOW_MEMBER_IMAGES={
  'workflow-producer':'static/assets/writeflow/member-workflow-producer.png',
  'writing-executor':'static/assets/writeflow/member-writing-executor.png',
  'article-illustrator':'static/assets/writeflow/team-content-creator.png',
  'editor-review':'static/assets/writeflow/member-editor-review.png',
  'research-expert':'static/assets/writeflow/member-research-expert.png',
...
function _writeflowStatusCardFromRun(run,data){
  ...
  const visualMembers=members.map(member=>({
    id:member.id||member.skill||member.name||'',
    name:member.name||member.id||'',
    role:member.role||'',
    skill:member.skill||'',
    status:member.status||'待命',
    image:member.image||STATUS_CARD_WRITEFLOW_MEMBER_IMAGES[member.id]||STATUS_CARD_WRITEFLOW_MEMBER_IMAGES[member.skill]||'',
  }));
  const visualTasks=tasks.map(task=>({
    id:task.id||'',
    title:task.title||task.id||'任务',
    worker_id:task.worker_id||'',
    worker_name:task.worker_name||'',
    status:task.status||'pending',
    status_label:task.status_label||task.status||'待执行',
    description:task.description||'',
    artifacts:Array.isArray(task.artifacts)?task.artifacts:[],
  }));
  const visualArtifacts=artifacts.map(item=>({
    id:item.id||'',
    label:item.label||item.path||item.id||'产物',
    kind:item.kind||item.change_type||'file',
    path:item.path||'',
    status:item.status||'',
  ...
  const visualReferenceArtifacts=referenceArtifacts.map(item=>({
    id:item.id||'',
    label:item.label||item.path||item.id||'参考材料',
    kind:item.kind||item.change_type||'file',
    path:item.path||'',
    status:item.status||'',
    placeholder:!!item.placeholder,
...
function _statusCardStateClass(status){
  ...
  if(text.includes('确认')||text.includes('等待')||text.includes('落盘')||text==='waiting_user'||text==='waiting')return 'waiting';
  if(text.includes('需要')||text.includes('失败')||text==='error'||text==='blocked'||text==='failed')return 'issue';
  if(text.includes('执行')||text.includes('处理')||text.includes('写作')||text==='running'||text==='in_progress'||text==='working')return 'running';
  return 'idle';
}

function _statusCardAvatarHtml(src,label,cls){
  const fallback=String(label||'?').trim().slice(0,1).toUpperCase()||'?';
  const klass=cls||'status-card-avatar';
  if(!src){
    return `<span class="${klass}"><span class="status-card-avatar-fallback">${esc(fallback)}</span></span>`;
  }
  return `<span class="${klass}"><img src="${esc(src)}" alt="${esc(label||'')}" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='inline-flex'"><span class="status-card-avatar-fallback">${esc(fallback)}</span></span>`;
}
...
function _isExpertTeamStatusCard(card){
  return !!(card&&card.kind==='expert_team');
}

function _expertTeamQuestionIsTerminal(question){
  return ['answered','skipped'].includes(String(question&&question.status||'').toLowerCase());
}

function _expertTeamPendingQuestions(card){
  const questions=_expertTeamActiveQuestionSet(card);
  return questions.filter(question=>!_expertTeamQuestionIsTerminal(question));
}
...
function _expertTeamActiveQuestionSet(card){
  ...
  const primary=card.primaryConfirmation||{};
  if(String(primary.kind||'')==='question'&&Array.isArray(primary.fields)&&primary.fields.length){
    const primaryId=String(primary.fields[0]&&primary.fields[0].id||'');
    const primaryQuestion=questions.find(question=>String(question&&question.id||'')===primaryId);
...
function _expertTeamPendingConfirmations(card){
  ...
  const terminal=new Set(['answered','skipped','done','approved','cancelled','canceled','dismissed']);
...
function _expertTeamConfirmationSummary(card){
  ...
  const primary=(card&&card.primaryConfirmation&&card.primaryConfirmation.id)?card.primaryConfirmation:(confirmations[0]||{});
  const first=primary||{};
  ...
  if(String(primary.kind||'')==='question'){
    const fields=Array.isArray(primary.fields)?primary.fields:[];
    const isOptional=fields.some(field=>field&&field.required===false);
    return {
      count:1,
      title:isOptional?'可选补充待处理':'必填需求待确认',
      detail:isOptional?'生成尚未开始，可补充后生成，也可以跳过。':'请先补充必填需求，专家团再继续推进。',
      action:'去确认',
    };
  }
  if(String(primary.kind||'')==='stage_review'){
    return {
      count:1,
      title:'初稿已生成，等待阶段复核',
      detail:'请查看初稿，可无修改进入下一阶段，或提交修改意见。',
      action:'去复核',
    };
  }
  return {
    count:1,
    title:first.title||'待你确认：1 项',
...
function _expertTeamAnsweredQuestionsSummary(card){
  const questions=_expertTeamActiveQuestionSet(card);
  const answered=questions.filter(question=>_expertTeamQuestionIsTerminal(question));
  return {
    count:answered.length,
    total:questions.length,
    title:answered.length?`已完成 ${answered.length}/${questions.length||answered.length} 项需求`:'暂无已确认需求',
    detail:answered.slice(0,3).map(question=>`${question.title||question.id||'问题'}：${String(question.status||'')==='skipped'?'已跳过':(question.answer||'已确认')}`).join(' / '),
  };
}
...
function _expertTeamDockSummary(card){
  ...
  return {
    state:stateClass==='issue'?'issue':'running',
    title:runningLabel,
    detail:`${phase} · 可继续输入补充要求`,
    action:'查看详情',
  };
}

function _expertTeamStateLabel(state,fallback){
  if(state==='done')return '完成';
  if(state==='waiting')return '待确认';
  if(state==='issue')return '异常';
  if(state==='running')return '进行中';
  return fallback||'待执行';
}

function _expertTeamStatusBadgeLabel(card,state){
  if(state==='done')return '已完成';
  if(state==='waiting')return '待处理';
  if(state==='issue')return '需关注';
  if(state==='running')return '执行中';
  return card&&card.statusLabel||card&&card.status||'待执行';
}

function _expertTeamMemberAvatarsHtml(members){
  members=Array.isArray(members)?members:[];
  return members.slice(0,4).map(member=>{
    const image=member&&member.image||STATUS_CARD_WRITEFLOW_MEMBER_IMAGES[member&&member.id]||STATUS_CARD_WRITEFLOW_MEMBER_IMAGES[member&&member.skill]||'';
    const name=member&&member.name||member&&member.id||'成员';
    return `<span class="expert-team-panel-member-avatar" title="${esc(name)}">${_statusCardAvatarHtml(image,name,'status-card-writeflow-mini-avatar')}</span>`;
  }).join('');
}

function _expertTeamExecutionRows(card,context){
  card=card||{};
  context=context||{};
  const phaseList=Array.isArray(context.phaseList)?context.phaseList:[];
  ...
  const done=Number(context.done||0);
  ...
  return phaseList.map((label,idx)=>{
    ...
    const isDeliveryPhase=phaseText.includes('交付');
    const exactTask=tasks.find(item=>{
    ...
    if(!member){
      member=visibleMembers.find(item=>item&&item.id&&!usedMemberIds.has(item.id))||visibleMembers[idx%Math.max(1,visibleMembers.length)]||null;
    }
    if(member&&member.id)usedMemberIds.add(member.id);
    let title=task&&task.title||phaseText||'执行阶段';
    let detail=task&&task.result_summary||task&&task.description||'等待推进';
    let state=task?_statusCardStateClass(task.status_label||task.status):'idle';
    let status=task?_expertTeamStateLabel(state,task.status_label||task.status):'待执行';
    if(isConfirmPhase){
      state=pending.length?'waiting':'done';
      title='需求确认';
      detail=pending.length?`${pending.length} 项信息待确认`:(answeredSummary.detail||'需求信息已确认');
      status=pending.length?'待确认':'完成';
    }else if(isDeliveryPhase){
      const delivered=readyArtifacts.length>0||overallState==='done'||(total>0&&done>=total);
      state=delivered?'done':(overallState==='issue'?'issue':'waiting');
      title='交付整理';
      detail=readyArtifacts.length?`产物已生成，${readyArtifacts.length} 个可打开`:(delivered?'结果已写入当前对话':'等待产物生成');
      status=delivered?'完成':_expertTeamStateLabel(state);
    }else if(task){
      detail=task.result_summary||task.description||`${task.worker_name||task.worker_id||'专家'}处理中`;
      status=task.status_label||_expertTeamStateLabel(state,task.status||'');
    }else if(idx===phaseList.length-1){
      const delivered=readyArtifacts.length>0||overallState==='done';
      state=delivered?'done':'waiting';
      title='交付整理';
      detail=delivered?'结果已写入当前对话':'等待产物生成';
      status=delivered?'完成':'待交付';
    }
    const role=(task&&task.worker_name)||(member&&(member.role||member.name))||(members.length?`${members.length} 位专家协作`:'专家团');
    return {
      index:idx+1,
      phase:phaseText,
      title,
...
function _expertTeamQuestionDomId(question,runId){
  const raw=`expert-team-question-${runId||'run'}-${question&&question.id||'question'}`;
  return raw.replace(/[^a-zA-Z0-9_-]+/g,'-');
}

function _expertTeamQuestionAriaLabel(question,context){
  context=context||{};
  const idx=Number(context.index||1);
  const total=Number(context.total||1);
  return `需求确认第 ${idx}/${total} 项：${question&&question.title||question&&question.id||'问题'}`;
}

function _syncExpertTeamQuestionInputs(root){
  if(typeof document==='undefined')return false;
  root=(root&&root.closest&&(
    root.closest('.status-card-writeflow')||
    root.closest('#writeflowStatusDock')||
    root.closest('.expert-team-workspace-panel')
  ))||root||document;
  if(!root.querySelectorAll)return false;
  root.querySelectorAll('.status-card-expert-question.pending').forEach(questionEl=>{
    const input=questionEl.querySelector('[data-expert-team-answer-input]');
    const button=questionEl.querySelector('.status-card-expert-question-submit');
    if(!button)return;
    const value=input?String(input.value||'').trim():String(button.dataset.expertTeamAnswer||'').trim();
    const filled=!!value||button.dataset.expertTeamAllowEmpty==='1';
    const nextLabel=filled?(button.dataset.expertTeamReadyLabel||'确认此项并继续'):(button.dataset.expertTeamEmptyLabel||'请先填写');
    questionEl.classList.toggle('is-filled',filled);
    questionEl.classList.toggle('is-empty',!filled);
    button.disabled=!filled;
    button.textContent=nextLabel;
    if(button.dataset.expertTeamAriaBase)button.setAttribute('aria-label',`${button.dataset.expertTeamAriaBase}，${nextLabel}`);
    if(button.getAttribute('aria-busy')==='true')button.textContent='提交中...';
  });
  return true;
}

let _expertTeamDraftFocusKey='';

function _rememberExpertTeamDraftFocus(target){
  if(typeof document==='undefined')return '';
  const active=document.activeElement;
  const input=(active&&active.matches&&active.matches('[data-expert-team-answer-input]'))
    ? active
    : (target&&target.matches&&target.matches('[data-expert-team-answer-input]')?target:null);
  const questionEl=input&&input.closest?input.closest('.status-card-expert-question'):null;
  const key=questionEl&&input?_expertTeamQuestionInputKey(questionEl,input):'';
  if(key&&key!=='::')_expertTeamDraftFocusKey=key;
  return _expertTeamDraftFocusKey;
}

function _restoreExpertTeamDraftFocusIfNeeded(dock){
  if(typeof document==='undefined'||!dock||!dock.querySelector)return false;
  const popover=dock.querySelector('.expert-team-question-popover:not([hidden])');
  if(!popover)return false;
  const active=document.activeElement;
  if(active&&popover.contains&&popover.contains(active)&&active.matches&&active.matches('[data-expert-team-answer-input],[data-expert-team-stage-feedback],textarea,input'))return true;
  const activeIsPopoverClose=!!(active&&active.classList&&active.classList.contains('expert-team-question-close'));
  if(active&&popover.contains&&popover.contains(active)&&!activeIsPopoverClose)return true;
  if(active&&active!==document.body&&active!==document.documentElement&&dock.contains&&!dock.contains(active))return false;
  const inputs=Array.from(popover.querySelectorAll('[data-expert-team-answer-input]'));
  if(!inputs.length)return false;
  let target=null;
  for(const input of inputs){
    const questionEl=input.closest?input.closest('.status-card-expert-question'):null;
    const key=questionEl?_expertTeamQuestionInputKey(questionEl,input):'';
    if(key&&key===_expertTeamDraftFocusKey){
      target=input;
      break;
    }
  }
  if(!target)target=inputs.find(input=>String(input.value||'').trim())||inputs[0];
...
function syncExpertTeamQuestionInputState(target){
  _rememberExpertTeamDraftFocus(target);
  return _syncExpertTeamQuestionInputs(target);
}

function _expertTeamQuestionHtml(question,runId,context){
  question=question||{};
  context=context||{};
  ...
  const idx=Number(context.index||1);
  const total=Number(context.total||1);
  const remaining=Number(context.remaining||1);
  const required=question.required!==false;
  const initialAnswer=String(question.answer||'');
  const hasInitialAnswer=!!initialAnswer.trim()||!required;
  const inputId=_expertTeamQuestionDomId(question,runId);
  const ariaLabel=_expertTeamQuestionAriaLabel(question,{index:idx,total});
  const stepHtml=`<span class="status-card-expert-question-step"><b>第 ${idx} 项</b><small>${remaining>1?`还剩 ${remaining} 项`:'最后一项'}</small></span>`;
  const showControls=!answered&&current;
  const optionHtml=(options.length&&(answered||showControls))?`<span class="status-card-expert-question-options">${options.map(option=>{
    return answered
      ? `<em>${esc(option)}</em>`
      : `<button class="status-card-expert-question-option" type="button" data-expert-team-run-id="${esc(runId||'')}" data-expert-team-question-id="${esc(qid)}" data-expert-team-answer="${esc(option)}" data-expert-team-required="${required?'1':'0'}" onclick="answerExpertTeamQuestion(this);event.stopPropagation()">选择“${esc(option)}”并继续</button>`;
  }).join('')}</span>`:'';
  const inputHtml=(!answered&&isTextQuestion&&showControls)?`<span class="status-card-expert-question-input">
    <textarea id="${esc(inputId)}" rows="3" data-expert-team-answer-input="1" aria-label="${esc(_expertTeamQuestionAriaLabel(question,context))}" placeholder="${esc(required?'请填写确认内容，填写后点击下方按钮继续':'可选补充，填写后点击下方按钮继续')}" oninput="syncExpertTeamQuestionInputState(this)">${esc(initialAnswer)}</textarea>
    <button class="status-card-expert-question-submit" type="button" data-expert-team-run-id="${esc(runId||'')}" data-expert-team-question-id="${esc(qid)}" data-expert-team-required="${required?'1':'0'}" data-expert-team-empty-label="请先填写" data-expert-team-ready-label="确认此项并继续" data-expert-team-aria-base="${esc(ariaLabel)}" aria-label="${esc(`${ariaLabel}，${hasInitialAnswer?'确认此项并继续':'请先填写'}`)}" ${hasInitialAnswer?'':'disabled'} onclick="answerExpertTeamQuestion(this);event.stopPropagation()">${hasInitialAnswer?'确认此项并继续':'请先填写'}</button>
    <small class="status-card-expert-question-hint">${required?'填写后点击“确认此项并继续”，专家团才会进入下一步。':'可以留空确认，也可以补充说明后继续。'}</small>
  </span>`:'';
  return `<span class="status-card-expert-question ${answered?'answered':'pending'} ${current?'is-current':'is-queued'} ${hasInitialAnswer?'is-filled':'is-empty'}" data-expert-team-run-id="${esc(runId||'')}" data-expert-team-question-id="${esc(qid)}">
    ${stepHtml}
    <label class="status-card-expert-question-label" ${isTextQuestion?`for="${esc(inputId)}"`:''}>
      <strong>${esc(question.title||qid||'问题')}</strong>
      <small>${answered?`已回复：${question.answer||'已确认'}`:(current?'请先完成这一项，确认后自动进入下一项。':'下一项会在确认后自动展开。')}</small>
    </label>
    ${optionHtml}
    ${inputHtml}
  </span>`;
...
function _expertTeamQuestionInputKey(questionEl,inputEl){
  const qid=questionEl&&questionEl.dataset?questionEl.dataset.expertTeamQuestionId:'';
  const runId=(questionEl&&questionEl.dataset&&questionEl.dataset.expertTeamRunId)
    ||(inputEl&&inputEl.dataset&&inputEl.dataset.expertTeamRunId)
    ||'';
  return `${runId}::${qid}`;
}

function _captureExpertTeamQuestionInputState(root){
  const state={activeKey:'',inputs:{}};
  if(!root||!root.querySelectorAll||typeof document==='undefined')return state;
  const active=document.activeElement;
  root.querySelectorAll('.status-card-expert-question').forEach(questionEl=>{
    const inputEl=questionEl&&questionEl.querySelector?questionEl.querySelector('[data-expert-team-answer-input]'):null;
    if(!inputEl)return;
    const key=_expertTeamQuestionInputKey(questionEl,inputEl);
    if(!key||key==='::')return;
    const isActive=active===inputEl;
    ...
    if(isActive)state.activeKey=key;
  });
  return state;
}

function _restoreExpertTeamQuestionInputState(root,state){
  if(!root||!root.querySelectorAll||!state||!state.inputs)return false;
  let focusTarget=null;
  let focusState=null;
  let restored=false;
  root.querySelectorAll('.status-card-expert-question').forEach(questionEl=>{
    if(questionEl&&questionEl.classList&&questionEl.classList.contains('answered'))return;
    const inputEl=questionEl&&questionEl.querySelector?questionEl.querySelector('[data-expert-team-answer-input]'):null;
    if(!inputEl)return;
    const key=_expertTeamQuestionInputKey(questionEl,inputEl);
    const saved=state.inputs[key];
    if(!saved)return;
    inputEl.value=String(saved.value||'');
    restored=true;
    if(saved.active||state.activeKey===key){
      focusTarget=inputEl;
      focusState=saved;
    }
  });
  ...
  if(focusTarget&&typeof focusTarget.focus==='function'){
    ...
    const end=focusState&&typeof focusState.selectionEnd==='number'?focusState.selectionEnd:start;
    if(start!==null&&typeof focusTarget.setSelectionRange==='function'){
      try{focusTarget.setSelectionRange(start,end);}catch(_){}
    }
  }
  _syncExpertTeamQuestionInputs(root);
  return restored;
}
...
function _expertTeamWorkspaceRenderKey(card){
  ...
  const compactItem=item=>({
    ...
    name:item&&item.name||'',
    role:item&&item.role||'',
    workerId:item&&item.worker_id||'',
    workerName:item&&item.worker_name||'',
    title:item&&item.title||'',
    label:item&&item.label||'',
    status:item&&(item.status_label||item.status)||'',
  ...
  const compact={
    runId:card.runId||card.sessionId||'',
    sourceSessionId:card.sourceSessionId||card.source_session_id||'',
    title:card.title||'',
    ...
    questions:(Array.isArray(card.questions)?card.questions:[]).map(question=>({
    ...
    referenceArtifacts:(Array.isArray(card.referenceArtifacts)?card.referenceArtifacts:[]).map(compactItem),
    stageReview:card.stageReview||{},
    stageOutputs:(Array.isArray(card.stageOutputs)?card.stageOutputs:[]).map(output=>({
      id:output&&output.id||'',
      taskId:output&&output.task_id||'',
      title:output&&output.title||output&&output.label||'',
      status:output&&output.status||'',
      content:output&&output.content||'',
      summary:output&&output.summary||'',
      preview:output&&output.preview||'',
      contentLength:output&&output.content_length||output&&output.contentLength||0,
      hasLongContent:!!(output&&output.has_long_content||output&&output.hasLongContent),
      note:output&&output.note||'',
      revisionCount:output&&output.revision_count||0,
    })),
  };
  try{return JSON.stringify(compact);}catch(_){return `${compact.runId}:${compact.status}:${compact.phase}`;}
}
...
function _expertTeamDockMiniHtml(card){
  ...
  return `<button class="status-card-writeflow-mini status-card-expert-dock-summary ${esc(summary.state||'running')}" type="button" aria-label="${esc(`专家团：${actionLabel}，${summary.title}`)}" data-expert-team-run-id="${esc(runId)}" data-expert-team-primary-artifact-path="${esc(primaryArtifact&&primaryArtifact.path||'')}" data-writeflow-artifact-path="${esc(primaryArtifact&&primaryArtifact.path||'')}" data-writeflow-artifact-download="${esc(primaryArtifact&&primaryArtifact.download_name||'')}" onclick="handleExpertTeamDockAction(this);event.stopPropagation()">
    ...
    <span class="status-card-expert-dock-action">${esc(actionLabel)} ${actionIcon}</span>
  </button>`;
}
...
function _expertTeamLifecycleCardHtml(card){
  ...
  const confirmations=_expertTeamPendingConfirmations(card);
  const stateClass=_statusCardStateClass(card&&card.statusLabel||card&&card.status);
  if(!confirmations.length&&stateClass!=='running')return '';
  const title=confirmations.length?'专家团已创建，等待需求确认':'专家团正在生成';
  const detail=confirmations.length?'请在右侧专家团工作台完成需求确认，确认窗口会保留你的输入。':'后台正在生成阶段成果，完成后会在右侧专家团工作台提示复核。';
  return `<div class="expert-team-lifecycle-card" data-expert-team-lifecycle="1" data-expert-team-run-id="${esc(card&&card.runId||card&&card.sessionId||'')}">
    <span class="expert-team-lifecycle-dot"></span>
    <span class="expert-team-lifecycle-copy">
...
function renderExpertTeamLifecycleNotice(card){
  ...
  inner.querySelectorAll('.expert-team-lifecycle-card').forEach(node=>node.remove());
  return false;
}
...
function _expertTeamQuestionWizardState(card){
  ...
  const pending=questions.filter(question=>!_expertTeamQuestionIsTerminal(question));
  const answered=questions.filter(question=>_expertTeamQuestionIsTerminal(question));
  const requiredQuestions=questions.filter(question=>question&&question.required!==false);
  const optionalQuestions=questions.filter(question=>question&&question.required===false);
...
function _expertTeamQuestionPopoverProgressHtml(state){
  ...
  return `<span class="expert-team-question-progress" aria-label="需求确认进度">${questions.map((question,idx)=>{
    const answered=_expertTeamQuestionIsTerminal(question);
    const current=String(question&&question.id||'')===String(currentId||'');
    const cls=answered?'done':(current?'current':'todo');
    const label=String(question&&question.status||'')==='skipped'?'已跳过':(answered?'已完成':(current?'当前':'待确认'));
...
function _expertTeamQuestionPopoverReviewHtml(state){
  ...
  return `<div class="expert-team-question-review">
    ...
    ${answered.map((question,idx)=>`<button type="button" data-expert-team-question-id="${esc(question&&question.id||'')}" onclick="editExpertTeamAnsweredQuestion(this);event.stopPropagation()">
      <b>${idx+1}</b>
      <span><strong>${esc(question&&question.title||question&&question.id||'问题')}</strong><small>${esc(String(question&&question.status||'')==='skipped'?'已跳过':(question&&question.answer||'已确认'))}</small></span>
    </button>`).join('')}
  </div>`;
}
    ...
    <div class="expert-team-question-popover-body">
      <article class="status-card-expert-question pending is-current ${hasInitialAnswer?'is-filled':'is-empty'}" data-expert-team-run-id="${esc(runId)}" data-expert-team-question-id="${esc(qid)}">
        ...
        <div class="expert-team-question-main">
          <strong>${esc(question.title||qid||'需求确认')}</strong>
...
function _focusExpertTeamQuestionPopover(trigger){
  ...
  const focusSelectors=[
    '.status-card-expert-question.pending.is-current [data-expert-team-answer-input]:not(:disabled)',
    '[data-expert-team-answer-input]:not(:disabled)',
    '.expert-team-question-primary:not(:disabled)',
    'button:not(.expert-team-question-close):not(:disabled)',
    '.expert-team-question-close:not(:disabled)',
  ];
  let target=null;
...
function goExpertTeamQuestionStep(trigger,delta){
  ...
  if(typeof renderExpertTeamStatusSurface==='function')renderExpertTeamStatusSurface(_activeExpertTeamStatusCard);
...
function _expertTeamWorkspacePanelHtml(card){
  ...
  const questions=Array.isArray(card.questions)?card.questions:[];
  const activeQuestions=_expertTeamActiveQuestionSet(card);
  const reviewItems=Array.isArray(card.reviewItems)?card.reviewItems:[];
  const readyArtifacts=_expertTeamReadyArtifacts(card);
  const deliveredArtifacts=artifacts.filter(item=>_expertTeamArtifactDeliveredToChat(item));
  ...
  const expertTeamMemberCount=members.length;
  const memberAvatars=_expertTeamMemberAvatarsHtml(members);
  const executionRows=_expertTeamExecutionRows(card,{phaseList,pending,readyArtifacts,done,total,stateClass});
  ...
  const answeredCount=activeQuestions.filter(question=>_expertTeamQuestionIsTerminal(question)).length;
  ...
  const questionSummaryHtml=pending.length
    ...
    : (activeQuestions.length?`<span class="expert-team-panel-answered-summary">
      ...
      </span>`:'<span class="expert-team-panel-empty">暂无待确认需求</span>');
  ...
  const stageReviewFallback=isFinalStageReview?'最终成果已写入当前对话，请检查后确认是否完成任务。':'阶段结果已写入当前对话，请检查后确认是否进入下一阶段。';
  const stageReviewContent=stageOutput.summary||stageOutput.preview||stageOutput.note||stageReviewFallback;
  ...
  const stageApproveLabel=isFinalStageReview?'无修改，完成任务':'无修改，进入下一阶段';
  const stageApproveBusyLabel=isFinalStageReview?'正在完成任务...':'正在进入下一阶段...';
  const stageLocateLabel=isFinalStageReview?'查看成果':'查看初稿';
  ...
  const stageReviewSectionHtml=(stageReviewActionable||stageReviewDisplayState==='history')&&(stageReview.task_id||stageOutput.task_id)?`<section class="expert-team-panel-section expert-team-stage-review ${stageReviewActionable?'awaiting-review':'is-history'}" aria-label="${stageReviewActionable?'请确认阶段成果':'阶段成果摘要'}">
      ...
      <div class="expert-team-stage-output">
        <strong>${esc(stageReviewTitle||'阶段产物')}</strong>
        <p>${esc(stageReviewContent)}</p>
      </div>
      ${canApproveStage||canRequestRevision?`<div class="expert-team-stage-actions">
        <button class="expert-team-panel-action expert-team-stage-locate" type="button" data-expert-team-stage-output-locator="${esc(stageOutputLocator)}" data-expert-team-stage-output-artifact-id="${esc(stageOutputArtifactId)}" onclick="locateExpertTeamStageOutput(this);event.stopPropagation()">${esc(stageLocateLabel)}</button>
        ${canApproveStage?`<button class="expert-team-panel-action expert-team-stage-approve" type="button" data-expert-team-stage-approve-run-id="${esc(runId)}" data-expert-team-stage-approve-label="${esc(stageApproveLabel)}" data-expert-team-stage-approve-busy-label="${esc(stageApproveBusyLabel)}" onclick="approveExpertTeamStage(this);event.stopPropagation()">${esc(stageApproveLabel)}</button>`:''}
  ...
  const executionHtml=executionRows.length?executionRows.map(row=>{
    return `<span class="expert-team-panel-execution-row ${esc(row.state||'idle')}">
      <i>${esc(row.index)}</i>
      <span class="expert-team-panel-execution-main"><strong>${esc(row.title||row.phase||'执行阶段')}</strong><small>${esc(row.detail||'')}</small></span>
      <span class="expert-team-panel-execution-role">${esc(row.role||'专家团')}</span>
      <b>${esc(row.status||_expertTeamStateLabel(row.state))}</b>
    </span>`;
  }).join(''):'<span class="expert-team-panel-empty">等待执行明细生成</span>';
  const referenceHtml=referenceArtifacts.length?`<section class="expert-team-panel-section">
  ...
  return `<div class="expert-team-panel-inner" data-expert-team-run-id="${esc(runId)}" onclick="handleExpertTeamPanelBlankCollapse(event)">
    <header class="expert-team-panel-head">
      <div class="expert-team-panel-topbar">
        ...
        <span class="expert-team-panel-status ${esc(summary.state||stateClass)}">${esc(_expertTeamStatusBadgeLabel(card,summary.state||stateClass))}</span>
      </div>
      <div class="expert-team-panel-overview">
        <span class="expert-team-panel-copy">
          <strong class="expert-team-panel-title" title="${esc(taskTitle)}">${esc(taskTitle)}</strong>
    ...
    <div class="expert-team-panel-expanded-body">
      ...
      <section class="expert-team-panel-priority-grid" aria-label="专家团重点信息">
        <span class="expert-team-panel-priority-card progress ${esc(stateClass)}">
          <strong>当前进度</strong>
          <b>${esc(phase)}</b>
        ...
        <span class="expert-team-panel-priority-card todo ${pending.length||needsResume||canApproveStage||canRequestRevision?'waiting':'done'}">
          <strong>待你处理</strong>
          <b>${esc(pendingAction)}</b>
          <small>${esc(pendingDetail)}</small>
          ${pendingButton}
        </span>
      </section>
      <section class="expert-team-panel-section expert-team-panel-execution">
        <div class="expert-team-panel-section-title">
          <span>执行明细</span>
          <small class="expert-team-panel-member-avatars">${memberAvatars}<b>${expertTeamMemberCount} 位专家协作</b></small>
        </div>
...
function mountExpertTeamWorkspacePanel(card){
  ...
  const messagesShell=mainChat&&mainChat.querySelector?mainChat.querySelector('.messages-shell'):null;
  if(!mainChat||!messagesShell)return false;
...
function _syncExpertTeamWorkspacePanelVisibility(){
  ...
  const panel=typeof document!=='undefined'?document.getElementById('expertTeamWorkspacePanel'):null;
  const runId=_expertTeamWorkspacePanelRunId(panel);
  const sourceSid=(panel&&panel.dataset&&panel.dataset.expertTeamSourceSessionId)||(_activeExpertTeamStatusCard&&_activeExpertTeamStatusCard.sourceSessionId)||'';
  ...
  const hasCurrentExpert=!!(isCurrentSession&&(runId||_activeExpertTeamStatusCard));
  const isChat=_expertTeamActivePanelName()==='chat';
  const visible=!!(hasCurrentExpert&&isChat);
  const collapsed=visible&&_expertTeamWorkspacePanelHiddenForRun(runId);
  if(shell){
    shell.classList.toggle('taiji-expert-team-active',visible);
    shell.classList.toggle('taiji-expert-team-panel-visible',visible);
    shell.classList.toggle('taiji-expert-team-panel-collapsed',collapsed);
    shell.classList.remove('taiji-expert-team-panel-hidden');
  }
  if(panel){
...
function toggleExpertTeamWorkspacePanel(btn){
  const panel=typeof document!=='undefined'?document.getElementById('expertTeamWorkspacePanel'):null;
  const runId=_expertTeamWorkspacePanelRunId(panel)||(btn&&btn.closest&&btn.closest('[data-expert-team-run-id]')&&btn.closest('[data-expert-team-run-id]').dataset.expertTeamRunId)||'';
  const hidden=_expertTeamWorkspacePanelHiddenForRun(runId);
  _setExpertTeamWorkspacePanelHiddenForRun(runId,!hidden);
  _syncExpertTeamWorkspacePanelVisibility();
  if(hidden&&panel&&panel.focus){
...
function shouldPreserveExpertTeamDraftDock(sessionId){
  const root=document.getElementById('expertTeamWorkspacePanel')||((typeof $==='function'&&$('writeflowStatusDock'))||document.getElementById('writeflowStatusDock'));
  if(!root||root.hidden)return false;
  const sourceSid=String((root.dataset&&root.dataset.expertTeamSourceSessionId)||(_activeExpertTeamStatusCard&&_activeExpertTeamStatusCard.sourceSessionId)||'');
  const targetSid=String(sessionId||'');
  if(targetSid&&sourceSid&&sourceSid!==targetSid)return false;
  const active=typeof document!=='undefined'?document.activeElement:null;
  const selectors=[
    '.status-card-expert-question.pending [data-expert-team-answer-input]',
    '.expert-team-question-popover [data-expert-team-answer-input]',
    '[data-expert-team-stage-feedback]',
  ];
  const inputs=Array.from(root.querySelectorAll(selectors.join(',')));
  const shouldPreserve=inputs.some(input=>input===active||String(input.value||'').trim());
  if(shouldPreserve&&typeof _restoreExpertTeamDraftFocusIfNeeded==='function')_restoreExpertTeamDraftFocusIfNeeded(root);
...
function focusExpertTeamWorkspacePanel(trigger){
  ...
  const panel=typeof document!=='undefined'?document.getElementById('expertTeamWorkspacePanel'):null;
  if(!panel)return focusExpertTeamBottomDock(trigger);
  try{panel.scrollIntoView({block:'nearest',inline:'nearest'});}catch(_){}
  const focusTarget=panel.querySelector&&(
    panel.querySelector('.expert-team-stage-review button:not(:disabled)')||
    panel.querySelector('.expert-team-panel-action:not(:disabled)')||
    panel.querySelector('.expert-team-panel-collapse-toggle')
  );
  if(focusTarget&&focusTarget.focus){
    try{focusTarget.focus({preventScroll:true});}catch(_){focusTarget.focus();}
  }
...
function openExpertTeamReviewPanel(trigger){
  showExpertTeamWorkspacePanel(trigger);
  const panel=typeof document!=='undefined'?document.getElementById('expertTeamWorkspacePanel'):null;
  const review=panel&&panel.querySelector?panel.querySelector('.expert-team-stage-review'):null;
  if(!review)return focusExpertTeamWorkspacePanel(trigger);
  panel.classList.add('is-review-open');
  try{review.scrollIntoView({block:'center',inline:'nearest'});}catch(_){}
  const focusTarget=review.querySelector('.expert-team-stage-locate:not(:disabled), .expert-team-stage-approve:not(:disabled), .expert-team-stage-revision-toggle:not(:disabled), button:not(:disabled)');
  if(focusTarget&&focusTarget.focus){
    try{focusTarget.focus({preventScroll:true});}catch(_){focusTarget.focus();}
  }
  return true;
...
function toggleWriteflowStatusCard(btn){
  const card=btn&&btn.closest&&btn.closest('.status-card-writeflow');
  if(!card)return;
...
if(typeof window!=='undefined')window.toggleWriteflowStatusCard=toggleWriteflowStatusCard;

async function openWriteflowArtifact(btn){
  ...
  try{
    if(typeof openArtifactPath==='function'){
      const opened=await openArtifactPath(path);
      if(opened===false&&typeof showToast==='function')showToast('无法打开产物：文件不存在或无法访问');
      return opened!==false;
    }
    if(typeof openFile==='function'){
      await openFile(path,{bustCache:true});
      return true;
    }
  }catch(_){
    if(typeof showToast==='function')showToast('无法打开产物：文件不存在或无法访问');
    return false;
  }
...
function openExpertTeamChatDelivery(btn){
  if(typeof openExpertTeamResultViewer==='function'&&openExpertTeamResultViewer(btn))return true;
...
function _expertTeamCompactSummary(text,limit=140){
  ...
  const first=raw.split('\n').map(line=>line.replace(/^\s*(?:#{1,6}\s*|[-*]\s*|\d+[\.、．)]\s*)/,'').trim()).find(Boolean)||raw;
  const compact=first.replace(/\s+/g,' ').trim();
  return compact.length>limit?`${compact.slice(0,limit).trim()}...`:compact;
}
function _expertTeamCompactPreview(text,limit=360){
  const compact=_expertTeamPlainText(text).replace(/\s+/g,' ').trim();
  return compact.length>limit?`${compact.slice(0,limit).trim()}...`:compact;
}
function _expertTeamResultFromCard(){
  const card=_activeExpertTeamStatusCard||{};
  const stageReview=card.stageReview||{};
  ...
  if(typeof S!=='undefined'&&Array.isArray(S.messages)){
    for(let idx=S.messages.length-1;idx>=0;idx--){
      const messageContent=_expertTeamPlainText(msgContent(S.messages[idx])||String(S.messages[idx]&&S.messages[idx].content||''));
  ...
  if(rawIdx===''&&typeof document!=='undefined'){
    const cards=Array.from(document.querySelectorAll('.expert-team-result-card[data-expert-team-result-raw-idx]'));
    const latestCard=cards.reverse().find(card=>card&&card.dataset&&card.dataset.expertTeamResultRawIdx!=='');
    if(latestCard&&latestCard.dataset)rawIdx=latestCard.dataset.expertTeamResultRawIdx;
  }
  return {
    rawIdx,
    title:stageReview.title||output.title||output.label||latest.title||latest.label||'专家团成果',
    phase:stageReview.phase||output.phase||latest.phase||card.phase||'交付成果',
    summary:output.summary||_expertTeamCompactSummary(content),
    preview:output.preview||_expertTeamCompactPreview(content),
    content,
    contentLength:Number(output.content_length||content.length||0),
...
function legacyExpertDeliveryInfoDisabled(message,rawIdx){
  ...
  if(!content)return null;
  const hasExpertShape=/(阶段目标|阶段产物|待人工补充事项|交付后核对事项|下一阶段建议)/.test(content)
    && /(阶段产物|待人工补充事项|交付后核对事项|下一阶段建议)/.test(content);
  const card=_activeExpertTeamStatusCard||{};
  const outputs=Array.isArray(card.stageOutputs)?card.stageOutputs:[];
  const matched=outputs.find(item=>item&&String(item.content||'').trim()===content);
  const activeOutput=(card.stageReview&&card.stageReview.output)||{};
  const matchesActive=String(activeOutput.content||'').trim()===content;
  if(!matched&&!matchesActive&&!hasExpertShape)return null;
  if(content.length<420&&!matched&&!matchesActive)return null;
  const output=matched||activeOutput||{};
  return {
    rawIdx,
    title:output.title||output.label||'专家团交付成果',
    phase:output.phase||card.phase||'交付成果',
    summary:output.summary||_expertTeamCompactSummary(content),
    preview:output.preview||_expertTeamCompactPreview(content),
    content,
    contentLength:Number(output.content_length||content.length||0),
    updatedAt:output.updated_at||message.timestamp||message._ts||'',
  };
}
...
function _expertTeamDeliveryCardHtml(info){
  info=info||{};
  const rawIdx=info.rawIdx===''?'':String(info.rawIdx);
  const count=Number(info.contentLength||String(info.content||'').length||0);
  const meta=[info.phase||'专家团成果',count?`${count} 字`:'',info.updatedAt?String(info.updatedAt):''].filter(Boolean).join(' · ');
  const icon=(typeof li==='function')?li('file-text',16):'□';
  return `<div class="expert-team-result-card" data-expert-team-result-card="1" data-expert-team-result-raw-idx="${esc(rawIdx)}">
    <div class="expert-team-result-card-icon">${icon}</div>
    <div class="expert-team-result-card-main">
      <strong>${esc(info.title||'专家团交付成果')}</strong>
      <small>${esc(meta||'结果已写入当前对话')}</small>
      <p>${esc(info.summary||info.preview||'点击查看完整成果。')}</p>
      <div class="expert-team-result-card-actions">
        <button type="button" data-expert-team-result-raw-idx="${esc(rawIdx)}" onclick="openExpertTeamResultViewer(this);event.stopPropagation()">查看完整成果</button>
        ${rawIdx!==''?`<button type="button" data-expert-team-result-raw-idx="${esc(rawIdx)}" onclick="locateExpertTeamDeliveryMessage(this);event.stopPropagation()">定位原文</button>`:''}
      </div>
    </div>
  </div>`;
}
function _expertTeamResultFromTrigger(trigger){
...
function _expertTeamResultViewerElement(){
  if(typeof document==='undefined')return null;
  let viewer=document.getElementById('expertTeamResultViewer');
  if(viewer)return viewer;
  viewer=document.createElement('div');
  viewer.id='expertTeamResultViewer';
  viewer.className='expert-team-result-viewer';
  viewer.hidden=true;
...
function openExpertTeamResultViewer(trigger){
  const info=_expertTeamResultFromTrigger(trigger);
  ...
  viewer.innerHTML=`<div class="expert-team-result-viewer-panel" role="dialog" aria-modal="true" aria-label="专家团完整成果预览" onkeydown="trapExpertTeamResultViewerKeydown(event)">
    <div class="expert-team-result-viewer-head">
      <span>
        ...
        <small>${esc([info.phase||'',info.contentLength?`${info.contentLength} 字`:''].filter(Boolean).join(' · ')||'完整成果预览')}</small>
      </span>
      <button type="button" onclick="closeExpertTeamResultViewer(this);event.stopPropagation()" aria-label="关闭专家团成果预览">×</button>
    </div>
    <div class="expert-team-result-viewer-content">${rendered}</div>
    <div class="expert-team-result-viewer-actions">
      ${info.rawIdx!==''?`<button type="button" data-expert-team-result-raw-idx="${esc(String(info.rawIdx))}" onclick="locateExpertTeamDeliveryMessage(this);event.stopPropagation()">定位原文</button>`:''}
      <button type="button" onclick="closeExpertTeamResultViewer(this);event.stopPropagation()">关闭</button>
    </div>
  </div>`;
  viewer.hidden=false;
...
function locateExpertTeamDeliveryMessage(trigger){
  ...
  target.classList.add('expert-team-result-source-focus');
  setTimeout(()=>{try{target.classList.remove('expert-team-result-source-focus');}catch(_){}},1800);
  if(typeof showToast==='function')showToast('已定位到专家团原文。');
  return true;
}
...
async function answerExpertTeamQuestion(btn){
  ...
  const skipOptional=!!(btn&&btn.dataset&&btn.dataset.expertTeamSkipOptional==='1');
  const questionEl=btn&&btn.closest?btn.closest('.status-card-expert-question'):null;
  const submitBtn=btn&&btn.matches&&btn.matches('.status-card-expert-question-submit')?btn:null;
  let answer=btn&&btn.dataset?btn.dataset.expertTeamAnswer:'';
  let input=null;
  if(!answer&&questionEl){
    input=questionEl.querySelector('[data-expert-team-answer-input]');
    answer=input?String(input.value||'').trim():'';
  }
  const root=btn&&btn.closest?(btn.closest('.status-card-writeflow')||btn.closest('.expert-team-workspace-panel')):null;
  const runId=(btn&&btn.dataset&&btn.dataset.expertTeamRunId)
    ||(questionEl&&questionEl.dataset?questionEl.dataset.expertTeamRunId:'')
    ||(root&&root.dataset?root.dataset.expertTeamRunId:'')
    ||(root&&root.dataset?root.dataset.writeflowRunId:'');
  ...
  if(!answer&&!required&&!skipOptional){
    showToast('请填写补充内容，或点击跳过并开始生成。');
    if(input&&typeof input.focus==='function'){
      try{input.focus({preventScroll:true});}catch(_){input.focus();}
    }
    syncExpertTeamQuestionInputState(input||btn);
...
function _expertTeamRunIdFromStageButton(btn,kind){
  const data=btn&&btn.dataset?btn.dataset:{};
  return data[`expertTeamStage${kind}RunId`]
...
async function reviseExpertTeamStage(btn){
  const runId=_expertTeamRunIdFromStageButton(btn,'Revise');
  const sid=typeof S!=='undefined'&&S.session&&S.session.session_id||'';
  const section=btn&&btn.closest?btn.closest('.expert-team-stage-review'):null;
  const input=section&&section.querySelector?section.querySelector('[data-expert-team-stage-feedback]'):null;
...
async function submitExpertTeamStageRevision(btn){
  return reviseExpertTeamStage(btn);
}
if(typeof window!=='undefined'){
  ...
  window.submitExpertTeamStageRevision=submitExpertTeamStageRevision;
...
function _statusCardWriteflowHtml(card,copyBtn){
  ...
  const total=Math.max(0,Number(progress.total||tasks.length||0));
  const pct=total?Math.max(0,Math.min(100,Math.round(done*100/total))):0;
  const memberDone=members.filter(member=>_statusCardStateClass(member.status)==='done').length;
  const activeMembers=members.filter(member=>{
    const cls=_statusCardStateClass(member.status);
    return cls==='running'||cls==='waiting'||cls==='issue';
  });
  ...
  const miniHtml=isExpertTeam?_expertTeamDockMiniHtml(card):`<div class="status-card-writeflow-mini" aria-label="专家团当前运行状态">
  ...
  const memberHtml=members.length?members.map(member=>{
    ...
    return `<span class="status-card-writeflow-member ${cls}">
      ${_statusCardAvatarHtml(image,member.name,'status-card-writeflow-avatar')}
      <span class="status-card-writeflow-member-copy">
        <strong>${esc(member.name||member.id||'成员')}</strong>
        <small>${esc(member.role||member.skill||'')}</small>
      </span>
      <em>${esc(member.status||'待命')}</em>
    </span>`;
  }).join(''):'<span class="status-card-writeflow-empty">等待成员接单</span>';
  const taskHtml=tasks.length?tasks.map(task=>{
    const cls=_statusCardStateClass(task.status_label||task.status);
    const image=STATUS_CARD_WRITEFLOW_MEMBER_IMAGES[task.worker_id]||'';
    const artifactCount=Array.isArray(task.artifacts)?task.artifacts.length:0;
    return `<span class="status-card-writeflow-task ${cls}">
      ${_statusCardAvatarHtml(image,task.worker_name||task.worker_id,'status-card-task-avatar')}
      <span>
        <strong>${esc(task.title||task.id||'任务')}</strong>
        <small>${esc(task.worker_name||task.worker_id||'')} · ${esc(task.status_label||task.status||'待执行')}${artifactCount?` · ${artifactCount} 个产物`:''}</small>
      </span>
    </span>`;
  }).join(''):'<span class="status-card-writeflow-empty">等待任务拆解</span>';
  const questionHtml=questions.length?questions.map(question=>_expertTeamQuestionHtml(question,card.runId||card.sessionId||'')).join(''):'';
  ...
  const artifactItemHtml=(item,reference)=>{
    ...
    const actions=isReady?`<span class="status-card-writeflow-artifact-actions">
      <button type="button" data-writeflow-artifact-path="${esc(item.path||'')}" onclick="openWriteflowArtifact(this);event.stopPropagation()">打开</button>
      <button type="button" data-writeflow-artifact-path="${esc(item.path||'')}" data-writeflow-artifact-download="${esc(downloadName)}" onclick="downloadWriteflowArtifact(this);event.stopPropagation()">下载</button>
    </span>`:'';
    return `<span class="status-card-writeflow-artifact${cls}">
      <i>${esc(String(item.kind||'file').slice(0,2).toUpperCase())}</i>
      <button class="status-card-writeflow-artifact-main" type="button" ${isReady?`data-writeflow-artifact-path="${esc(item.path||'')}" onclick="openWriteflowArtifact(this);event.stopPropagation()"`:'disabled'}>
        <strong>${esc(item.label||item.path||item.id||'产物')}</strong>
        <small>${esc(meta||'待生成')}</small>
      </button>
      ${actions}
  ...
  return `<div class="status-card status-card-writeflow ${expanded?'is-expanded':'is-collapsed'}" data-status-card="1" data-status-card-kind="writeflow" data-writeflow-card-key="${esc(storageKey)}" data-expert-team-run-id="${esc(card.runId||card.sessionId||'')}">
    ...
    <div class="status-card-writeflow-body ${isExpertTeam?'status-card-expert-bottom-body':''}">
      ...
      <div class="status-card-writeflow-phases" aria-label="写作三步流程">${phaseHtml}</div>
      <div class="status-card-writeflow-detail">
        <div class="status-card-writeflow-section">
          <div class="status-card-writeflow-section-title"><span>成员状态</span><small>${esc(team.title||'专家团队')}</small></div>
          <div class="status-card-writeflow-members">${memberHtml}</div>
        </div>
        <div class="status-card-writeflow-section">
        ...
        ${referenceArtifacts.length?`<div class="status-card-writeflow-section">
...
function _statusCardHtml(card){
  ...
  const copyBtn=sessionId
    ...
    : '';
  if(card.type==='writeflow'||card.kind==='writeflow'||_isExpertTeamStatusCard(card)){
    return _statusCardWriteflowHtml(card,'');
  }
...
function _hideExpertTeamLegacyDock(){
  const dock=(typeof $==='function'&&$('writeflowStatusDock'))||document.getElementById('writeflowStatusDock');
  if(!dock)return false;
  dock.innerHTML='';
  dock.hidden=true;
...
function renderWriteflowStatusDock(card){
  const dock=(typeof $==='function'&&$('writeflowStatusDock'))||document.getElementById('writeflowStatusDock');
  if(!dock||!card){
  ...
  const isExpertTeam=_isExpertTeamStatusCard(card);
  if(isExpertTeam)return renderExpertTeamStatusSurface(card);
  const renderKey=isExpertTeam?_expertTeamWorkspaceRenderKey(card):'';
  const dockInputState=isExpertTeam?_captureExpertTeamQuestionInputState(dock):null;
  const canReuseExpertDock=isExpertTeam&&dock.dataset.expertTeamRenderKey===renderKey&&dock.querySelector('.status-card-writeflow');
  if(!canReuseExpertDock){
    dock.innerHTML=_statusCardHtml(card);
    if(isExpertTeam){
      dock.dataset.expertTeamRenderKey=renderKey;
      _restoreExpertTeamQuestionInputState(dock,dockInputState);
    }else{
      delete dock.dataset.expertTeamRenderKey;
    }
  }
...
if(typeof window!=='undefined'){
  window.renderExpertTeamStatusSurface=renderExpertTeamStatusSurface;
...
async function saveDashboardSettings(){
  ...
  try{
    ...
    if(statusEl) statusEl.textContent='仪表盘链接设置已保存。';
    await refreshDashboardStatus(true);
  }catch(err){
    if(statusEl) statusEl.textContent='仪表盘链接设置保存失败。';
    else if(typeof showToast==='function') showToast('仪表盘链接设置保存失败。');
  }
}
...
(function(){
  ...
  function _ptrCreateIndicator(){
    ...
    _indicator.innerHTML='<span class="ptr-icon">↓</span> <span class="ptr-text">下拉刷新</span>';
...
function _renderQueueChips(sid){
  ...
  if(q.length>1){
    ...
    mergeBtn.onclick=()=>{
      ...
      if(hasFiles){
        if(typeof showToast==='function') showToast('队列消息中的附件将被移除',2600,'warning');
      }
...
async function applyUpdates(){
  ...
  try{
    ...
    showToast('更新已应用 — 正在重启… restarting…');
...
async function forceUpdate(btn){
  ...
  if(!confirmed) return;
  btn.disabled=true;btn.textContent='正在强制更新…';
  const errEl=$('updateError');
  if(errEl){errEl.style.display='none';}
  try{
    const res=await api('/api/updates/force',{method:'POST',body:JSON.stringify({target}),timeoutMs:120000});
    if(!res.ok){
      if(errEl){errEl.textContent='强制更新失败：'+(res.message||'未知错误');errEl.style.display='block';}
...
function appendLiveToolCard(tc){
  ...
  if(waiting&&!toolDone) waiting.textContent='等待工具结果';
  // Update existing card in place (tool_complete after tool_start)
  if(tid){
...
function loadPdfInline(container){
  ...
  root.querySelectorAll('.pdf-preview-load:not([data-loaded])').forEach(el=>{
    ...
    if(_pdfjsReady){
      ...
      s.type='module';
      s.textContent=`
        import * as pdfjsLib from './static/vendor/pdfjs-dist/4.9.155/pdf.min.mjs';
        pdfjsLib.GlobalWorkerOptions.workerSrc='static/vendor/pdfjs-dist/4.9.155/pdf.worker.min.mjs';
