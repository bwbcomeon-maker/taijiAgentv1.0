...
async function loadSession(sid){
  ...
  if (currentSid !== sid || forceReload) {
    ...
    if (_msgInner && currentSid !== sid) _msgInner.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--text-muted);font-size:14px;padding:40px;text-align:center;">正在加载对话...</div>';
  }
  ...
  try {
    ...
    if(_msgInner){
      if(e.status===404){
        _msgInner.innerHTML='<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--text-muted);font-size:14px;padding:40px;text-align:center;">此会话在 Web UI 中不可用。</div>';
        ...
        _msgInner.innerHTML='<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--text-muted);font-size:14px;padding:40px;text-align:center;">加载会话失败。请尝试切换会话或刷新。</div>';
        if(typeof showToast==='function') showToast('加载会话失败',3000,'error');
      }
    }
    if (_loadingSessionId === sid) _loadingSessionId = null;
    return;
  ...
  if(INFLIGHT[sid]){
    ...
    await _hydrateWriteflowStatusCardForSession(sid);
    if (_loadingSessionId !== sid) return;
    S.busy=true;
    ...
    try {
      ...
      const _msgInner = $('msgInner');
      if (_msgInner) {
        _msgInner.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--text-muted);font-size:14px;padding:40px;text-align:center;">加载消息失败。请尝试切换会话或刷新。</div>';
      }
      if (typeof showToast === 'function') showToast('加载对话消息失败', 3000, 'error');
      if (_loadingSessionId === sid) _loadingSessionId = null;
      return;
    }
    ...
    if(activeStreamId){
      ...
      if(typeof attachLiveStream==='function') attachLiveStream(sid, activeStreamId, S.session.pending_attachments||[], {
        reconnecting:true,
        expertTeamProtocol:window.isExpertTeamExecutionDisplayMessage(S.session.pending_user_message),
      });
...
function _stopWriteflowStatusRefresh(){
  if(_writeflowStatusRefreshTimer){
    clearInterval(_writeflowStatusRefreshTimer);
...
function _scheduleWriteflowStatusRefresh(sid,run){
  if(!sid||!_writeflowRunIsActive(run)){
    if(_writeflowStatusRefreshSid===sid)_stopWriteflowStatusRefresh();
    return;
  }
  if(_writeflowStatusRefreshTimer&&_writeflowStatusRefreshSid===sid)return;
  _stopWriteflowStatusRefresh();
  _writeflowStatusRefreshSid=sid;
  _writeflowStatusRefreshTimer=setInterval(()=>{
    if(!S.session||S.session.session_id!==sid){
      _stopWriteflowStatusRefresh();
      return;
    }
    _hydrateWriteflowStatusCardForSession(sid,{silent:true});
...
async function _hydrateExpertTeamStatusCardForSession(sid,options={}){
  ...
  const hasLatestVersion=_expertTeamLatestAppliedVersionByRun.has(runId);
  ...
  if(hasLatestVersion&&runVersion===latestVersion){
    _scheduleWriteflowStatusRefresh(sid,run);
    return {status:'preserved',reason:'unchanged_version'};
  }
...
async function refreshWriteflowStatusDockForActiveSession(){
  ...
  if(!sid)return false;
  return _hydrateWriteflowStatusCardForSession(sid,{silent:true});
}

function _removeWriteflowStatusCardsFromMessages(){
  if(!Array.isArray(S.messages))return false;
  const before=S.messages.length;
  S.messages=S.messages.filter(m=>!(m&&m._statusCard&&(m._statusCard.type==='writeflow'||m._statusCard.kind==='writeflow')));
...
function _showBatchProjectPicker(){
  ...
  none.onclick=async()=>{picker.remove();
    try{await Promise.all(ids.map(sid=>api('/api/session/move',{method:'POST',body:JSON.stringify({session_id:sid,project_id:null})})));
      showToast('已从项目中移除');exitSessionSelectMode();await renderSessionList();
  ...
  for(const p of(_allProjects||[])){
    ...
    item.onclick=async()=>{picker.remove();
      try{await Promise.all(ids.map(sid=>api('/api/session/move',{method:'POST',body:JSON.stringify({session_id:sid,project_id:p.project_id})})));
        showToast('已移动到 '+p.name);exitSessionSelectMode();await renderSessionList();
      }catch(e){showToast('移动失败：'+(e.message||e));}
  ...
  try{
    ...
    if(resp.status === 503 || data.watcher_running === false){
      ...
      if(!_gatewaySSEWarningShown && typeof showToast === 'function'){
        showToast('Gateway 同步不可用 — 已回退到定时刷新。', 5000);
...
function renderSessionListFromCache(){
  ...
  function _renderOneSession(s, isPinnedGroup=false){
    ...
    const startRename=()=>{
    if(_isReadOnlySession(s)){ if(typeof showToast==='function') showToast('只读导入会话不能重命名。',3000); return; }
...
if(typeof window!=='undefined'){
  ...
  window.addEventListener('popstate', () => {
    ...
    if(S.busy){
      if(typeof showToast==='function') showToast('请先完成当前回合，再切换会话。',3000);
      return;
    }
...
async function deleteSession(sid, beforeDelete=null){
  ...
  if(S.session&&S.session.session_id===sid){
    ...
    if(remaining.sessions&&remaining.sessions.length){
      ...
      const _tm=$('topbarMeta');if(_tm)_tm.textContent='开始新对话';
      $('msgInner').innerHTML='';
