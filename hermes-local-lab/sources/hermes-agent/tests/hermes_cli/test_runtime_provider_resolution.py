...
def test_custom_endpoint_uses_model_key_env(monkeypatch):
    """provider: custom can keep its API key in .env via model.key_env."""
    monkeypatch.setattr(rp, "resolve_provider", lambda *a, **k: "openrouter")
    monkeypatch.setattr(
        rp,
        "_get_model_config",
        lambda: {
            "provider": "custom",
            "base_url": "https://my-api.example.com/v1",
            "key_env": "HERMES_CUSTOM_MODEL_API_KEY",
        },
    )
    monkeypatch.setenv("HERMES_CUSTOM_MODEL_API_KEY", "custom-env-key")
    ...
    assert resolved["base_url"] == "https://my-api.example.com/v1"
    assert resolved["api_key"] == "custom-env-key"
