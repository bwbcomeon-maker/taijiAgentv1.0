...
class TestResolvePath:
    ...
    def test_falls_back_to_hermes_workspace_before_process_cwd(self, monkeypatch, tmp_path):
        """Without TERMINAL_CWD, relative paths use HERMES_WORKSPACE when available."""
        monkeypatch.delenv("TERMINAL_CWD", raising=False)
        monkeypatch.setenv("HERMES_WORKSPACE", str(tmp_path))
        from tools.file_tools import _resolve_path

        result = _resolve_path("articles/demo/draft_v1.md")
        assert result == tmp_path / "articles" / "demo" / "draft_v1.md"
