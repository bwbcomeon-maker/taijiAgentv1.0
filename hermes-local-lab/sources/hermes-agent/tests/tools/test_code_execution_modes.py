...
class TestResolveChildCwd(unittest.TestCase):
    ...
    def test_project_without_terminal_cwd_uses_getcwd(self):
        env = {k: v for k, v in os.environ.items() if k not in {"TERMINAL_CWD", "HERMES_WORKSPACE"}}
        with patch.dict(os.environ, env, clear=True):
            self.assertEqual(_resolve_child_cwd("project", "/tmp/staging"), os.getcwd())

    def test_project_uses_hermes_workspace_when_terminal_cwd_unset(self):
        import tempfile
        with tempfile.TemporaryDirectory() as td:
            env = {k: v for k, v in os.environ.items() if k != "TERMINAL_CWD"}
            env["HERMES_WORKSPACE"] = td
            with patch.dict(os.environ, env, clear=True):
    ...
    def test_project_bogus_terminal_cwd_falls_back_to_getcwd(self):
        env = {k: v for k, v in os.environ.items() if k != "HERMES_WORKSPACE"}
        env["TERMINAL_CWD"] = "/does/not/exist/anywhere"
        with patch.dict(os.environ, env, clear=True):
            self.assertEqual(_resolve_child_cwd("project", "/tmp/staging"), os.getcwd())
