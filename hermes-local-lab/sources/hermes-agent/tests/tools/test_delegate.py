...
class TestDelegateHeartbeat(unittest.TestCase):
    ...
    def test_heartbeat_does_not_trip_idle_stale_while_inside_tool(self):
        ...
        def slow_run(**kwargs):
            heartbeat_continued.wait(timeout=0.5)
            return {"final_response": "done", "completed": True, "api_calls": 1}

        child.run_conversation.side_effect = slow_run

        # Make the two thresholds deliberately close while synchronizing on
        # the third heartbeat. The idle branch would stop before the event,
        # while the in-tool branch must keep touching the parent.
        with (
            patch("tools.delegate_tool._HEARTBEAT_INTERVAL", 0.01),
            patch("tools.delegate_tool._HEARTBEAT_STALE_CYCLES_IDLE", 2),
            patch("tools.delegate_tool._HEARTBEAT_STALE_CYCLES_IN_TOOL", 5),
        ):
            _run_single_child(
                task_index=0,
        ...
        self.assertTrue(
            heartbeat_continued.is_set(),
            "Heartbeat used the idle stale limit while child was inside a tool",
        )
