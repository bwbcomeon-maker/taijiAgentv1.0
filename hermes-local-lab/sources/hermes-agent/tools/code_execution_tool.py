...
def _resolve_child_cwd(mode: str, staging_dir: str) -> str:
    ...
    - ``project``: the session's TERMINAL_CWD (same as the terminal tool), then
      HERMES_WORKSPACE, then ``os.getcwd()`` if neither points at a real dir.
      Falls back to the staging tmpdir as a last resort so we never invoke
      Popen with a nonexistent cwd.
    """
    if mode != "project":
        return staging_dir
    raw = (os.environ.get("TERMINAL_CWD") or os.environ.get("HERMES_WORKSPACE") or "").strip()
    if raw:
