...
def _get_env_config() -> Dict[str, Any]:
    ...
    if env_type == "local":
        ...
        workspace_cwd = os.getenv("HERMES_WORKSPACE", "").strip()
        if workspace_cwd:
            expanded_workspace_cwd = os.path.expanduser(workspace_cwd)
            if os.path.isdir(expanded_workspace_cwd):
                default_cwd = expanded_workspace_cwd
    elif env_type == "ssh":
        default_cwd = "~"
    else:
        default_cwd = "/root"
