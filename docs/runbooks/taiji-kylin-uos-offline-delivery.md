...
工具可用不代表 Mac 验包已通过。Linux payload 可包含仅大小写不同的目录（例如 `terminfo/A` 与 `terminfo/a`）；在不区分大小写的文件系统上解包会合并目录，无法忠实还原归档，随后可能被链接/manifest 检查拒绝。完整物理验包需要区分大小写的受控解包空间。当前候选控制器清理子进程环境，不传递调用者的 `TMPDIR`，尚未提供 Mac 专用的区分大小写工作区流程；不能用忽略大小写、跳过校验或 Linux 构建通过来替代本地验包。在此环境问题闭合前，重新 `BUILD` 也不构成本地取回闭环方案。
