...
| 开发线与基线 | `main@e609b6b0fffcacd00b0c5c7d77ecadabe341e118`；`origin/main@18a607bc96a5689b184e4631a9606ce1cbb24e1e`，本地领先 1、远端未领先 |
| 写入边界 | 当前实施者是共享工作树及 Git index 唯一写入者；其他协作者只读 |
| 当前证据层 | 本地源码提交和随产品资源工作树候选；尚未形成运行接口或浏览器证据 |
| 已完成 | 工具链绑定；固定上游检出；273 个源角色、分区和许可入库候选；只读加载、完整性/版本校验、详情读取和安全错误；双哨兵单测 |
| 未完成 | 中文展示/运行适配、F01–F12 产品闭环、A01–A17 完整验收、前端 UX QA |
| 当前出口 | 阶段 0 已闭合；阶段 1 固定原文、许可、清单和加载器已具备，中文展示/运行适配继续分批 |
...
| F02 内置角色目录 | 实施中 | 完整固定原文、许可、分区清单和加载器已入库候选；中文层、适配层及发布对账未完成 |
...
| A02 目录对账 | 实施中 | 273 个固定源、分区与许可在临时目录和随产品资源一一校验；中文详情、有效提示词和排除记录未完成 |
