...
The script derives the slug from the final path segment after the role-ID namespace: replace runs of non-ASCII-lowercase-or-digit characters with one hyphen and strip edge hyphens. Append the first 12 hexadecimal characters of the full role-ID SHA-256, for example `sales-engineer-52d4228afb02.webp`. It never overwrites a different digest and emits reviewer fields as `null` until approval. It also writes `pilot-generation-record.json` containing the exact 12-role set, each role's name/category/summary/capabilities input snapshot, final prompt, model identifier, source PNG SHA-256 and dimensions, WebP conversion parameters, output digest, and review state.

The built-in Image 2.0 tool produced 1254×1254 PNGs in this run and exposes no size parameter. Preserve each original file's `generated_source_sha256` and `generated_source_dimensions`; use the build-time `normalize_generated_png()` helper with fixed Pillow LANCZOS resampling to create the 1024×1024 PNG input. Preserve `normalized_source_sha256`, `normalized_source_dimensions`, and `normalization_method` alongside the original evidence. The converter still refuses anything except a fully decoded 1024×1024 PNG; the normalization does not relax the conversion gate. Keep the per-role provenance and exact final prompts in `/tmp/zhinang-image2-pilot/generation-inputs.json`, which the conversion command reads before emitting its audit record. The installed Pillow extension in this run is x86_64, so prefix the specified Python 3.13 commands with `arch -x86_64`; this is a build-only architecture selection and adds no product dependency.
...
Keep the original built-in PNGs outside the repository at the returned generated-image paths. Save their LANCZOS-normalized PNGs under `/tmp/zhinang-image2-pilot/<role-slug>.png`, verify each decoded normalized input is exactly 1024×1024, visually reject any text artifact or role mismatch, then run the conversion command. Inspect the AI search role at 96px with only its professional character, three linked webpage nodes and one citation beacon; exclude the globe, plant, books, additional panels and decorative props.

- [ ] **Step 4: Validate all twelve files**

Before generation and again after conversion, run `/Users/bwb/Documents/工作/taiji-agentv1.0/hermes-local-lab/sources/hermes-agent/venv/bin/python /Users/bwb/Documents/工作/taiji-agentv1.0/scripts/check-local-change-safety.py` from the repository root. Run the Task 1 loader command and the exact `test_prepare_zhinang_role_images.py` command above with the build-only architecture selection described in Step 2. The latter fully decodes every output, calls `image.verify()`, reloads pixels, and asserts WebP format and 512×512 dimensions. It also asserts the manifest and generation record contain exactly the fixed 12 role IDs from the design, twelve distinct paths, each normalized source PNG record is 1024×1024, original source dimensions and SHA-256 are separately retained, every WebP is at most 160 KiB, and every prompt/input/output digest is present.
