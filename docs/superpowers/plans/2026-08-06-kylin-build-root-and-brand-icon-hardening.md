...
HEAD:     implementation checkpoint (verify with `git rev-parse HEAD` before execution; target Linux packaging and main integration remain pending)
~~~
...
~~~bash
python3 -m unittest tests.test_linux_desktop_packaging_static.LinuxDesktopPackagingStaticTest -v
~~~
...
- [x] **Step 1: 用 GPT Image 2.0 生成小尺寸适配候选**
...
- [x] **Step 2: 在 macOS 生成确定性尺寸和 ICO**

使用系统 sips 派生 PNG；macOS 的 sips 不支持 ICO 时使用已核验的 ffmpeg PNG-backed ICO 生成，不在制包机运行图片生成：
...
# 若 sips 不支持 ICO，使用本机已核验的 PNG-backed ICO 生成工具（本次使用 ffmpeg），
# 生成后必须由 validate_icon_assets.py 检查 ICO 头、PNG payload 和尺寸。
~~~
...
- [x] **Step 4: 写图标链失败测试并运行 RED/GREEN**
...
- [x] **Step 2: 增加 Linux desktop-id/WM_CLASS 合同**
...
- [x] **Step 3: 运行 Web/desktop 静态与 JS 测试**

~~~bash
pytest -q hermes-local-lab/sources/hermes-webui/tests/test_pwa_manifest_sw.py
python3 -m unittest tests.test_linux_icon_chain -v
...
- [x] **Step 3: 扩展 payload contract 和安装后 native verify**
...
hermes-local-lab/scripts/taiji-native-verify 在 installed-production 下用 shell/file/cmp 检查 512 资源图标与 Web favicon-512.png 同源，并输出 [OK] Product icon chain is consistent 或 [FAIL]；构建期的 Python validator 不额外进入安装 payload。

- [x] **Step 4: 运行 payload/DEB 静态回归**
...
结果：退出码为 0；167 项通过，2 项按当前 macOS 环境条件跳过（PyYAML 未安装、Unix socket 创建被沙箱拒绝），原因已保留在命令输出和交付报告边界中。
...
当前 macOS 结果：`python3 -m unittest discover -s tests -p 'test_*.py' -v` 未通过，失败集中在未创建的 `hermes-local-lab/sources/hermes-agent/venv`（Python runtime/license 测试）；这属于制包机/项目 venv 前置条件，不能替代 Linux amd64 制包机回归。

- [x] **Step 4: 运行前端 UX QA 源码审计**
