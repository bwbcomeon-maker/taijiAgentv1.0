...
## 7. 27 类 must-first 测试

以下编号是 Task B 的验收索引。类别 3 是当前五个 Provider 已真实通过的
characterization GREEN，必须先单独落证且不得为制造 RED 改坏；其余类别必须
先证明当前实现因目标断言失败，再做 GREEN。RED 不能来自测试语法、fixture、
import 或环境错误。
...
2. **Custom aliases without Secret lookup** —
   ...
   `custom`、`custom-image`、`custom:*` 一律归 `custom` family；
   `custom:dashscope` 不得借用内置 DashScope family，且 Secret seam 调用数
   为零；不要求临时发明 exception 契约。
3. **Known built-in model exactness** —
   `test_builtin_known_image_models_resolve_exactly`：五个内置 Provider 使用
   各自精确 `_model`/Doubao `_resolve_model` 返回所选 model。该项为既有
   characterization GREEN，不是行为性 RED；实现不得为制造 RED 改坏它。
...
- 类别 3 有独立 characterization GREEN 证据，其余 26 类均有真实 RED 原因
  和 GREEN 通过证据；
