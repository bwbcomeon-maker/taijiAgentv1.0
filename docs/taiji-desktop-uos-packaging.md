...
完全离线交付使用根目录下的 `taijiagent 打包交付/00_制包机_生成离线交付包.sh`。该脚本在联网 Linux amd64 制包机上校验源码包、准备构建工具、生成 Linux Python venv、安装 Linux Electron runtime、执行 DEB 构建，并生成唯一 DEB、SHA256 sidecar、manifest、构建报告、`.build-success` 和当前验收工具；当前 v3 不再生成 `离线依赖/` APT 仓库。正式构建不使用系统 `uv` 或 PATH 后备：只下载并验证 `uv 0.12.2` 的官方 Linux x86_64 GNU 归档（归档 SHA256 `d66e96b5…c41bf4`，可执行文件 SHA256 `72c5f455…68e23e2`）；Node.js 也固定为 `22.23.1` 的官方 Linux x64 归档（归档 SHA256 `9749e988…ee578`，可执行文件 SHA256 `93956de2…8e068`）。Electron 必须来自本轮受控缓存中与 canonical policy 固定 SHA256 匹配的 Linux x64 ZIP，最终 staged `dist/` 全文件比对通过后才可入包。Python venv 只允许对提交态 `uv.lock` 执行一次写入式 strict sync，构建器随后只做 `--locked --check` 只读复核；正式入口只接受未设置或 `TAIJI_UV_LOCK_MODE=strict`，明确拒绝 `auto`/`unlocked`、现场 lock refresh 和二次无锁 `pip install`。WebUI 的 Python requirements 必须是 Agent 直接依赖在 lock 中的精确版本子集，同步后还会实际导入并核对版本。只有与 lock 同步维护并经过验证的内网镜像才能覆盖 `TAIJI_UV_INDEX_URL`。
